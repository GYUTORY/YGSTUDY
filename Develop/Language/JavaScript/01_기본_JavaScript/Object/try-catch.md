...
### try-catch의 필요성
- **오류 처리**: 예상치 못한 오류에 프로그램이 멈추는 것을 막는다
- **사용자 경험**: 오류가 나도 애플리케이션은 계속 동작한다
- **디버깅**: 오류 정보를 모아 문제 해결에 쓴다
- **리소스 정리**: finally 블록에서 정리 작업을 보장한다

### 기본 개념
- **try 블록**: 오류가 날 수 있는 코드를 넣는다
- **catch 블록**: 오류가 났을 때 실행할 코드를 넣는다
...
IEEE 754 부동소수점이 무한대와 `NaN` 을 정상적인 값으로 정의한다. 그래서 다른 언어의 `ZeroDivisionError` 같은 것이 없다. 나눗셈 결과가 이상해도 **아무 일도 일어나지 않은 것처럼 계속 흘러간다.**
...
### 결론
try-catch는 JavaScript에서 예외를 처리하는 핵심 구문이다.
에러 타입을 제대로 골라 써야 구체적인 오류 정보가 남는다.
리소스 정리는 finally 블록에 넣어 보장한다.
중첩 try-catch 로 오류 처리를 더 잘게 나눈다.
비동기 작업에서는 async/await 와 같이 써야 에러 처리가 깔끔해진다.
오류 로깅과 사용자에게 보여줄 메시지를 함께 챙기면 디버깅도 사용자 경험도 나아진다.

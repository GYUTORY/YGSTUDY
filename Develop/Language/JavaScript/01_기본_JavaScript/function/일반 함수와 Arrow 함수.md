...
JavaScript에서 함수를 정의하는 방법은 크게 두 가지다. 일반 함수와 화살표 함수. 문법이 조금 다른 정도로 보이지만 `this` 바인딩과 호이스팅이 갈리기 때문에, 상황을 보고 골라 써야 한다.

### 함수 정의 방식의 필요성
- 가독성 — 어떻게 정의했는지만 봐도 그 함수를 어떻게 쓸 생각인지 드러난다
- `this` 바인딩 — 함수 안에서 `this`가 가리키는 객체가 서로 다르다
- 메모리 — 함수를 만들고 실행하는 비용이 다르다
- 호이스팅 — 선언이 끌어올려지느냐 아니냐가 갈린다
...
일반 함수는 JavaScript의 전통적인 함수 정의 방식이고, 두 가지 형태로 나뉜다.
...
ES6에서 들어온 화살표 함수는 기존 함수 정의 방식을 줄여 쓴 문법이다.
...
#### this 바인딩의 핵심 차이
가장 큰 차이는 `this` 키워드가 동작하는 방식이고, 함수를 어떻게 호출하느냐와 직결된다.
...
**일반 함수에서의 this 변화**
일반 함수는 호출 방식에 따라 `this`가 바뀐다. 메서드로 호출하면 그 객체를, 그냥 함수로 호출하면 전역 객체를 참조한다. JavaScript의 함수가 일급 객체이면서 동시에 메서드로도 쓰이기 때문이다.

일반 함수의 `this`는 실행 컨텍스트가 만들어질 때 정해진다. 함수가 호출되면 JavaScript 엔진이 새로운 실행 컨텍스트를 만들고 그때 `this` 바인딩을 설정한다. 호출 방식별로 정리하면 이렇다.

1. **메서드 호출**: 점 표기법이나 대괄호 표기법으로 호출되면, 점 앞의 객체가 `this`가 된다.
2. **일반 함수 호출**: 어떤 객체의 프로퍼티로 호출되지 않으면, 전역 객체가 `this`가 된다. strict mode에서는 `undefined`다.
3. **생성자 호출**: `new` 연산자와 함께 호출되면, 새로 생성되는 빈 객체가 `this`가 되고, 함수 실행 후 이 객체가 반환된다.
4. **명시적 호출**: `call`, `apply`, `bind` 메서드를 쓰면, 첫 번째 인자로 넘긴 객체가 `this`가 된다.

**화살표 함수에서의 this 고정**
화살표 함수는 자기 `this`가 없다. 정의된 시점의 상위 스코프 `this`를 그대로 쓴다. 애초에 `this`를 바인딩하지 않으니 동작이 예측 가능하다.

화살표 함수는 실행 컨텍스트를 만들 때 자기 `this` 바인딩을 하지 않는다. 대신 정의된 시점의 렉시컬 환경에서 `this`를 찾는다. 변수를 찾는 방식과 똑같다. 스코프 체인을 따라 올라가면서 `this` 바인딩을 가진 가장 가까운 외부 함수의 `this`를 쓴다.

렉시컬 환경은 코드가 작성된 구조를 따르니, 화살표 함수의 `this`는 코드를 쓰는 시점에 이미 정해진다. 런타임에 어떻게 호출하든 이 값은 바뀌지 않는다. `call`, `apply`, `bind`를 써도 화살표 함수의 `this`는 못 바꾼다.

#### 실행 컨텍스트와 this의 관계

JavaScript 코드가 실행되면 실행 컨텍스트(Execution Context)가 만들어진다. 코드가 도는 환경을 추상화한 개념이고, 다음 정보를 담는다.
...
일반 함수가 호출되면:
- 새로운 실행 컨텍스트가 콜 스택에 푸시된다
- 이 실행 컨텍스트에 `this` 바인딩이 설정된다
- 함수 실행이 끝나면 실행 컨텍스트가 콜 스택에서 팝된다

화살표 함수가 호출되면:
- 새로운 실행 컨텍스트가 만들어지지만 `this` 바인딩은 설정되지 않는다
- `this`를 참조하면 스코프 체인을 따라 외부 환경의 `this`를 찾는다
- 일반 변수를 참조하는 것과 똑같은 방식이다

### 2-1. 렉시컬 환경과 클로저

#### 렉시컬 환경의 개념
렉시컬 환경(Lexical Environment)은 JavaScript 코드가 실행되는 환경을 추상화한 개념이고, 식별자(변수, 함수 이름 등)와 그 값의 매핑 정보를 담는다. 함수, 코드 블록, 스크립트는 실행될 때 저마다 자기 렉시컬 환경을 하나씩 만든다.

렉시컬 환경은 두 가지 주요 구성 요소로 이뤄진다.

1. **환경 레코드(Environment Record)**: 현재 스코프의 모든 지역 변수, 함수 선언, 매개변수 등을 저장하는 객체다.
2. **외부 렉시컬 환경에 대한 참조**: 상위 스코프의 렉시컬 환경을 가리키는 참조다. 이 참조가 이어지면서 스코프 체인이 만들어진다.

#### 스코프 체인의 동작
변수를 참조하면 JavaScript 엔진은 현재 렉시컬 환경의 환경 레코드부터 뒤진다. 못 찾으면 외부 렉시컬 환경 참조를 따라 상위 스코프로 올라가며 계속 탐색한다. 이 과정이 스코프 체인이다.
...
#### 클로저의 개념
클로저(Closure)는 함수와 그 함수가 선언된 렉시컬 환경의 조합이다. 함수가 자기가 정의된 자리의 상위 스코프 변수에 접근할 수 있는 건 클로저 덕분이다.

핵심은 여기 있다. 함수가 만들어질 때 그 함수의 내부 슬롯 `[[Environment]]`에 현재 렉시컬 환경에 대한 참조가 저장된다. 나중에 이 함수가 어디서 호출되든, 이 참조로 함수가 정의된 시점의 렉시컬 환경에 접근한다.
...
위 예제에서 `createCounter`가 반환한 함수는 `createCounter`의 렉시컬 환경을 기억한다. `createCounter` 실행이 끝났는데도 반환된 함수가 `count` 변수에 접근할 수 있는 건 이 때문이다.

#### 화살표 함수와 렉시컬 환경
화살표 함수가 `this`를 렉시컬하게 바인딩하는 것도 클로저와 같은 원리다. 화살표 함수는 자기 `this` 바인딩을 만들지 않고, 정의된 시점의 렉시컬 환경에서 `this`를 찾는다. 변수를 찾는 방식과 완전히 같다.
...
화살표 함수 안에서 `this`를 참조하면, 스코프 체인을 따라 올라가며 `this` 바인딩을 찾는다. 일반 변수를 찾는 것과 똑같은 방식이다. 화살표 함수의 `this`를 "렉시컬하다"고 부르는 이유가 여기 있다.

#### 클로저의 메모리 관리
클로저는 편하지만 조심해서 써야 한다. 함수가 외부 렉시컬 환경을 참조하고 있으면, JavaScript 가비지 컬렉터는 그 환경을 메모리에서 못 지운다. 쓸데없는 클로저가 메모리 누수로 이어지는 이유다.
...
**일반 함수의 this 바인딩 규칙**
JavaScript에서 `this`는 함수를 어떻게 호출하느냐로 정해진다.
...
**화살표 함수의 this 상속**
화살표 함수는 이 규칙을 전부 무시하고, 렉시컬 스코프의 `this`를 그대로 쓴다.
...
**전통적인 해결 방법들**
화살표 함수가 나오기 전에는 `this` 바인딩 문제를 이런 식으로 우회했다.
...
#### 호이스팅의 개념
호이스팅은 JavaScript의 변수와 함수 선언이 스코프의 최상단으로 끌어올려지는 현상이다. JavaScript 엔진이 코드를 실행하기 전에 선언을 먼저 처리하기 때문에 그렇게 보인다.

정확히 말하면, 코드 실행 전 실행 컨텍스트의 생성 단계에서 변수와 함수 선언이 메모리에 등록된다. 그래서 선언이 "끌어올려진" 것처럼 보이는 효과가 생긴다.

#### 일반 함수의 호이스팅

**함수 선언식의 호이스팅**
함수 선언식은 전체 함수가 스코프의 최상단으로 끌어올려진다. 함수의 이름과 본문이 다 호이스팅되니, 함수 선언 전에도 호출할 수 있다.
...
이렇게 동작하는 건 JavaScript 엔진이 코드를 다음 순서로 처리하기 때문이다.
...
**함수 표현식의 호이스팅**
함수 표현식은 변수 선언만 호이스팅되고, 함수 할당은 실행 단계에서 일어난다. 선언 전에 호출하면 에러가 난다.
...
**let/const와 함수 표현식**
`let`이나 `const`로 선언된 함수 표현식은 Temporal Dead Zone(TDZ) 때문에 더 엄격하다.
...
화살표 함수는 언제나 표현식이라, 함수 표현식과 똑같이 동작한다. 변수 선언 방식(`var`, `let`, `const`)에 따라 호이스팅 동작이 갈린다.
...
**함수 선언 순서의 자유로움**
함수 선언식을 쓰면 함수의 선언 순서에 신경 쓰지 않고 코드를 짤 수 있다. 덕분에 코드의 논리적 순서를 더 명확하게 드러낼 수 있다.
...
#### arguments 객체란?
`arguments`는 함수 안에서 따로 선언 없이 쓸 수 있는 특별한 객체다. 함수에 넘어온 모든 인수를 유사 배열 형태로 담는다. 유사 배열은 배열처럼 인덱스로 접근할 수 있고 `length` 프로퍼티도 있지만, 배열의 메서드(`map`, `filter` 등)는 바로 못 쓰는 객체를 말한다.
...
**arguments 객체의 활용**
매개변수 개수를 유동적으로 처리해야 할 때 쓸 만하다. 인수가 몇 개 올지 미리 알 수 없거나, 가변 인수를 받는 함수를 짤 때다. ES6 이전에는 가변 인수를 처리하는 대표적인 방법이었다.
...
화살표 함수는 자기 `arguments` 객체를 만들지 않는다. 경량화된 함수로 설계됐기 때문이다. 화살표 함수의 목적은 간결한 함수 표현과 렉시컬 `this` 바인딩이다. `arguments` 객체, `super`, `new.target` 같은 바인딩을 만들지 않으니 함수 생성 비용이 줄고, 렉시컬 스코프의 일관성도 유지된다.

화살표 함수 안에서 `arguments`를 참조하면 `this`처럼 상위 스코프의 `arguments`를 집는다.
...
**화살표 함수에서 가변 인수 처리**
화살표 함수에서 가변 인수를 받으려면 ES6의 나머지 매개변수(Rest Parameters)를 써야 한다. 나머지 매개변수는 진짜 배열이라 배열 메서드를 바로 붙일 수 있어 더 편하다.
...
JavaScript 클래스에서 메서드를 정의하는 방법도 크게 두 가지다. 프로토타입 체인과 `this` 바인딩에서 갈린다.

**일반 메서드 (프로토타입 메서드)**
클래스 본문에 일반 함수로 정의한 메서드는 클래스의 `prototype`에 붙는다. 모든 인스턴스가 프로토타입 체인으로 이 메서드를 공유하니 메모리에 유리하다.
...
**화살표 함수 필드 (인스턴스 메서드)**
클래스 필드로 화살표 함수를 정의하면, 인스턴스마다 자기 메서드 복사본을 하나씩 갖는다. 메모리를 더 쓰는 대신 `this` 바인딩 문제가 알아서 해결된다.
...
프로토타입 메서드는 호출 방식에 따라 `this`가 달라진다. 메서드를 다른 변수에 담거나 콜백으로 넘기면 `this` 바인딩이 깨진다.
...
**화살표 함수 필드로 해결**
화살표 함수를 클래스 필드로 정의하면, `this`가 인스턴스에 영구적으로 바인딩된다. 메서드를 어떻게 호출하든 항상 제 인스턴스를 가리킨다.
...
React 클래스 컴포넌트에서 이벤트 핸들러를 정의할 때 `this` 바인딩 문제가 자주 터진다. 해결책은 여러 가지다.
...
#### 생성자 함수의 개념
생성자 함수는 `new` 키워드와 함께 호출해 새로운 객체 인스턴스를 만드는 함수다. JavaScript의 프로토타입 기반 객체 지향 프로그래밍에서 핵심이 되는 개념이다.
...
**new 연산자의 내부 동작 과정**
`new` 연산자로 함수를 호출하면 JavaScript 엔진은 다음 과정을 밟는다.

1. **빈 객체 생성**: 완전히 새로운 빈 객체가 메모리에 만들어진다.
2. **프로토타입 연결**: 새로 만들어진 객체의 내부 프로퍼티 `[[Prototype]]`이 생성자 함수의 `prototype` 프로퍼티가 가리키는 객체로 설정된다. 이렇게 프로토타입 체인이 만들어진다.
3. **this 바인딩**: 생성자 함수 내부의 `this`가 새로 만들어진 객체를 가리키도록 바인딩된다.
4. **코드 실행**: 생성자 함수의 본문이 실행되며, `this`에 프로퍼티를 추가하거나 초기화 작업을 한다.
5. **객체 반환**: 생성자 함수가 명시적으로 다른 객체를 반환하지 않으면, 새로 만들어진 객체가 자동으로 반환된다. 명시적으로 객체를 반환하면 그 객체가 반환되지만, 원시값을 반환하면 무시되고 `this`가 반환된다.
...
- **prototype 프로퍼티**: 함수 객체만 가지는 프로퍼티다. 생성자 함수로 쓰일 때, 이 함수로 생성될 인스턴스들이 상속받을 프로토타입 객체를 가리킨다.
- **[[Prototype]] (또는 __proto__)**: 모든 객체가 가지는 내부 슬롯이다. 객체 자신의 프로토타입 객체를 가리킨다. 프로토타입 체인을 타고 상속이 이뤄지는 실제 링크다.
...
**프로토타입 체인의 동작 원리**
객체의 프로퍼티나 메서드에 접근하면 JavaScript 엔진은 다음 순서로 탐색한다.
...
이 연결 구조가 프로토타입 체인이다. 모든 객체는 결국 `Object.prototype`에 이어지고, `Object.prototype`의 `[[Prototype]]`은 `null`이다.
...
**프로토타입을 통한 메모리 효율성**
생성자 함수의 `prototype`에 메서드를 정의하면, 모든 인스턴스가 같은 메서드를 공유한다. 반대로 생성자 함수 본문 안에서 메서드를 정의하면, 인스턴스마다 별도의 메서드가 만들어져 메모리를 버린다.
...
**생성자로 사용할 수 없는 이유**
화살표 함수에는 `prototype` 속성도 없고, 내부 메서드 `[[Construct]]`도 없다. 생성자 함수로 호출하려면 이 내부 메서드가 있어야 하는데 그게 없으니, `new` 키워드를 붙이는 순간 에러가 난다.
...
**내부적인 차이점**
일반 함수와 화살표 함수는 내부 메서드부터 다르다.

- **일반 함수**: `[[Call]]`과 `[[Construct]]` 내부 메서드를 모두 가진다. `[[Call]]`은 일반 호출 시, `[[Construct]]`는 `new`와 함께 호출 시 쓰인다.
- **화살표 함수**: `[[Call]]` 내부 메서드만 가지며, `[[Construct]]`는 없다. 그래서 생성자로 쓸 수 없다.

**렉시컬 this와 생성자의 비호환성**
화살표 함수의 `this`는 렉시컬 스코프에서 물려받는다. 화살표 함수를 생성자로 쓸 수 있다고 치면 `new` 연산자가 새로운 객체를 `this`로 바인딩해야 하는데, 화살표 함수의 `this`는 바꿀 수가 없다. 애초에 앞뒤가 맞지 않는다.
...
**함수 객체 생성의 차이점**
일반 함수든 화살표 함수든 함수를 만들 때 메모리를 쓴다. 다만 어디에 어떻게 만드느냐에 따라 효율이 갈린다.

화살표 함수 자체는 일반 함수보다 약간 가볍다. `arguments` 객체, `prototype` 속성, `super` 바인딩 같은 게 없으니 생성 비용이 그만큼 낮다. 그래도 실제 성능 차이는 미미해서 대부분은 체감하지 못한다.
...
**클로저와 메모리**
클로저는 외부 변수를 붙잡고 있으니, 쓸데없는 변수 참조는 만들지 않는 게 좋다.
...
**함수 호출 성능**
일반 함수와 화살표 함수의 실행 성능은 거의 같다. V8, SpiderMonkey 같은 현대 JavaScript 엔진은 두 가지 모두 고도로 최적화한다.
...
**스택 트레이스의 차이점**
스택 트레이스는 에러가 났을 때 함수 호출 경로를 보여준다. 함수에 이름이 붙어 있으면 디버깅이 훨씬 쉽다.
...
일반 함수와 화살표 함수의 차이는 문법에서 끝나지 않는다. JavaScript의 실행 컨텍스트와 렉시컬 환경을 다루는 방식 자체가 다르다. 각각의 특성을 알고 상황에 맞게 골라 써야 한다.
...
두 함수의 차이를 파고들면 결국 JavaScript의 실행 모델을 들여다보게 된다. 실행 컨텍스트, 렉시컬 환경, 스코프 체인, 프로토타입 체인이 전부 한 줄로 이어져 있다.
...
성능 차이보다 코드의 명확성과 유지보수성을 먼저 본다. 다만 인스턴스를 대량으로 만드는 상황이라면 프로토타입 메서드로 메모리를 아끼는 쪽을 고민한다.

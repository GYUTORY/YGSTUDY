...
updated: 2025-08-10
description: "JavaScript 생성자 함수와 new 키워드로 객체를 생성하고 초기화하는 방법"
---

# JavaScript 생성자 (Constructor) 개념과 사용법

## 생성자가 필요한 이유
생성자는 객체를 만들고 초기화하는 특별한 메서드다. 새 객체를 만들 때 그 객체의 초기 상태와 동작을 설정한다. JavaScript에서는 함수를 생성자로 써서 객체를 만든다.

### 생성자의 필요성
- **객체 생성**: 같은 구조의 객체를 여러 개 생성
- **초기화**: 객체의 초기 상태 설정
- **코드 재사용**: 공통된 객체 구조 정의
- **캡슐화**: 객체의 속성과 메서드 묶기
...
### 기본 개념
- **생성자 함수**: 객체를 생성하는 함수
- **new 키워드**: 생성자 호출에 쓰는 키워드
- **this 바인딩**: 새로 생성되는 객체 참조
- **프로토타입**: 인스턴스끼리 공유하는 속성과 메서드
...
### 결론
생성자는 JavaScript에서 객체를 만드는 전통적인 방법이다.
생성자 함수는 new 키워드로 호출해야 한다.
프로토타입을 쓰면 메서드를 메모리 효율적으로 공유할 수 있다.
ES6 클래스는 생성자 함수의 문법적 설탕이다.
안전한 생성자 패턴을 쓰면 new 키워드 없이도 동작하게 만들 수 있다.
생성자를 활용해 재사용 가능하고 유지보수하기 쉬운 코드를 작성하자.

...
### Map의 필요성
- **다양한 키 타입**: 문자열은 물론 객체나 함수도 키로 쓸 수 있다
- **삽입 순서 보장**: 요소를 추가한 순서 그대로 순회한다
- **성능 최적화**: 키-값 쌍을 자주 넣고 지우는 경우에 효율적이다
- **크기 추적**: 내장 size 속성으로 요소 개수를 바로 확인한다

### 기본 개념
- **키-값 쌍**: Map 의 기본 데이터 단위다
- **해시 테이블**: 내부적으로 해시 테이블을 써서 검색이 빠르다
...
### 결론
Map 은 키-값 쌍을 효율적으로 관리하는 강력한 자료구조다.
키 타입이 자유롭고 삽입 순서가 보장되니 그만큼 유연하다.
성능이 중요한 키-값 조작이면 Map 을 쓴다.
메모리 사용량과 직렬화 요구사항을 따져 보고 고른다.
메모리 누수가 걱정되면 WeakMap 을 쓴다.
Map 을 안전하게 다루려면 에러 처리를 제대로 붙인다.
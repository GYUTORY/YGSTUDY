...
평소 TypeScript 코드만 쓴다면 문제되지 않는다. 위험한 건 **경계 밖에서 값이 들어오는 자리**다. `JSON.parse` 로 만든 객체나 자바스크립트로 작성된 라이브러리가 넘긴 객체는 클래스를 거치지 않으므로 메서드가 없고, 타입만 맞춰 놓으면 컴파일러는 통과시킨다. 추상 클래스는 **직렬화·역직렬화 경계에서 형태를 보장해 주지 않는다.**
...
`new.target` 은 `new` 로 호출된 실제 생성자를 가리키므로, 하위 클래스로 생성하면 통과하고 추상 클래스로 직접 생성할 때만 걸린다.
...
`이(가)` 표기 자체가 한국어를 다루는 코드의 알려진 문제이기도 하다. 받침 유무로 조사가 갈리므로(`오리가` / `독수리가` vs `멍멍이가`) 문자열을 이어 붙이는 방식으로는 자연스러운 문장을 만들 수 없다. 마지막 글자의 유니코드 값으로 받침을 계산하면 해결된다.

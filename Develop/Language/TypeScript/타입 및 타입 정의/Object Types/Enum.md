...
숫자 Enum 은 오랫동안 타입 안전성이 약했다. 정의되지 않은 숫자를 그대로 받아들였는데, **TypeScript 5.0 에서 막혔다.** 그 버전부터 모든 enum 이 union enum 으로 취급되면서 멤버에 없는 숫자는 거부된다.
...
move(999);          // TS 5.0+ : error TS2345 — Direction 에 없는 값
                    // TS 4.9  : 통과했다
```

API 에서 받은 숫자를 그대로 Enum 타입 파라미터로 넘기는 코드가 꽤 많은데, 예전에는 위처럼 아무 숫자나 통과했다. 실측으로 4.9.5 는 통과하고 5.0.4 부터 `TS2345` 로 막는다. 다만 컴파일러가 막아 주는 건 **리터럴을 직접 넘길 때**뿐이고, `as Direction` 로 단언하거나 `number` 변수를 넘기면 여전히 빠져나간다. 런타임 값이라면 결국 범위 검사를 직접 해야 한다. 5년 전에 이걸로 결제 상태 코드가 엉뚱한 값으로 DB에 들어간 적이 있다. 숫자 Enum을 쓸 거면 경계값 검증을 따로 해야 한다.

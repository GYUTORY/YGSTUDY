...
TypeScript의 접근 제어자(Access Modifiers)는 클래스 안의 속성과 메서드를 외부에서 건드릴 수 있는지 정하는 기능이다.

### 접근 제어자의 필요성
- **캡슐화**: 객체의 내부 상태를 밖에서 못 건드리게 막는다
- **데이터 은닉**: 중요한 데이터에 직접 손대지 못하게 막는다
- **인터페이스 설계**: 밖에 내보낼 API를 분명히 한다
- **유지보수성**: 내부 구현을 바꿔도 밖에 영향이 덜 간다

### 기본 개념
- **public**: 어디서나 접근할 수 있다 (기본값)
- **private**: 클래스 안에서만 접근할 수 있다
- **protected**: 클래스 안과 상속받은 클래스에서만 접근할 수 있다
...
**`private` 는 컴파일 타임에만 존재한다.** 위 두 줄이 막히는 건 컴파일러가 막는 것이고, 컴파일된 자바스크립트에는 아무 보호 장치도 남지 않는다.

```javascript
// 위 클래스를 tsc --target ES2022 로 컴파일한 결과
class BankAccount {
    balance;              // ← private 이라는 표시가 없다. 그냥 공개 필드다
    accountNumber;
...
```javascript
Object.keys(account)      // ['balance', 'accountNumber']
JSON.stringify(account)   // {"balance":15000,"accountNumber":"123-456-789"}
```

**계좌번호와 잔액이 `JSON.stringify` 한 번에 그대로 나간다.** 응답 객체를 그대로 직렬화하는 API 에서 내부 필드가 유출되는 사고가 이 경로로 일어난다. `private` 는 보안 기능이 아니라 **협업용 표시**다.

TypeScript 안에서도 완전하지 않다. 점 접근은 막히지만 대괄호 접근은 통과한다.

```typescript
account.balance;        // error TS2341: Property 'balance' is private
account['balance'];     // 에러 없음 — 의도적으로 열어 둔 우회로다
```

진짜로 런타임까지 막으려면 자바스크립트 표준인 `#` 프라이빗 필드를 쓴다.
...
`#` 필드는 컴파일 후에도 `#balance` 로 남아 `Object.keys` · `JSON.stringify` · 대괄호 접근 어디에도 노출되지 않는다. 클래스 바깥에서 접근하면 문법 오류다.

정리하면 **`private` 는 실수를 막고 `#` 는 접근을 막는다.** 민감한 값을 담거나 외부 코드가 건드릴 여지가 있으면 `#` 를 쓴다.
...
마지막 두 줄이 이 예제의 문제를 스스로 보여 준다. `user` 의 타입은 `ReadableUser` 이고 모든 프로퍼티가 `readonly` 인데, **그 값이 바뀌었다.**

`readonly` 는 **그 참조를 통한 쓰기만** 막는다. 같은 객체를 가리키는 다른 참조가 있으면 아무 소용이 없다.

```typescript
const u = new User(1, '홍길동');
const ro: ReadableUser = u;   // readonly 뷰

ro.name = 'x';    // error TS2540: Cannot assign to 'name' because it is a read-only property
u.name = '김철수'; // 통과 — 같은 객체인데 이쪽 참조는 readonly 가 아니다

console.log(ro.name);  // "김철수"
```

`createUser` 가 `ReadableUser` 를 반환한 건 좋은 의도지만, `UserManager` 가 `Map` 안에 **원본 `User` 인스턴스를 그대로 들고 있고** `updateUser` 가 그걸 직접 수정하므로 밖으로 나간 readonly 참조도 함께 바뀐다. `readonly` 는 `private` 와 마찬가지로 컴파일 타임 표시일 뿐 런타임에 `Object.freeze` 같은 걸 걸어 주지 않는다.

정말 불변으로 주려면 복사본을 내보낸다. 같은 파일의 `AdminUser.getPermissions()` 가 `[...this.permissions]` 로 복사본을 반환하는 게 정확히 이 처리다 — 그쪽은 제대로 했는데 여기서는 빠졌다.

```typescript
public getUser(id: number): ReadableUser | null {
    const user = this.users.get(id);
    return user ? { ...user } : null;   // 스냅샷을 준다
}
```

`updateUser` 에도 확인해 둘 게 있다. `if (updates.name)` 는 빈 문자열을 거른다. 이름을 `''` 로 바꾸려는 요청은 조용히 무시되고 반환값은 `true` 다 — 성공했다고 보고하면서 아무것도 안 한다. `if (updates.name !== undefined)` 가 의도에 맞는다.

`generateId()` 도 `Math.random()` 기반이라 충돌 가능성이 있고, 충돌하면 `Map.set` 이 기존 사용자를 **경고 없이 덮어쓴다.** 백만 분의 1이라 테스트에서는 안 잡히고 운영에서 드물게 터지는 부류다.
...
1. **최소 권한 원칙**: 필요한 만큼만 접근 권한을 준다
2. **캡슐화**: 내부 구현을 밖에서 못 건드리게 막는다
3. **인터페이스 설계**: 밖에 내보낼 API를 분명히 정한다
4. **상속 고려**: protected는 상속 관계를 보고 정한다

### 결론
TypeScript의 접근 제어자는 객체지향 프로그래밍의 캡슐화를 구현하는 핵심 도구다.
상황에 맞는 접근 제어자를 골라 데이터를 안전하게 지킨다.
내부 구현은 private으로 감추고, 밖에 드러낼 인터페이스는 public으로 분명히 한다.
상속 관계에서 열어 줄 범위는 protected로 조절한다.
게터와 세터를 두면 데이터를 안전하게 읽고 쓰게 할 수 있다.
접근 제어자로 API를 분명하게 설계해 두면 나중에 고치기 쉽다.

...
TypeScript 의 `object` 타입은 모든 객체 유형을 포함하는 일반 객체 타입이다.

### object 타입의 필요성
- 일반적인 객체 처리: 구체적인 구조를 알 수 없는 객체를 다룰 때
- 동적 객체: 런타임에 구조가 정해지는 객체
- 외부 라이브러리: 타입 정의가 없는 라이브러리의 객체
- 타입 안전성: 객체임은 보장하면서 유연하게 다루기

### 기본 개념
- 모든 객체 포함: 프로퍼티가 있는 JavaScript 객체 전부
- 구조 불명: 구체적인 프로퍼티 구조는 모름
...
### 결론
TypeScript 의 object 타입은 모든 객체를 포함하는 일반 타입이다.
구체적인 프로퍼티 구조를 알 수 없을 때 쓸모가 있다.
가능하면 구체적인 인터페이스나 타입을 쓰는 편이 낫다.
타입 가드와 타입 단언으로 object 타입을 안전하게 다룬다.
Record 타입을 쓰면 object 타입을 더 구체적으로 정의할 수 있다.
object 타입의 한계를 알고 맞는 상황에서만 쓴다.

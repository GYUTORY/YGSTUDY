...
TypeScript에서 `number` 타입은 숫자 값을 나타내는 기본 데이터 타입이다.

### number 타입의 필요성
- **수치 연산**: 덧셈, 뺄셈, 곱셈, 나눗셈 등 기본 연산
- **반복문**: for 루프의 카운터 변수
- **수학 함수**: Math 객체가 제공하는 수학 함수 사용
...
### 결론
TypeScript의 number 타입은 수치 연산과 계산에 쓰는 핵심 데이터 타입이다.
복잡한 수학 연산은 Math 객체가 제공하는 함수로 처리한다.
NaN, Infinity 같은 특수 값을 어떻게 처리하느냐가 중요하다.
부동소수점 연산에는 정밀도 한계가 있으니 필요하면 정수 연산으로 간다.
런타임 오류는 숫자 데이터의 유효성 검사와 안전한 변환으로 막는다.

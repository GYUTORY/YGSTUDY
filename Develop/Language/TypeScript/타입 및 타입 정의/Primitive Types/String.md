...
TypeScript에서 `string` 타입은 텍스트 데이터를 나타내는 기본 데이터 타입이다.
...
### 결론
TypeScript의 string 타입은 텍스트 데이터를 처리하는 기본 타입이다.
동적 문자열은 템플릿 리터럴로 만든다.
여러 문자열 조작은 내장 메서드로 처리한다.
복잡한 문자열 패턴은 정규표현식과 같이 쓴다.
문자열이 불변이라는 걸 알고 그에 맞는 메서드를 고른다.
성능이 걸리는 자리는 문자열 연산을 최적화한다.
런타임 오류는 안전한 문자열 처리로 막는다.

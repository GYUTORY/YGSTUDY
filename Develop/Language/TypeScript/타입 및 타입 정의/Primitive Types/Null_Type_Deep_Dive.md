...
이건 동작한다. 그리고 **중간에 함수를 불러도 풀리지 않는다.**

```typescript
if (obj["a"] !== null) {
  doSomething();
  console.log(obj["a"].length); // 에러 없음
}
```

TypeScript 4.7 · 4.9 · 5.0 · 5.4 · 5.9 에서 전부 확인했다. 컴파일러는 호출된
함수가 `obj` 를 건드릴 수 있다는 가정을 하지 않는다.

좁히기를 실제로 푸는 건 **그 대상에 대한 대입**이다.

```typescript
if (obj["a"] !== null) {
  obj["a"] = Math.random() > 0.5 ? null : "x";
  console.log(obj["a"].length); // error TS2531: Object is possibly 'null'.
}
```

바꿔 말하면 여기서 안전한 이유는 컴파일러가 똑똑해서가 아니라 **아무도 안
건드렸기 때문**이다. `doSomething()` 이 실제로 `obj["a"]` 를 null 로 만들면
런타임에 터지는데 컴파일러는 그걸 모른다. 그래서 값을 일찍 변수에 담아두는
패턴이 여전히 낫다 — 좁히기가 풀려서가 아니라, 검사한 값과 쓰는 값이 같다는
보장을 코드에 박아두기 때문이다.

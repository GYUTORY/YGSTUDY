...
객체지향 프로그래밍(Object-Oriented Programming, OOP)은 객체(Object)를 기반으로 프로그램을 구성하는 프로그래밍 패러다임이다. Java는 대표적인 객체지향 언어로, 클래스와 객체를 활용하여 코드의 재사용성을 높이고 유지보수를 용이하게 만든다.
...
#### 캡슐화의 개념
데이터(필드)를 외부에서 직접 접근하지 못하도록 `private`으로 보호하고, Getter와 Setter를 이용하여 데이터를 안전하게 접근하는 기법이다.
...
#### 상속의 개념
기존 클래스의 속성과 메서드를 새로운 클래스가 재사용할 수 있게 하는 기법이다. 코드 중복을 줄이고 계층 구조를 만들 수 있다.
...
#### 추상화의 개념
복잡한 시스템에서 핵심적인 개념이나 기능을 간추려 표현하는 기법이다. 추상 클래스와 인터페이스를 통해 구현된다.

...
JVM은 크게 **Class Loader**, **Runtime Data Area**, **Execution Engine**, **Native Interface** 네 부분으로 구성된다.

```mermaid
graph TB
    subgraph JVM
        direction TB
        CL["Class Loader<br/>Loading → Linking → Initialization"]
        ...
        JNI["Native Interface (JNI)<br/>C/C++ 라이브러리 호출"]
    end
...
| 구성 요소 | 역할 |
|-----------|------|
| Class Loader | .class 파일을 찾아 메모리에 올린다 |
| Runtime Data Area | 실행 중 데이터를 저장하는 메모리 영역 |
| Execution Engine | 바이트코드를 해석하고 실행한다 |
| Native Interface | C/C++ 네이티브 코드를 호출할 때 사용한다 |
...
```mermaid
graph TB
    REQ["클래스 로딩 요청"] --> APP["Application ClassLoader<br/>(classpath의 클래스)"]
    APP -->|"못 찾으면 부모에게 위임"| PLAT["Platform ClassLoader<br/>(JDK 확장 모듈, JDK 9+)"]
    PLAT -->|"못 찾으면 부모에게 위임"| BOOT["Bootstrap ClassLoader<br/>(java.lang.*, java.util.* 등 핵심 클래스)"]
    BOOT -->|"여기서도 못 찾으면"| ERR["ClassNotFoundException"]

    style BOOT fill:#2d3748,color:#fff
    style ERR fill:#c53030,color:#fff
```
...
```mermaid
graph TB
    subgraph shared["공유 영역 (모든 스레드)"]
        direction TB
        MA["Method Area<br/>클래스 메타데이터, static 변수, 상수 풀<br/>JDK 8+: Metaspace (Native Memory)"]
        subgraph HEAP["Heap"]
            direction LR
            ...
            OG["Old Generation<br/>장기 생존 객체"]
        end
    end

    subgraph perThread["스레드 전용 영역 (스레드마다 독립)"]
        direction LR
...
객체가 Heap에서 이동하는 흐름은 다음과 같다:

```mermaid
graph LR
    NEW["new 객체 생성"] --> EDEN["Eden"]
    EDEN -->|"Minor GC 생존"| SURV["Survivor (S0/S1)"]
    SURV -->|"Age 임계값 초과"| OLD["Old Generation"]
    EDEN -->|"객체가 너무 크면<br/>바로 Old Gen"| OLD
    OLD -->|"Old Gen 가득 참"| FGC["Full GC 발생<br/>(Stop-The-World)"]

    style FGC fill:#c53030,color:#fff
```
...
```mermaid
graph TB
    BC["바이트코드"] --> INTERP["Interpreter<br/>한 줄씩 해석 실행"]
    INTERP -->|"반복 실행 감지<br/>(invocation counter 임계값 초과)"| JIT["JIT Compiler<br/>네이티브 코드로 컴파일"]
    JIT --> CC["Code Cache에 저장"]
    CC -->|"이후 실행"| NATIVE["네이티브 코드 직접 실행"]
...
초록색 객체(A, B, C)는 GC Roots에서 도달 가능하므로 살아남는다. 빨간색 객체(D, E, F)는 도달할 수 없으므로 GC 대상이다.
...
아래는 상황별로 어떤 도구를 먼저 사용해야 하는지 정리한 흐름이다:

```mermaid
graph TB
    SYM["장애 증상 발생"] --> Q1{"어떤 증상?"}
    
    Q1 -->|"OOM / 메모리 부족"| JSTAT["jstat -gcutil<br/>GC 상태 확인"]
    Q1 -->|"응답 지연 / 행"| JSTACK["jstack<br/>스레드 덤프"]
    Q1 -->|"프로세스 확인"| JPS["jps -l<br/>PID 확인"]
    
    JSTAT -->|"Old Gen 계속 증가"| JCMD["jcmd GC.heap_dump<br/>Heap Dump 생성"]
    JSTAT -->|"Metaspace 증가"| JCMD2["jcmd VM.classloader_stats<br/>클래스 로더 확인"]
    JSTACK -->|"BLOCKED 스레드 다수"| DEADLOCK["데드락 / 락 경합 확인"]
    JCMD --> MAT["Eclipse MAT로 분석"]

    style SYM fill:#c53030,color:#fff
```

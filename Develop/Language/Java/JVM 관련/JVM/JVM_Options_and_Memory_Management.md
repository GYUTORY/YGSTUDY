...
│                                                  │
│  ┌──────────────┐  ┌────────────────────────┐    │
│  │  Thread      │  │    Direct Memory       │    │
│  │  Stacks      │  │  (NIO ByteBuffer)      │    │
│  │  (-Xss)      │  │ (-XX:MaxDirectMemory)  │    │
│  └──────────────┘  └────────────────────────┘    │
└─────────────────────────────────────────────────┘
```
...
Heap 크기를 정하는 기준은 딱히 공식이 없다. 실제로는 이렇게 접근한다:
...
```
컨테이너 메모리 제한: 2048MB
├── Heap (-Xmx):          1280MB  (약 62%)
├── Metaspace:              256MB
├── Thread Stacks:          200MB  (200 threads x 1MB)
├── Code Cache:              64MB
├── Direct Memory:          100MB
├── JVM 내부 + 여유:        148MB
└── 합계:                  2048MB

...
## DevSecOps 파이프라인 전체 구조

PR이 올라오면 보안 스캔이 어떤 순서로 돌아가는지 전체 흐름이다. 각 스캔은 병렬로 실행되고, 모든 결과를 종합해서 보안 게이트에서 통과 여부를 판단한다.

```mermaid
flowchart LR
    A[PR 생성 / 커밋 푸시] --> B{병렬 실행}
    
    B --> C[SAST<br/>Semgrep / SonarQube<br/>코드 정적 분석]
    B --> D[SCA<br/>Trivy / Snyk<br/>의존성 CVE 스캔]
    B --> E[Secret Scan<br/>gitleaks / trufflehog<br/>시크릿 탐지]
    ...
    F -->|통과| G[이미지 빌드]
    F -->|실패| H[PR 머지 차단<br/>담당자 알림]
    
    G --> I[Image Scan<br/>Trivy<br/>컨테이너 취약점]
    I -->|통과| J[스테이징 배포]
    I -->|실패| H
    
    J --> K[DAST<br/>OWASP ZAP<br/>동적 분석]
    K -->|통과| L[프로덕션 배포]
    K -->|실패| M[취약점 리포트<br/>수정 후 재배포]
```

PR 파이프라인에서는 SAST, SCA, Secret Scan만 돌린다. DAST는 시간이 오래 걸리기 때문에 스테이징 배포 후에 별도로 실행하거나 야간 스케줄로 돌린다. 이미지 스캔은 SAST를 통과한 코드에 대해서만 빌드하고 스캔하는 구조가 runner 비용을 아끼는 데 도움이 된다.

아래는 각 보안 도구가 파이프라인의 어느 시점에 동작하고, 어떤 데이터를 주고받는지 보여주는 아키텍처 구성도다.

```mermaid
flowchart TB
    subgraph DEV["개발 환경"]
        A1[개발자 로컬]
        A2[pre-commit hook<br/>gitleaks]
    end
    
    subgraph PR_PIPELINE["PR 파이프라인 — 병렬 실행"]
        B1[SAST<br/>Semgrep / SonarQube]
        B2[SCA<br/>Trivy fs scan]
        B3[Secret Scan<br/>gitleaks / trufflehog]
    end
    
    subgraph GATE["보안 게이트"]
        C1{결과 종합<br/>Critical → 차단<br/>High → 단계별<br/>Medium → 경고}
    end
    
    subgraph BUILD["빌드 & 이미지"]
        D1[Docker Build]
        D2[Image Scan<br/>Trivy image]
        D3[SBOM 생성<br/>Syft / Trivy]
    end
    
    subgraph DEPLOY["배포 & 동적 분석"]
        E1[스테이징 배포]
        E2[DAST<br/>OWASP ZAP<br/>Baseline Scan]
        E3[야간 스케줄<br/>ZAP Full Scan]
    end
    
    subgraph REPORT["리포팅"]
        F1[GitHub Security Tab<br/>SARIF 업로드]
        F2[Slack / 이메일 알림]
        F3[SBOM 저장소<br/>Dependency-Track]
    end
...
개발자가 커밋하면 pre-commit hook이 시크릿을 1차로 걸러내고, PR이 올라가면 SAST/SCA/Secret Scan이 병렬로 돈다. 보안 게이트를 통과한 코드만 이미지로 빌드되고, 이미지 스캔과 SBOM 생성이 동시에 실행된다. 스테이징에 배포되면 DAST가 돌고, 모든 결과는 SARIF 포맷으로 GitHub Security 탭에 모인다.
...
DAST는 실행 중인 서버가 필요하기 때문에, 다른 보안 스캔과 실행 시점이 다르다. 아래는 DAST가 파이프라인에서 어떤 흐름으로 동작하는지 보여준다.

```mermaid
flowchart LR
    subgraph CI["CI 파이프라인"]
        A[보안 게이트 통과] --> B[Docker 이미지 빌드]
        B --> C[이미지 스캔 통과]
    end
    ...
    subgraph DAST_SCAN["DAST 실행"]
        F[ZAP Baseline Scan<br/>passive — 매 배포]
        G[ZAP Full Scan<br/>active — 야간/릴리스]
    end
    
    subgraph RESULT["결과 처리"]
        H{취약점 발견?}
        I[SARIF 리포트 생성]
        J[GitHub Issue 자동 생성]
        K[프로덕션 배포 진행]
    end
...
Baseline Scan은 5~10분이면 끝나서 매 배포마다 돌릴 수 있다. Full Scan은 30분 이상 걸리기 때문에 야간 cron이나 릴리스 전에만 실행한다. 두 스캔 모두 스테이징 환경이 정상 동작하는 상태(헬스체크 통과)에서 시작해야 한다. ZAP이 서버가 뜨기 전에 돌면 모든 요청이 실패해서 결과가 무의미해진다.
...
SBOM(Software Bill of Materials)은 프로젝트에 포함된 모든 의존성의 목록이다. 어떤 라이브러리가 어떤 버전으로 들어가 있는지, 라이선스는 뭔지를 기록한다. Log4Shell 사태 때 "우리 서비스에 Log4j가 들어가 있나?"라는 질문에 바로 답할 수 있느냐 없느냐의 차이가 SBOM이 있느냐 없느냐였다.

### SBOM 생성 자동화 흐름

SBOM은 빌드 시점에 자동으로 생성하고, 중앙 저장소에 보관하는 구조를 만들어야 한다.

```mermaid
flowchart TD
    subgraph BUILD["빌드 파이프라인"]
        A[소스 코드] --> B[의존성 해석<br/>npm install / gradle build]
        B --> C[애플리케이션 빌드]
        C --> D[Docker 이미지 빌드]
    end
    
    subgraph SBOM_GEN["SBOM 생성 — 병렬"]
        E[Syft<br/>소스 코드 SBOM]
        F[Trivy<br/>이미지 SBOM]
    end
    
    subgraph FORMAT["출력 포맷"]
        G[CycloneDX JSON]
        H[SPDX JSON]
    end
    
    subgraph STORE["저장 & 분석"]
        I[Dependency-Track<br/>SBOM 관리 플랫폼]
        J[취약점 지속 모니터링<br/>새 CVE 발표 시 알림]
        K[라이선스 컴플라이언스<br/>GPL 등 확인]
    end
...
소스 코드 기반 SBOM과 이미지 기반 SBOM을 둘 다 만드는 이유가 있다. 소스 코드 SBOM은 npm, gradle 같은 패키지 매니저의 의존성을 잡고, 이미지 SBOM은 베이스 이미지에 포함된 OS 패키지(openssl, zlib 등)까지 잡는다. 둘 다 있어야 빠짐없이 커버된다.

### Syft로 SBOM 생성

Syft는 Anchore에서 만든 SBOM 생성 도구다. 소스 코드 디렉토리와 컨테이너 이미지 둘 다 지원한다.
  ...
  sbom:
    ...
    steps:
      ...
      - name: Generate Image SBOM
        run: syft my-app:${{ github.sha }} -o cyclonedx-json=sbom-image.json
...
CycloneDX와 SPDX 중에서 CycloneDX를 쓰는 팀이 늘고 있다. OWASP에서 만든 포맷이라 보안 도구와의 연동이 좋고, Dependency-Track이 CycloneDX를 기본으로 지원한다.

### Trivy로 SBOM 생성

Trivy는 취약점 스캔 도중 SBOM도 같이 생성할 수 있다. 별도 도구를 추가하기 싫으면 Trivy 하나로 스캔과 SBOM을 동시에 처리할 수 있다.
...
SBOM을 생성만 하고 저장소에 안 넣으면 의미가 없다. Dependency-Track은 SBOM을 업로드하면 알려진 CVE와 자동으로 매칭해서, 새로운 취약점이 발표됐을 때 우리 서비스에 해당하는지 알림을 보내준다.

```bash
# Dependency-Track API로 SBOM 업로드
...
SBOM을 매 빌드마다 올리면 특정 버전에 어떤 의존성이 들어갔는지 추적할 수 있다. Log4Shell 같은 사태가 터졌을 때 "지금 프로덕션에 배포된 버전에 Log4j가 있나?"를 Dependency-Track에서 바로 검색할 수 있다.
...
아래는 보안 게이트가 스캔 결과를 받아서 최종 판단을 내리는 흐름이다.

```mermaid
flowchart TD
    A[보안 스캔 결과 수집] --> B{Critical 취약점<br/>있는가?}
    
    B -->|있음| C[빌드 즉시 실패]
    C --> D[Slack / 이메일 알림]
    D --> E[담당자 지정<br/>git blame 기반]
    
    B -->|없음| F{High 취약점<br/>있는가?}
    
    F -->|있음| G{도입 단계<br/>확인}
    G -->|초기: 모니터링| H[경고만 표시<br/>빌드 통과]
    G -->|안정화 후| I[빌드 실패]
    I --> D
    
    F -->|없음| J{Medium 취약점<br/>있는가?}
    
    J -->|있음| K[경고 표시<br/>빌드 통과<br/>백로그 등록]
    J -->|없음| L[보안 게이트 통과]
...
처음 도입할 때 Critical, High를 한꺼번에 차단하면 기존 코드의 이슈가 한꺼번에 터져서 개발이 멈춘다. 단계적으로 차단 범위를 넓혀야 한다.

### 보안 게이트 판단 기준 — 도입 단계별 변화

보안 게이트의 차단 기준은 고정이 아니라 도입 단계에 따라 점진적으로 강화한다. 아래는 각 단계에서 severity별로 어떻게 처리하는지 보여준다.

```mermaid
flowchart LR
    subgraph PHASE1["1단계: 모니터링<br/>1~2주"]
        A1[Critical → 경고]
        A2[High → 경고]
        A3[Medium → 무시]
    end
    
    subgraph PHASE2["2단계: 부분 차단<br/>2~4주"]
        B1[Critical → 차단]
        B2[High → 경고]
        B3[Medium → 무시]
    end
    
    subgraph PHASE3["3단계: 안정화<br/>1개월 이후"]
        C1[Critical → 차단]
        C2[High → 차단]
        C3[Medium → 경고]
    end
    
    PHASE1 -->|기존 이슈<br/>해결 후| PHASE2
    PHASE2 -->|false positive<br/>정리 후| PHASE3
```

단계를 넘길 때 중요한 건 "기존 이슈를 먼저 해결했는가"다. 이전 단계에서 발견된 취약점이 남아있는데 다음 단계로 넘기면 모든 PR이 실패해서 개발이 멈춘다. 기존 이슈를 예외 처리하거나 수정한 뒤에 단계를 올려야 한다.
...
```mermaid
flowchart TD
    A[취약점 발견] --> B{Severity 확인}
    
    B -->|Critical| C[CI 실패 + 즉시 알림]
    B -->|High| D[SLA: 1주일 이내]
    B -->|Medium| E[다음 스프린트 백로그]
    B -->|Low| F[백로그 기록]
    
    C --> G[담당자 지정<br/>git blame 확인]
    G --> H{실제 exploit<br/>가능한가?}
    
    H -->|가능| I[즉시 수정]
    H -->|불가능| J[사유 기록 후<br/>예외 처리<br/>만료일 설정]
    
    I --> K{수정 방법}
    K -->|라이브러리 취약점| L[버전 업그레이드]
    K -->|코드 취약점| M[코드 수정]
    K -->|즉시 수정 불가| N[WAF 룰 추가<br/>임시 차단]
    
    L --> O[스캔 재실행<br/>해결 확인]
    M --> O
    N --> P[수정 일정 수립<br/>임시 조치 해제 계획]
    
    J --> Q[분기별 재검토]
```

Critical은 발견 즉시 처리해야 하고, 나머지는 SLA에 따라 처리한다. 중요한 건 exploit 가능 여부를 반드시 확인하는 것이다. CVSS 점수가 9.8이어도 우리 환경에서 해당 코드 경로를 타지 않으면 실질적 위험도는 낮다.

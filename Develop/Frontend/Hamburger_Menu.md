...
tags: [frontend, javascript, performance, design-patterns]
updated: 2026-08-26
---
...
햄버거 메뉴는 결국 `isOpen`이라는 불리언 하나를 여러 UI 요소에 반영하는 문제다. 버튼의 `aria-expanded`, 메뉴 DOM의 클래스, body의 스크롤 잠금, 오버레이 표시 — 이 네 가지가 항상 같은 상태를 바라봐야 한다.

상태 하나를 네 군데에서 따로 건드리는 방식은 반드시 불일치가 생긴다. 열기/닫기 로직을 `openMenu()` / `closeMenu()` 함수 한 쌍으로 캡슐화하고, 상태 변경은 그 함수만 담당하게 만든다. 버튼 클릭이든 ESC 키든 오버레이 클릭이든 트리거가 달라도 항상 같은 함수를 호출한다.

React나 Vue 같은 프레임워크를 쓴다면 `isMenuOpen` 상태를 컴포넌트 최상위에 두고 props로 내려보내거나, 규모가 커지면 Context나 Pinia 같은 전역 상태로 올린다. 다만 햄버거 메뉴처럼 레이아웃 레벨의 상태는 페이지 라우팅마다 초기화되는 게 자연스럽다 — 전역으로 올렸다면 라우트 변경 시점에 명시적으로 닫아줘야 한다.
...
checkbox hack 방식은 JS 없이 동작하지만, `aria-expanded` 같은 접근성 속성을 동적으로 바꿀 수 없다. 스크린 리더 사용자는 메뉴 상태를 알 수 없다. 간단한 인터랙션 데모나 정적 사이트가 아니라면 쓰지 않는다.

...
위 코드에 실제로 있는 상태 클래스는 `PendingState`·`ApprovedState` 둘뿐이고, 인터페이스 메서드가 4개니 구현 지점은 2 × 4 = 8개다. 그런데 아래 전이 다이어그램은 상태 6개(Pending·Approved·Shipped·Delivered·Rejected·Cancelled)에 이벤트 5개(`deliver` 포함)를 그린다. **그림대로 다 만들면 클래스 6개에 메서드 30개**가 된다. if-else 를 없앤 대신 **전체 상태 기계를 한눈에 볼 수 있는 자리가 사라진다.** 문서가 전이 다이어그램을 별도 코드 블록으로 그려 둔 것이 그 증거다 — 그 그림이 코드 어디에도 없기 때문에 따로 그려야 했고, 코드가 바뀌어도 그림은 따라오지 않는다.

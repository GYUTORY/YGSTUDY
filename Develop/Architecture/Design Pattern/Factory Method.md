...
Factory Method 패턴은 객체 생성 방법을 추상화하는 디자인 패턴이다. "어떤 객체를 만들지"는 결정하되, "어떻게 만들지"는 서브클래스에 맡기는 방식이다.

### 패턴의 핵심 아이디어

"객체 생성을 캡슐화해서 클라이언트가 구체적인 클래스를 알 필요 없게 만든다"

자동차 공장에서 고객이 "빨간색 세단을 주세요"라고 요청하면 공장이 알아서 차를 만들어 주는 것과 같다. 고객은 그 차가 어떻게 만들어지는지 몰라도 되고, 원하는 결과물만 받으면 된다.
...
#### 4. 리스코프 치환 원칙 (LSP)
- 서브클래스가 부모 클래스를 대체할 수 있어야 함
- 모든 팩토리는 동일한 인터페이스 제공

## 패턴 구조 이해하기

Factory Method 패턴은 네 가지 구성 요소로 이루어진다. 각각 무슨 역할인지 보자.

### 구성 요소별 역할

#### 1. Product (제품) - "만들어질 것"
**역할**: 팩토리가 만들 객체들의 공통 규격을 정의

**실생활 비유**: 자동차의 기본 설계도
- 모든 자동차는 "시동을 걸고", "달리고", "멈춘다"는 기능을 공통으로 갖춰야 함
- 다만 구현 방식은 차종마다 다름
...
#### 2. ConcreteProduct (구체적 제품) - "실제 만들어지는 것"
**역할**: Product의 규격에 맞춰 실제로 만들어진 객체

**실생활 비유**: 실제 자동차들 (세단, SUV, 트럭 등)
- 특징은 저마다 다르지만 자동차의 기본 기능은 모두 수행
...
#### 3. Creator (생성자) - "만드는 방법을 정하는 것"
**역할**: 어떻게 만들지를 정하는 규칙을 담은 추상 클래스

**실생활 비유**: 자동차 제조 공정의 기본 틀
- "자동차를 만드는 과정"은 정의하되 구체적인 자동차 타입은 서브클래스가 결정
...
#### 4. ConcreteCreator (구체적 생성자) - "실제로 만드는 것"
**역할**: 어떤 구체 객체를 만들지 결정하고 실제로 생성
...
#### 1. 클래스 수 증가
- 각 제품마다 팩토리 클래스가 필요
- 작은 프로젝트에서는 과한 설계가 되기 쉬움

#### 2. 복잡성 증가
- 단순한 객체 생성에 붙는 과한 추상화
- 처음 보는 사람은 이해하기 어려움

#### 3. 성능 오버헤드
- 추상화 계층이 하나 늘어난 만큼의 성능 저하
- 대부분은 무시할 수준
...
3. 클라이언트가 구체 클래스를 몰라도 되나?
   - Yes → Factory Method 패턴 고려
   - No → 직접 생성 고려

4. 테스트에서 다른 구현체가 필요한가?
   - Yes → Factory Method 패턴 고려
   - No → 직접 생성 고려

두 개 이상 Yes 면 Factory Method 패턴을 써 볼 만하다.

## 단계별 구현 예제

### 1단계: 기본 Factory Method 패턴

가장 기본적인 Factory Method 패턴을 교통수단 예제로 구현해 본다. 단계별로 따라가 보자.
...
1. 클라이언트는 구체 클래스를 모름 - `Car`, `Motorcycle`, `Bicycle` 클래스를 직접 쓰지 않음
2. 객체 생성 책임은 팩토리에 있음 - 어떤 객체를 만들지 팩토리가 결정
3. 확장이 쉬움 - 교통수단 타입을 추가할 때 팩토리 하나만 새로 만들면 됨
4. 인터페이스가 일관됨 - 모든 교통수단이 같은 메서드를 갖춤
...
클래스를 인스턴스화하지 않고 정적 메서드로 객체를 만드는 방식이다.
...
React, Vue 같은 프레임워크에서 쓰는 컴포넌트 팩토리 패턴이다.
...
자주 만들고 버리는 객체의 메모리 할당·해제 비용을 줄이는 기법이다.
...
Factory Method는 "무엇을" 만들지 정하고 Builder는 "어떻게" 만들지 관리한다.
...
팩토리 자체를 싱글톤으로 만들어 전역에서 인스턴스 하나만 쓴다.
...
- **확장성**: 시스템을 넓히기 쉽고 유지보수 비용이 줄어듦
- **테스트 용이성**: Mock 객체로 단위 테스트가 간단해짐
- **의존성 관리**: 클라이언트와 구체 클래스 간의 느슨한 결합

### 주의사항
- **복잡성 증가**: 단순한 객체 생성에는 과도한 추상화
- **성능 오버헤드**: 추상화 계층이 늘어난 만큼의 성능 저하
- **학습 곡선**: 팀원들의 패턴 이해 필요

### 권장사항
1. **적절한 시점에 도입**: 객체 생성 로직이 복잡해지거나 확장이 예상될 때
2. **점진적 적용**: 기존 시스템에 조금씩 넣어 리스크를 줄임
3. **성능 모니터링**: 도입 후 성능 영향 지속적 모니터링
4. **팀 교육**: 팀이 패턴을 이해하도록 교육

Factory Method 패턴은 제대로 쓰면 코드 품질과 유지보수성을 크게 끌어올린다. 프로젝트 요구사항과 팀 상황을 보고 도입 여부를 정하면 된다.

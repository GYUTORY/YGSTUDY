...
옵저버 패턴은 객체지향 프로그래밍에서 가장 널리 쓰이는 디자인 패턴 중 하나다. 핵심은 한 객체의 상태가 변할 때 그 객체에 의존하는 다른 객체들에게 자동으로 알림이 간다는 것이다.

### 왜 옵저버 패턴이 필요한가?

뉴스 구독 서비스를 떠올리면 쉽다. 뉴스 에이전시에 새 기사가 뜨면 구독 중인 뉴스 채널과 신문사가 그 기사를 받아 보도한다. 이때 에이전시는 채널이나 신문사가 누구인지 정확히 몰라도 된다. "구독자들에게 알림을 보낸다"는 것만 알면 그만이다.

프로그래밍에서도 마찬가지다. 사용자가 로그인하면 여러 시스템이 한꺼번에 반응해야 한다.
- 보안 시스템이 로그인 시도를 기록
- 이메일 시스템이 로그인 알림 발송
- 대시보드가 사용자 정보 업데이트
- 분석 시스템이 사용자 활동 추적

옵저버 패턴을 쓰면 이 시스템들이 서로 강하게 얽히지 않고도 효율적으로 협력한다.

### 핵심 구성 요소

**Subject (주체/발행자)**
- 상태 변화의 관찰 대상이 되는 객체
- Observer들을 관리하고 상태가 바뀌면 알림을 보냄
...
3. **Concrete Subject (구체적 주체)**
   ...
   - 실제 상태를 직접 가지며 상태 변화 시 Observer들에게 알림

4. **Concrete Observer (구체적 관찰자)**
   - Observer 인터페이스의 실제 구현체
   - Subject가 보낸 알림을 실제로 처리하는 로직 구현
...
제일 이해하기 쉬운 예제인 뉴스 에이전시 시스템부터 만들어 보자. 새 뉴스가 나오면 구독 중인 뉴스 채널과 신문사가 그 뉴스를 받아 보도하는 구조다.
...
실제 웹 애플리케이션에서 제일 많이 나오는 사례다. 사용자가 로그인하면 여러 시스템이 동시에 반응해야 하는 상황을 코드로 옮겨 보자.
...
실시간 주식 가격을 지켜보다가 조건에 따라 알림을 보내는 시스템이다.
...
특정 조건을 만족할 때만 반응하는 옵저버도 만들 수 있다. 쓸데없는 처리와 알림이 줄어드니 성능에 도움이 된다.
...
중요도에 따라 옵저버의 실행 순서를 제어한다. 보안 관련 알림은 일반 알림보다 먼저 처리돼야 하는 식이다.
...
옵저버들이 줄줄이 이어져 실행되는 패턴이다. 한 옵저버의 결과가 다음 옵저버의 입력이 될 때 쓴다.
...
옵저버 패턴에서 제일 흔한 문제가 메모리 누수다. Observer를 제대로 떼어내지 않으면 Subject가 그 Observer를 계속 참조해서 가비지 컬렉션이 되지 않는다.
...
실제 애플리케이션에서는 Observer의 update 메서드가 비동기 작업을 하기도 한다. 그러면 에러 처리와 비동기 관리를 따로 챙겨야 한다.
...
Observer가 많아지면 알림 전송 성능이 떨어진다. 이걸 해결하는 기법 몇 가지를 보자.
...
Observer 시스템이 복잡해지면 디버깅이 어려워진다. 로깅과 모니터링 시스템을 미리 깔아두는 편이 낫다.
...
Observer 패턴과 Command 패턴을 같이 쓰면 시스템이 더 유연해진다. Command 객체를 Observer로 만들면 실행 취소(Undo) 기능도 붙일 수 있다.
...
옵저버 패턴은 현대 소프트웨어 개발에서 가장 널리 쓰이는 디자인 패턴 중 하나다. 느슨한 결합으로 시스템의 유연성과 확장성을 끌어올린다는 점이 이 패턴의 핵심 가치다.
...
옵저버 패턴을 제대로 이해하고 활용하면 더 유연하고 확장 가능한 소프트웨어를 만들 수 있다. JavaScript의 EventEmitter, React의 상태 관리를 비롯해 여러 프레임워크에서 이 패턴의 변형을 만나게 되니, 실무에서도 적극적으로 써먹어 볼 만하다.

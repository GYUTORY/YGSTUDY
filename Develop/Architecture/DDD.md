...
### "하나의 트랜잭션 = 하나의 Aggregate" 를 지키면 무엇을 내주는가

이 규칙 하나가 DDD 도입 비용의 대부분을 결정한다. 주문을 접수하면서 재고를 함께 차감하고 싶은데, Order 와 Product 가 다른 Aggregate 면 **같은 트랜잭션에서 못 한다.** 대신 이벤트를 발행하고 결과적 일관성에 맡긴다. 그 순간 아래가 전부 새로 생긴다.

- **중간 상태가 도메인에 생긴다.** "접수됨(재고 확인 중)" 같은 상태를 모델에 추가해야 하고, 화면도 그 상태를 보여줘야 한다. 사용자에게 "주문은 됐는데 될지 안 될지 모릅니다"를 설명할 수 있어야 한다.
- **보상 처리를 짜야 한다.** 재고 차감이 실패하면 주문을 되돌려야 하고, 그 되돌리기도 실패할 수 있다. (아래 "결과적 일관성 실패 시 보상 처리" 절이 그 이야기다.)
- **이벤트 전달을 보장해야 한다.** 커밋은 됐는데 이벤트가 안 나가면 그대로 불일치다. Outbox 같은 장치가 따라온다.
- **테스트가 통합 테스트로 밀린다.** 한 트랜잭션 안에서 검증하던 것이 여러 프로세스에 걸친 흐름이 된다.

**이 값을 치를 만한지는 도메인이 정한다.** 아래 셋 중 하나라도 "아니오"면 규칙을 어기고 한 트랜잭션에 묶는 편이 낫다.

| 질문 | "예" 여야 결과적 일관성을 쓸 수 있다 |
|---|---|
| 잠깐 어긋나 있어도 되는가 | 재고가 몇 초 부정확해도 되는 규모인가 |
| 사용자에게 중간 상태를 설명할 수 있는가 | "처리 중" 화면을 만들 자리가 있는가 |
| 되돌리기가 실제로 가능한가 | 이미 보낸 알림·차감한 포인트를 되돌릴 수 있는가 |
| 두 Aggregate 가 정말 따로 변하는가 | 항상 같이 바뀐다면 애초에 경계가 잘못 그어진 것이다 |

마지막 줄이 중요하다. **경계를 정하는 근거는 "함께 바뀌는가"** 다. 개념이 다르다고 나눴는데 항상 같은 트랜잭션에서 함께 바뀐다면, 그건 하나의 Aggregate 로 두는 게 맞고 크기가 커지는 것을 받아들여야 한다. 뒤의 "큰 Aggregate로 인한 락 경합" 은 반대 방향의 실패이고, 두 실패 사이에서 균형을 잡는 일이 Aggregate 설계의 전부다.
...
### 리치 모델을 만들면 조회가 불편해진다

쓰기 쪽 이야기는 위 표대로다. 문제는 읽기 쪽이다.

주문 목록 화면이 필요한 것은 주문번호·상태·금액·날짜 네 개인데, Repository 가 Aggregate 단위로만 로드하면 **주문 20건을 위해 Aggregate 20개를 통째로 조립한다.** 항목·주소·결제 정보까지 딸려 오고, 지연 로딩을 걸면 이번엔 N+1 이 된다. 캡슐화를 지키느라 `getStatus()` 같은 게터도 최소한만 열어 뒀다면 DTO 로 옮기는 코드까지 늘어난다.

그래서 DDD 를 실제로 쓰는 프로젝트는 거의 예외 없이 **조회 경로를 따로 판다.** 목록·검색·리포트는 Aggregate 를 거치지 않고 필요한 컬럼만 읽는 전용 쿼리로 처리하고, 도메인 모델은 상태를 바꾸는 경로에만 쓴다. CQRS 라고 부르는 것의 가장 실용적인 부분이 이 분리다.

| | 도메인 모델을 거친다 | 조회 전용 쿼리 |
|---|---|---|
| 쓰는 곳 | 주문 접수·취소·변경 | 목록, 검색, 대시보드, 엑셀 내보내기 |
| 이유 | 불변식을 지켜야 한다 | 불변식과 무관하고 모양이 화면마다 다르다 |
| 대가 | 로딩이 무겁다 | 모델을 우회하므로 규칙이 새어 나갈 수 있다 |

**이 분리를 처음부터 인정하지 않으면**, 목록이 느리다는 이유로 Aggregate 안에 조회용 메서드가 하나씩 늘고 결국 리치 모델이 다시 데이터 덩어리로 돌아간다.

### 캡슐화는 주석이 아니라 구조로 막는다

위 Aggregate 예시에 이런 주석이 있다.
...
`getItems()` 가 내부 리스트를 그대로 돌려주면 저 코드는 **문법적으로 완전히 정상이고 컴파일도 통과한다.** 주석은 리뷰에서만 작동하고 리뷰는 매번 통과하지 않는다. 읽기 전용 뷰를 돌려주면 같은 코드가 런타임 예외로 바뀐다.
...
JPA 를 쓰면 더 조심해야 한다. 영속 컬렉션을 그대로 노출하면 외부에서 넣은 항목이 그대로 저장되고, `recalculateTotal()` 은 불리지 않아 **합계와 항목이 어긋난 채로 커밋된다.** 예외도 안 난다.

### 정책 상수를 엔티티에 박지 않는다

`cancel()` 안의 `Money.of(1_000_000)` 도 다시 볼 만하다. "100만원 이상은 관리자 승인" 은 도메인 규칙이라기보다 **자주 바뀌는 정책**이다. 여기에 박아 두면 금액을 바꿀 때마다 도메인 코드를 고쳐 배포해야 하고, 과거 주문을 재계산할 때 "그때의 기준"을 알 수 없다.

이런 조건은 밖에서 주입하거나 아래 Specification 패턴으로 분리한다. 판별 기준은 하나다 — **비즈니스가 "이 숫자를 바꿔 달라"고 요청할 수 있는 값인가.** 그렇다면 그것은 규칙이 아니라 설정이다.
...
이 폴링 루프는 서버가 한 대일 때만 성립한다. 두 대 이상이면 **둘 다 같은 미발행 행을 읽어 각자 발행한다.** `findByPublishedFalse()` 에 잠금이 없고 개수 제한도 없다. 잠금을 걸되 이미 잠긴 행은 건너뛰도록 `FOR UPDATE SKIP LOCKED`(JPA 라면 비관적 잠금 + skip-locked 힌트)를 쓰고, 한 번에 가져올 개수도 제한한다. 밀린 이벤트가 수만 건일 때 전부 메모리로 올리면 그 자체가 장애다.

`publish` 성공 후 `markAsPublished` 저장 전에 죽으면 다음 주기에 같은 이벤트가 다시 나간다. 순서를 뒤집으면 이번엔 유실이다. **Outbox 가 보장하는 것은 정확히 1회가 아니라 최소 1회**이고, 그래서 구독자 쪽 중복 처리가 항상 같이 필요하다. 위 `OutboxEvent` 에는 구독자가 중복을 판별할 고유 이벤트 ID 가 payload 에 들어가지 않는다 — 발행 시 `id` 를 메시지 헤더에 함께 실어야 구독자가 "이미 처리한 것"을 알 수 있다.
...
### 1단계가 실패하면 나머지는 형식만 남는다

이 순서에서 유일하게 코드가 아닌 것이 1단계이고, 실패도 대개 거기서 난다. 유비쿼터스 언어는 **도메인 전문가와 같은 말을 쓰기로 합의하는 활동**이지 용어집 문서를 만드는 작업이 아니다. 전문가와 정기적으로 대화할 통로가 없으면 개발자끼리 만든 단어 목록이 되고, 그 위에 올린 Bounded Context 는 패키지 나누기, Aggregate 는 엔티티 묶기로 축소된다. 문법은 DDD 인데 모델이 여전히 테이블 구조를 따라간다.

그래서 앞의 "DDD가 필요한 시점" 표에는 조건이 하나 더 붙는다.

| 확인할 것 | 없으면 |
|---|---|
| 물어볼 도메인 전문가가 있는가 | 용어를 개발자가 지어내게 된다 |
| 요구사항이 계속 바뀌는가 | 한 번 만들고 끝나는 것은 CRUD 로 충분하다 |
| 모델을 고칠 시간이 있는가 | 도메인 모델은 한 번에 안 나온다. 리팩토링 예산이 필요하다 |

세 번째가 가장 자주 빠진다. **DDD 의 산출물은 완성된 모델이 아니라 모델을 계속 고쳐 나가는 습관**이다. 첫 설계를 정답으로 두고 이후 변경을 다 예외 처리로 흡수하면, 리치 모델이 조건문 덩어리가 되면서 원래의 서비스 레이어보다 읽기 어려워진다.

용어 하나를 정할 여지가 없는 프로젝트라면 전술 패턴만 골라 쓰는 것도 방법이다. Value Object 와 리치 엔티티는 그 자체로 값을 하고, 유비쿼터스 언어나 Bounded Context 없이도 도입할 수 있다. **전부 하거나 안 하거나의 문제가 아니다.**

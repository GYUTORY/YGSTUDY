...
tags: [backend, monitoring, devops]
updated: 2026-03-30
---
...
#### 위 시나리오의 실패율 계산이 맞지 않는다

같은 순서(성공 4번 뒤 실패 6번)로 누적 실패율을 계산하면 이렇게 된다.
...
요청 7: 실패 → 42.9%      (문서는 30%)
요청 8: 실패 → 50.0%      (문서는 40%)
요청 9: 실패 → 55.6%  ← 여기서 이미 50% 초과
요청 10: 실패 → 60.0%
```

문서는 실패 건수를 10 으로 나눠 놓았다. 아직 오지도 않은 요청까지 분모에 넣은 셈이다. 임계값 50% 라면 **요청 9에서 열린다.**

계산을 짚는 이유는 산수 때문이 아니다. **분모가 무엇인지가 서킷 브레이커 설정의 핵심**이기 때문이다. 최근 N개 요청인지 최근 N초 안의 요청인지에 따라 같은 임계값이 전혀 다르게 동작한다. 트래픽이 적은 API 에서 시간 창을 쓰면 요청 두 건 중 한 건만 실패해도 100% 가 되어 회로가 열리고, 그래서 최소 호출 수(`volumeThreshold`) 가 함께 필요하다.

위 설정의 주석도 다시 봐야 한다. `rollingCountTimeout: 10000` 에 "슬라이딩 윈도우 크기(최근 N개 요청)"이라는 설명이 붙어 있는데, 같은 블록의 `resetTimeout: 10000` 에는 "(ms)" 가 붙어 있다. 이름에 `Timeout` 이 들어간 값을 요청 개수로 읽는 것은 앞뒤가 안 맞고, 요청 10,000건을 창으로 잡는 설정도 현실적이지 않다. **옵션의 실제 단위는 쓰는 라이브러리 문서에서 확인하고 넘어가야 한다** — 여기를 잘못 읽으면 회로가 영영 안 열리거나 너무 자주 열린다.
...
이 문서는 처음부터 끝까지 `POST /process` 결제 호출을 예로 쓴다. **재시도가 가장 위험한 종류의 요청이다.**

타임아웃이 났다고 요청이 서버에 안 닿은 것이 아니다. 결제사가 요청을 받아 승인까지 마쳤는데 응답만 늦었을 수도 있다. 클라이언트는 그 둘을 구분할 방법이 없다. 그대로 세 번 재시도하면 **승인이 네 건 날 수 있다.**

| HTTP 응답 | 재시도해도 되나 |
|---|---|
| 연결 실패, DNS 오류 | 된다. 요청이 나가지 않았다 |
| 5xx | 조건부. 서버가 처리 전에 실패했다는 보장이 없다 |
| 타임아웃 | **가장 위험하다.** 성공했는지 알 수 없다 |
| 4xx | 하면 안 된다. 다시 보내도 결과가 같다 |

재시도를 붙이려면 **연산이 여러 번 실행돼도 결과가 같아야 한다.** 조회는 그냥 되고, 결제·주문 생성처럼 상태를 바꾸는 호출은 중복 방지 키(idempotency key)를 요청에 실어 보내 서버가 같은 키의 두 번째 요청을 첫 번째 결과로 되돌려주게 해야 한다.

```typescript
const key = `pay:${request.orderId}`;   // 재시도 사이에 값이 바뀌면 안 된다
this.httpService.post(url, request, { headers: { 'Idempotency-Key': key } })
  .pipe(retry({ count: 3, ... }));
```

키를 재시도 루프 **안에서** 만들면 매번 달라져 아무 소용이 없다. 그리고 이건 서버(결제사)가 지원해야 성립한다 — 지원하지 않으면 재시도 자체를 하지 않는 것이 맞다. **"재시도는 공짜"라는 전제가 이 패턴에서 가장 자주 깨지는 지점**이고, 깨졌을 때 나오는 것이 중복 청구다.

같은 이유로 위 `retry` 는 실패 종류를 가리지 않는다는 점도 봐야 한다. 400 이나 401 을 세 번 다시 보내면 결과는 같고 지연만 늘어난다. `delay` 콜백에서 에러를 보고 재시도할 것만 골라야 한다.
...
#### 두 개를 겹칠 때 순서가 결과를 바꾼다

위 코드는 브레이커가 재시도를 감싼다(`new CircuitBreaker(this.callWithRetry)`). 반대로 감쌀 수도 있고, 둘은 장애 상황에서 다르게 움직인다.

| | 브레이커가 바깥 (위 코드) | 재시도가 바깥 |
|---|---|---|
| 브레이커가 세는 것 | 재시도까지 다 실패한 건 | 개별 호출 |
| 임계값 도달 속도 | 느리다. 실제 호출 4번에 실패 1건으로 계산 | 빠르다 |
| OPEN 이후 | 재시도 없이 즉시 fallback | **열린 회로에 재시도를 반복**한다 |
| 장애 시 최악 지연 | 백오프 합계만큼 (1+2+4초) | 즉시 |

**위 구조의 대가는 지연이다.** 회로가 열리기 전까지 모든 요청이 백오프를 다 소진하고 나서야 실패한다. 상위 호출자에게 타임아웃이 걸려 있다면 그 전에 잘려 나가고, 그러면 브레이커는 실패조차 기록하지 못한 채 계속 닫혀 있다. 브레이커를 넣었는데 안 열린다면 이 조합을 먼저 의심한다.

재시도가 바깥이면 반대 문제가 생긴다. 회로가 열린 뒤에도 세 번 재시도하는데, 열린 회로는 즉시 실패하므로 그냥 세 번 헛돈다. 재시도 조건에서 브레이커의 `EOPENBREAKER` 계열 에러를 제외해야 한다.

**어느 쪽이든 총 지연의 상한을 계산해 두는 것이 먼저다.** 타임아웃 × (재시도 횟수 + 1) + 백오프 합계가 상위 호출자의 인내심보다 작아야 한다. 이 계산을 안 하면 장애 시 스레드·커넥션이 대기에 묶여, 회복 장치를 넣었는데 오히려 더 크게 무너진다.
...
**Fallback 이 성공처럼 보이면 안 된다.** 앞의 서킷 브레이커 예시가 그렇다.
...
결제는 시도조차 되지 않았는데 `PENDING` 이 나간다. 호출한 쪽은 "처리 중"으로 읽고 주문을 진행시키고, 사용자는 결제 완료 화면을 본다. **결제 없이 주문이 성립하는 경로**가 만들어진다. 게다가 `PENDING` 을 나중에 정산할 배치가 없다면 그 주문은 영원히 미결로 남는다.

프로필 조회의 `Guest` 기본값과 결제의 `PENDING` 은 겉보기에 같은 "기본값 반환"이지만 성질이 다르다.

| | 기본값을 줘도 되는 것 | 주면 안 되는 것 |
|---|---|---|
| 예 | 추천 목록, 프로필 사진, 배너 | 결제, 재고 차감, 잔액 |
| 판단 기준 | 없어도 화면이 성립한다 | 이후 로직이 이 결과를 근거로 삼는다 |
| 실패 시 | 조용히 넘어가도 된다 | 호출자가 실패를 알아야 한다 |

**Fallback 을 쓸 수 없는 자리도 있다는 것을 인정하는 편이 낫다.** 결제가 안 되면 결제가 안 된 것이고, 그때 할 수 있는 최선은 빠르게 실패하고 사용자에게 다시 시도하게 하는 것이다. 두 번째 예시처럼 `FAILED` 를 돌려주거나, 아예 예외를 그대로 올려 상위에서 처리하게 한다.

Fallback 이 실행됐다는 사실 자체를 지표로 남기는 것도 필요하다. 위 코드는 `console.error` 한 줄이 전부다. Fallback 이 조용히 잘 도는 동안 원래 기능은 죽어 있는 것이고, 알림이 없으면 **장애가 아니라 정상으로 보인다.** 회복 패턴을 넣을 때 가장 흔한 사고가 이것이다.

---
title: Backend · API 전체 보기
tags: []
hide:
  - toc
---

<!-- AUTO-SECTION-INDEX: tools/section_index.py 가 빌드마다 다시 만든다. 직접 고치지 말 것. -->

# Backend · API 전체 보기

문서 13개.

## 개요

- [API 남용 방지 실무 패턴](API_Abuse_Prevention.md)
- [API 멱등성 (Idempotency)](API_Idempotency.md)
- [API 버저닝 실무](API_Versioning.md)
- [API 설계 패턴](API_Design_Patterns.md)
- [API 인증/인가 실무 패턴](API_Security_Patterns.md)
- [API 캐싱 (HTTP Caching)](API_Caching.md)
- [API 페이지네이션 패턴](API_Pagination_Patterns.md)
- [HTTP 에러 응답 설계 (RFC 7807 Problem Details)](HTTP_Error_Response_Design.md)
- [HTTP 조건부 요청 심화](HTTP_Conditional_Requests.md)
- [Polling과 Webhook](Polling_and_Webhook.md)
- [REST 리소스 모델링 심화](REST_Resource_Modeling.md)
- [서비스 간 API 인증 — mTLS](m_TLS_Service_Auth.md)
- [정합성 검증 시점](Consistency_Validation_Timing.md)


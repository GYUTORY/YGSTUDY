...
```mermaid
graph LR
    UPLOAD[대용량 파일<br/>Storage 업로드] --> SPLIT[분할 함수<br/>파일을 1000줄 단위로<br/>청크 분리]
    SPLIT -->|청크 1| PS[Pub/Sub 토픽]
    SPLIT -->|청크 2| PS
    SPLIT -->|청크 N| PS
    PS --> W1[처리 함수<br/>인스턴스 1]
    PS --> W2[처리 함수<br/>인스턴스 2]
    PS --> W3[처리 함수<br/>인스턴스 N]
    W1 --> RESULT[처리 결과<br/>Storage / DB]
    W2 --> RESULT
    W3 --> RESULT
```

하나의 함수가 전체 파일을 처리하는 대신, 분할 함수가 청크로 나누고 Pub/Sub을 통해 여러 인스턴스가 병렬로 처리한다. 각 청크가 타임아웃 안에 끝나도록 크기를 조절해야 한다.
...
```mermaid
graph LR
    subgraph "방식 1: VPC Access 커넥터"
        CF1[Cloud Functions] -->|트래픽| VC[VPC 커넥터<br/>e2-micro x 2~3대]
        VC -->|VPC 내부 라우팅| VPC1[VPC 네트워크]
        VPC1 --> SQL1[Cloud SQL]
        VPC1 --> MEM1[Memorystore]
    end

    subgraph "방식 2: Direct VPC Egress (2세대)"
        CF2[Cloud Functions] -->|서브넷 직접 연결| VPC2[VPC 네트워크]
        VPC2 --> SQL2[Cloud SQL]
        VPC2 --> MEM2[Memorystore]
    end

    style VC fill:#f59e0b,stroke:#d97706,color:#000
```

커넥터 방식은 중간에 e2-micro VM이 끼어 있어서 추가 비용과 레이턴시가 발생한다. Direct VPC Egress는 이 중간 단계가 없다.
...
```mermaid
graph LR
    CF[Cloud Functions] -->|Unix 소켓<br/>/cloudsql/PROJECT:REGION:INSTANCE| PROXY[Cloud SQL Auth Proxy<br/>내장]
    PROXY -->|인증된 연결| CSQL[Cloud SQL 인스턴스]

    CF2[Cloud Functions<br/>+ VPC 커넥터] -->|Private IP<br/>TCP 연결| VPC[VPC 네트워크]
    VPC -->|10.x.x.x:5432| CSQL2[Cloud SQL 인스턴스<br/>Private IP 활성화]

    style PROXY fill:#34d399,stroke:#059669,color:#000
```

Unix 소켓 방식은 VPC 커넥터 없이 동작하고 설정이 간단하다. Private IP 방식은 VPC 커넥터가 필요하지만 네트워크 레이턴시가 더 낮다. 대부분의 경우 Unix 소켓으로 충분하다.
...
```mermaid
graph TB
    CF[Cloud Functions 실행] --> LOG[Cloud Logging<br/>print/logging 자동 수집]
    CF --> MON[Cloud Monitoring<br/>실행 횟수, 시간, 메모리]
    CF --> ERR[Error Reporting<br/>예외 자동 수집, 그룹핑]

    LOG -->|severity>=ERROR 필터| ALERT1[알림 정책]
    MON -->|에러율, 타임아웃 임계값| ALERT2[알림 정책]
    ERR -->|새 에러 그룹 감지| ALERT3[알림 정책]

    ALERT1 --> NOTIFY[알림 채널<br/>이메일, Slack, PagerDuty]
    ALERT2 --> NOTIFY
    ALERT3 --> NOTIFY
```

Cloud Functions는 별도 설정 없이 세 가지 모니터링 서비스가 자동 연동된다. 로그는 Cloud Logging, 실행 지표는 Cloud Monitoring, 예외는 Error Reporting에 각각 수집된다.
...
```mermaid
graph LR
    REQ[요청 도착] --> INVOKE[호출 횟수 과금<br/>월 200만 회 무료<br/>이후 100만 회당 $0.40]
    REQ --> COMPUTE[컴퓨팅 과금<br/>GB-초 + GHz-초<br/>실행 시간 x 할당 리소스]
    REQ --> NET[네트워크 과금<br/>아웃바운드 트래픽<br/>월 5GB 무료]

    MIN[최소 인스턴스<br/>유휴 비용] -.->|인스턴스 떠있는 동안<br/>계속 과금| COMPUTE
    VPC_C[VPC 커넥터<br/>e2-micro 상시 가동] -.->|함수 미사용 시에도<br/>VM 비용 발생| EXTRA[추가 비용]

    style INVOKE fill:#60a5fa,stroke:#3b82f6,color:#000
    style COMPUTE fill:#f472b6,stroke:#ec4899,color:#000
    style NET fill:#a78bfa,stroke:#8b5cf6,color:#000
    style EXTRA fill:#f59e0b,stroke:#d97706,color:#000
```
...
```mermaid
graph LR
    GIT[Git Push<br/>main 브랜치] --> TRIGGER[Cloud Build<br/>트리거 발동]
    TRIGGER --> TEST[테스트 실행<br/>pytest]
    TEST -->|성공| DEPLOY[gcloud functions deploy]
    TEST -->|실패| FAIL[빌드 실패<br/>배포 중단]
    DEPLOY --> REV[새 리비전 생성]
    REV --> CANARY{카나리 배포?}
    CANARY -->|아니오| FULL[트래픽 100%<br/>새 리비전]
    CANARY -->|네| SPLIT[트래픽 분할<br/>10% 신규 / 90% 기존]
    SPLIT -->|문제 없음| FULL
    SPLIT -->|문제 발생| ROLLBACK[이전 리비전으로<br/>트래픽 전환]
```
...
```mermaid
graph TD
    DEPLOY[함수 배포] --> S{배포 성공?}
    S -->|실패| D1[빌드 에러<br/>requirements.txt 누락<br/>런타임 버전 오류]
    S -->|성공| RUN{실행 시 에러?}
    RUN -->|ModuleNotFoundError| D2[의존성 누락<br/>로컬에만 설치된 패키지]
    RUN -->|Permission Denied| D3[서비스 계정 권한 부족<br/>기본 SA에 역할 미부여]
    RUN -->|Timeout| D4[타임아웃 기본값 60초<br/>처리 시간 초과]
    RUN -->|트리거 미동작| D5[리전 불일치<br/>Storage 버킷과 함수 리전 다름]
    RUN -->|정상| OK[정상 동작]
```

배포 자체는 성공하는데 실행 시 에러가 나는 경우가 더 까다롭다. 로그를 확인하면 원인을 빨리 찾을 수 있다.

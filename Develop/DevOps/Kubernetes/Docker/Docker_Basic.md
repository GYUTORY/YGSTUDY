...
=== "VM 구조"
    ...
    block-beta
        ...
        style vm1 fill:#e8d5f5,stroke:#7c3aed
        style vm2 fill:#e8d5f5,stroke:#7c3aed
        style Hypervisor fill:#fde68a,stroke:#d97706
        style HostOS1 fill:#bfdbfe,stroke:#2563eb
        style HW1 fill:#d1d5db,stroke:#6b7280
    ```
...
=== "컨테이너 구조"
    ...
    block-beta
        columns 3
        block:c1["Container 1"]:1
            A3["App A"]
        end
        block:c2["Container 2"]:1
            A4["App B"]
        end
        block:c3["Container 3"]:1
            A5["App C"]
        end
        ...
        style HostOS2 fill:#bfdbfe,stroke:#2563eb
        style HW2 fill:#d1d5db,stroke:#6b7280
    ```

VM은 각 VM마다 Guest OS를 포함하고 Hypervisor 위에서 돌아간다. 컨테이너는 Guest OS 없이 Docker Engine이 호스트 커널을 공유해서 프로세스를 격리한다. 이 구조 차이가 부팅 속도, 메모리 사용량, 이미지 크기의 차이를 만든다.
...
flowchart LR
    ...
    style Daemon fill:#d1fae5,stroke:#059669
    style Registry fill:#fde68a,stroke:#d97706
```

Docker Client가 CLI 명령어를 받으면 REST API로 Docker Daemon에 전달한다. Daemon이 실제로 이미지를 빌드하고 컨테이너를 실행한다. 이미지를 주고받을 때는 Registry와 통신한다.

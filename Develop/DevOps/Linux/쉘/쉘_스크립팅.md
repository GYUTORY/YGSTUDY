...
```bash
# [ ]에서는 반드시 인용 필요
if [ -z $var ]; then     # var에 공백이 있으면 인자가 쪼개져 에러
                         # (빈 값일 땐 조용히 통과해서 오히려 더 위험하다)
if [ -z "$var" ]; then   # 정상
...
## 이 문서의 스크립트를 그대로 돌리기 전에

아래는 위 예제들을 macOS(bash 3.2.57, arm64)와 GNU bash에서 직접 실행해 확인한 것이다.

### `set -e`는 생각보다 좁게 작동한다

`-e`를 "명령이 실패하면 즉시 종료"로 외우면 틀린 곳이 나온다. `&&`나 `||` 리스트 안에서 실패한 명령은 **마지막 `&&`/`||` 뒤에 오는 명령이 아닌 이상 `-e`를 발동시키지 않는다.**

```bash
[ -z "${env}" ] || [ -z "${tag}" ] && usage
```

env와 tag가 둘 다 채워져 있으면 두 `[` 모두 1을 반환하고 `usage`는 실행되지 않는다. 이 줄 전체의 종료 코드는 1이지만 스크립트는 멈추지 않고 다음 줄로 넘어간다. `cleanup()` 안의 `[ -n "${TMPDIR}" ] && rm -rf "${TMPDIR}"`도 같다 — TMPDIR이 비어 있어도 트랩이 중간에 죽지 않는다. 의도한 동작이지만, `-e`를 믿고 검사를 이 형태로 쓰면 "실패하면 알아서 멈추겠지"가 성립하지 않는다.

**단, 그 줄이 스크립트의 마지막 명령이면 종료 코드 1이 그대로 스크립트의 종료 코드가 된다.** cron이나 CI가 이걸 실패로 읽는다.

### `local`/`readonly`는 `$(...)`의 실패를 삼킨다

```bash
local v="$(false)"      # 실패를 삼키고 스크립트가 계속 진행된다
local v; v="$(false)"   # 선언과 대입을 나누면 -e가 걸려 여기서 멈춘다
```

`local`과 `readonly`는 그 자체가 명령이라, 종료 코드가 명령 치환이 아니라 `local`의 것이 된다. 이 문서의 `readonly SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"`도 마찬가지다 — `cd`가 실패하면 SCRIPT_DIR이 **빈 문자열인 채로** 스크립트가 계속 돈다(실측: 종료 코드 0). 이후 `"${SCRIPT_DIR}/config"` 같은 경로는 `/config`가 된다. ShellCheck가 SC2155로 잡아주는 항목이다.

### `$((10#num))`은 동작하지 않는다

앞의 "산술 확장에서 앞에 0이 붙은 숫자" 예제의 `echo $((10#num))`은 실행하면 이렇게 죽는다.

```
bash: 10#num: value too great for base (error token is "10#num")
```

진법 접두사 뒤는 리터럴로 읽히므로 변수를 `$`로 펼쳐야 한다.
...
덧붙여 `$((num))`처럼 산술 확장이 실패했을 때의 동작은 **셸마다 다르다.** `sh`(POSIX 모드)와 `zsh` 는 그 자리에서 종료하지만(종료 코드 1), **bash 는 에러만 찍고 다음 줄을 계속 실행한다**(종료 코드 0). `set -e` 를 켜도 마찬가지다.

```
$ bash s.sh   # 에러 출력 후 다음 줄 실행,  exit=0
$ sh   s.sh   # 즉시 종료,                  exit=1
$ zsh  s.sh   # 즉시 종료,                  exit=1
```

이 문서의 스크립트는 전부 `#!/bin/bash` 라 **에러를 찍고 그냥 지나간다.** 잘못된 값이 조용히 흘러가는 쪽이므로 오히려 더 위험하다 — 입력값 검증을 따로 넣어야 한다.

### macOS의 `/bin/bash`는 3.2다 — `declare -A`가 안 된다

Apple이 배포하는 `/bin/bash`는 라이선스 때문에 bash 3.2에 묶여 있다. 이 문서의 배포 스크립트와 헬스체크 스크립트는 `#!/bin/bash` + `declare -A` 조합인데, macOS에서 그대로 실행하면 첫 줄에서 죽는다.

```
t5.sh: line 3: declare: -A: invalid option
declare: usage: declare [-afFirtx] [-p] [name[=value] ...]
exit=2
```

`set -euo pipefail`이 있든 없든 결과는 같다. Homebrew bash를 깔아도 `/bin/bash`는 그대로이므로, 연관 배열을 쓸 거면 shebang을 `#!/usr/bin/env bash`로 바꾸고 PATH에 bash 5가 앞서 오는지 확인해야 한다. 개발자는 맥, 배포 대상은 리눅스인 환경에서 "리눅스에서는 되는데 로컬에서만 안 되는" 스크립트가 여기서 나온다.

### 임시 디렉토리 변수 이름으로 `TMPDIR`을 쓰지 않는다

`TMPDIR`은 이미 환경에 export되어 있는 표준 변수다. macOS에서는 로그인 셸마다 `/var/folders/.../T/`가 잡혀 있다. 배포 스크립트처럼 `TMPDIR=""` 후 `TMPDIR=$(mktemp -d)`로 재사용하면 **export 속성이 남아 있어 자식 프로세스가 전부 그 값을 본다.**

```
child sees TMPDIR = []                                        # TMPDIR="" 직후
after mktemp -d: /var/folders/.../T/tmp.cHb1xZyyTN
child sees TMPDIR = /var/folders/.../T/tmp.cHb1xZyyTN         # 재할당 이후
```

이 상태에서 스크립트가 부르는 `curl`, `ssh`, `tar`, 그리고 또 다른 `mktemp`가 전부 그 디렉토리 안에 임시 파일을 만든다. 그러다 `trap cleanup EXIT`이 `rm -rf`로 통째로 지운다. 자식 프로세스가 아직 살아 있으면 파일이 사라진 채로 계속 도는 셈이다. 변수 이름을 `TMP_WORK_DIR` 같은 것으로 바꾸면 끝나는 문제다.

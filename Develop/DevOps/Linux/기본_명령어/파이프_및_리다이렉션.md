...
# `> /dev/null > file.log` 처럼 앞에 리다이렉션을 하나 더 붙여도 비워지긴 한다.
# 앞의 것은 아무 일도 안 하는 잔재라서 흉내낼 형태가 아니다.
...
# 되돌릴 수 없다. sdX 를 실제 장치명으로 바꿔 쓰기 전에 lsblk 로 두 번 확인할 것 —
# 장치명 한 글자를 잘못 적으면 쓰던 디스크가 날아간다.

...
JWT를 처음 보면 "세 번째 조각이 페이로드를 해시한 값인가?" 하고 오해하기 쉽다. 아니다. 세 번째 조각은 서명이고, 해싱은 그 서명을 만드는 과정 안에서 일어난다. 이 둘을 구분하지 못하면 검증이 왜 동작하는지, 왜 비밀키가 필요한지가 안 잡힌다.

### 서명이 만들어지는 단계

서명의 입력은 헤더도 페이로드도 아니고, 둘을 이어 붙인 문자열이다.

```
signing_input = base64UrlEncode(header) + "." + base64UrlEncode(payload)
```

이 `signing_input` 하나를 알고리즘에 따라 다르게 처리한다.

HS256(HMAC) 계열은 비밀키와 `signing_input`을 함께 SHA-256에 넣어 키 있는 해시(MAC)를 만든다. 키가 해시 계산에 섞여 들어가는 구조라, 비밀키를 모르면 같은 값을 못 만든다.

RS256/ES256 계열은 두 단계로 나뉜다. 먼저 `signing_input`을 SHA-256으로 해시해 32바이트 다이제스트를 만들고, 그 다이제스트에 개인키 서명을 적용한다. 해싱과 서명이 분리되어 있다.

```
HS256:  MAC = HMAC_SHA256(secret, signing_input)
RS256:  digest = SHA256(signing_input)
        signature = RSA_sign(private_key, digest)
```

검증자는 같은 `signing_input`을 다시 만들어, HS256이면 같은 키로 MAC을 다시 계산해 비교하고, RS256이면 공개키로 서명을 풀어 다이제스트와 맞춰본다.

### 해싱·인코딩·암호화·서명은 다 다르다

네 가지가 섞여서 오해가 생긴다. 구분하면 이렇다.

| 연산 | 가역성 | 키 | 목적 | JWT에서 |
|------|--------|-----|------|---------|
| 인코딩 (Base64URL) | 가역 | 없음 | URL에 안전하게 싣기 | 헤더·페이로드·서명 모두 |
| 해싱 (SHA-256) | 단방향 | 없음 | 무결성 지문 | 서명 만들 때 내부에서 |
| 암호화 | 가역 | 있음 | 기밀성 | JWS엔 없음 (JWE만) |
| 서명 (HMAC/RSA) | - | 있음 | 무결성 + 진위 | 세 번째 조각 |

흔한 오해가 "JWT가 페이로드를 해싱하거나 암호화한다"는 것이다. 둘 다 아니다. 페이로드는 그냥 Base64URL로 인코딩만 한다. 누구나 디코딩해서 읽을 수 있다. 해싱은 서명을 만들 때 내부에서만 쓰이고, 페이로드 자체를 해시값으로 바꾸지 않는다. 암호화(기밀성)는 JWS에서는 아예 일어나지 않는다. 그래서 민감 정보를 페이로드에 넣으면 그대로 노출된다.

### 단순 해시로는 위변조를 못 막는다

여기서 키가 왜 필요한지가 갈린다. 만약 서명을 키 없는 SHA-256으로만 만든다고 하자.

```
signature = SHA256(signing_input)   // 키 없음
```

SHA-256은 공개 알고리즘이고 키가 없으니 누구나 계산할 수 있다. 공격자가 페이로드를 `{"role":"admin"}`으로 바꾸고, 바뀐 `signing_input`으로 SHA-256을 다시 돌려 새 서명을 붙이면 검증이 그대로 통과한다. 무결성 지문 역할은 하지만 위변조 방지는 안 된다.

막으려면 공격자가 모르는 무언가가 서명 계산에 섞여야 한다. 그게 HMAC의 비밀키이고, RSA/ECDSA의 개인키다. 공격자는 페이로드는 바꿀 수 있어도 올바른 서명은 못 만든다. 비밀키 없이 HMAC을 재현할 수 없고, 개인키 없이 유효한 RSA 서명을 못 만들기 때문이다. JWT 서명이 "키 있는 해시"나 "개인키 서명"이어야 하는 이유가 이거다.

### HS256 / HS384 / HS512

HMAC 계열의 숫자는 내부에서 쓰는 SHA 변형을 가리킨다.

| 알고리즘 | 해시 함수 | 출력 길이 |
|----------|-----------|-----------|
| HS256 | HMAC-SHA256 | 256비트 |
| HS384 | HMAC-SHA384 | 384비트 |
| HS512 | HMAC-SHA512 | 512비트 |

차이는 해시 출력 길이뿐이다. 길수록 무조건 안전하다고 생각해 HS512를 기본으로 까는 경우가 있는데 대부분 과하다. SHA-256만으로 현실적인 공격 여유가 충분하다. 길이를 늘리면 서명이 길어져 토큰 크기가 커지고, 키도 그만큼 길게 줘야 의미가 있다. HS512를 쓰면서 키는 16바이트짜리를 쓰면 길이만 늘었지 강도는 그대로다. 특별한 규제 요구가 없으면 HS256으로 시작하면 된다.

### RSA는 다이제스트에만 적용된다

RS256을 처음 보면 "RSA로 토큰 전체를 서명하나?" 싶지만 아니다. RSA 연산은 `signing_input` 전체가 아니라 그걸 SHA-256으로 줄인 32바이트 다이제스트에만 적용된다.

이유는 두 가지다. RSA는 모듈러스 크기(2048비트면 256바이트)보다 큰 데이터를 한 번에 못 다룬다. 페이로드가 그보다 크면 직접 서명이 안 된다. 그리고 RSA 연산 자체가 느려서 큰 데이터를 통째로 넣으면 비현실적으로 느려진다. 그래서 메시지를 고정 길이 다이제스트로 줄인 뒤 그 작은 값에만 서명한다. 모든 공개키 서명에서 쓰는 hash-then-sign 방식이다. ES256도 똑같이 SHA-256 다이제스트에 ECDSA 서명을 한다.

### 비밀번호 해싱과는 목적이 다르다

같은 "해싱"이라도 비밀번호 저장과 토큰 서명은 정반대 요구사항을 갖는다. 비밀번호는 bcrypt나 argon2 같은 의도적으로 느린 해시로 저장한다. 공격자가 유출된 해시로 brute-force를 돌릴 때 한 번 계산에 시간이 오래 걸리게 만들어 대입을 어렵게 하는 게 목적이다. salt를 섞어 같은 비밀번호도 다른 해시가 나오게 한다. 반대로 토큰 서명에 쓰는 SHA-256은 빠른 게 미덕이다. 매 요청마다 검증해야 하니까 느리면 곤란하다. 목적도 다르다. 비밀번호 해싱은 원본을 숨기는 단방향 저장이고, 토큰 서명은 위변조를 잡는 무결성·진위 확인이다. SHA-256으로 비밀번호를 저장하면 너무 빨라서 brute-force에 취약하고, bcrypt로 토큰을 서명하려 들면 매 요청 검증이 느려서 못 쓴다.

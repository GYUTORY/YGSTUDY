...
sequenceDiagram
    ...
    participant Client as 클라이언트 (서버)
    participant AuthServer as 인증 서버
    participant Resource as 리소스 서버

    User->>Client: 로그인 버튼 클릭
    Client->>User: 인증 서버 로그인 페이지로 리다이렉트
    Note right of Client: response_type=code<br/>client_id, redirect_uri,<br/>scope, state

    User->>AuthServer: 로그인 + 권한 동의
    AuthServer->>User: redirect_uri로 리다이렉트
    Note left of AuthServer: ?code=XXXXX&state=YYYYY

    User->>Client: 콜백 URL 호출 (code 전달)
    Client->>Client: state 검증 (CSRF 방어)

    Client->>AuthServer: 토큰 교환 요청 (서버-서버 통신)
    Note right of Client: grant_type=authorization_code<br/>code, client_id,<br/>client_secret, redirect_uri

    AuthServer->>Client: access_token, refresh_token 발급
    Client->>Resource: API 요청 (Bearer access_token)
...
sequenceDiagram
    ...
    Client->>Client: code_verifier 생성 (랜덤 문자열)
    Client->>Client: code_challenge = SHA256(code_verifier)

    User->>Client: 로그인 버튼 클릭
    Client->>User: 인증 서버로 리다이렉트
    Note right of Client: response_type=code<br/>code_challenge,<br/>code_challenge_method=S256

    User->>AuthServer: 로그인 + 권한 동의
    AuthServer->>AuthServer: code_challenge 저장
    AuthServer->>User: redirect_uri로 리다이렉트 (?code=XXXXX)

    User->>Client: 콜백 URL 호출 (code 전달)

    Client->>AuthServer: 토큰 교환 요청
    Note right of Client: grant_type=authorization_code<br/>code, code_verifier<br/>(client_secret 없음)

    AuthServer->>AuthServer: SHA256(code_verifier) ==<br/>저장된 code_challenge 비교
    AuthServer->>Client: access_token 발급
```
...
```mermaid
sequenceDiagram
    participant ServiceA as 서비스 A (Client)
    participant AuthServer as 인증 서버
    participant ServiceB as 서비스 B (리소스 서버)

    ServiceA->>AuthServer: 토큰 요청
    Note right of ServiceA: grant_type=client_credentials<br/>client_id, client_secret<br/>scope=read:orders

    AuthServer->>AuthServer: client_id/secret 검증
    AuthServer->>ServiceA: access_token 발급

    ServiceA->>ServiceB: API 요청 (Bearer access_token)
    ServiceB->>ServiceA: 응답
```
...
```mermaid
sequenceDiagram
    participant Device as 기기 (TV/CLI)
    participant AuthServer as 인증 서버
    participant User as 사용자 (브라우저)

    Device->>AuthServer: device code 요청
    Note right of Device: client_id, scope

    AuthServer->>Device: device_code, user_code,<br/>verification_uri 반환

    Device->>Device: 화면에 표시<br/>"https://example.com/device 접속 후<br/>코드 ABCD-1234 입력"

    User->>AuthServer: verification_uri 접속
    User->>AuthServer: user_code 입력 + 로그인

    loop 인증 완료될 때까지 (interval 간격)
        Device->>AuthServer: 폴링 (device_code로 토큰 요청)
        AuthServer->>Device: authorization_pending
    end

    User->>AuthServer: 권한 동의
    Device->>AuthServer: 폴링 (device_code로 토큰 요청)
    AuthServer->>Device: access_token, refresh_token 발급
```

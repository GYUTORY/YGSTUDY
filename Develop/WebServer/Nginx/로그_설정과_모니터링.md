...
성능 문제가 발생했을 때 로그를 보는 순서:

1. **범위 파악**: 특정 URI만 느린지, 전체가 느린지 — access_log의 `request_time` 분포로 확인
2. **병목 위치**: `request_time`과 `upstream_response_time` 비교 — Nginx 앞단인지, 뒷단인지 구분
3. **커넥션 상태**: stub_status의 Active/Writing/Waiting 비율 — 커넥션 한도나 upstream 병목 확인
4. **에러 로그 대조**: error_log에서 같은 시간대의 upstream 에러, timeout 메시지 확인
5. **시스템 리소스**: `iowait`, 메모리, CPU 사용률과 로그 타임스탬프를 대조

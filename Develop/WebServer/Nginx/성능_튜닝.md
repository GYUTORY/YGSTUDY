...
이 문서의 HTTP/2 절은 `listen 443 ssl http2;`와 `http2_push`를 쓰고, HTTP/3 절은 "1.25.0부터 기본 포함"이라고 적혀 있다. 두 설정을 같은 서버에 합치면 버전에 따라 지시어가 인식되지 않을 수 있다. 최근 nginx는 `listen ... http2`를 `http2 on;`으로 대체했고 서버 푸시는 제거됐다.

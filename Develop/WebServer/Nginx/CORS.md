...
### 웹 보안의 핵심: Same-Origin Policy

웹 브라우저는 기본적으로 **Same-Origin Policy(동일 출처 정책)**라는 강력한 보안 메커니즘을 가지고 있습니다. 이 정책은 웹의 보안을 지키는 가장 기본적인 방어선이라고 할 수 있습니다.

동일 출처 정책이란, 웹 페이지가 오직 **같은 출처(Origin)**에서만 리소스를 요청할 수 있도록 제한하는 정책입니다. 여기서 출처는 다음 세 가지 요소의 조합으로 정의됩니다:

- **프로토콜** (http, https, ftp 등)
- **호스트** (도메인명 또는 IP 주소)
- **포트 번호** (명시적으로 지정된 경우)

이 세 요소가 모두 일치해야만 "동일 출처"로 인정됩니다. 하나라도 다르면 "다른 출처(Cross-Origin)"로 간주되어 브라우저가 요청을 차단합니다.

### CORS의 등장 이유

그런데 현대 웹 개발에서는 이런 제약이 오히려 문제가 되었습니다. 

**개발 환경**에서는 프론트엔드와 백엔드가 서로 다른 포트에서 실행되는 경우가 많습니다. 예를 들어 React 개발 서버는 3000번 포트에서, Express API 서버는 8080번 포트에서 실행됩니다. 이는 브라우저 입장에서는 완전히 다른 출처로 인식됩니다.

**프로덕션 환경**에서는 마이크로서비스 아키텍처로 인해 여러 서비스가 서로 다른 도메인에서 운영됩니다. 프론트엔드는 `https://example.com`에서, API 서버는 `https://api.example.com`에서 실행되는 식입니다.

이런 상황에서도 안전하게 리소스를 공유할 수 있도록 등장한 것이 바로 **CORS(Cross-Origin Resource Sharing)**입니다.

### CORS의 핵심 철학

CORS는 "모든 요청을 허용하자"가 아니라 "서버가 명시적으로 허용한 요청만 통과시키자"는 철학을 가지고 있습니다. 이는 보안을 유지하면서도 필요한 경우에만 크로스 오리진 요청을 허용하는 균형잡힌 접근법입니다.

## CORS의 작동 원리와 메커니즘

### 브라우저의 CORS 검증 과정

CORS는 브라우저와 서버 간의 협력으로 이루어지는 보안 메커니즘입니다. 브라우저는 크로스 오리진 요청을 보낼 때마다 다음과 같은 검증 과정을 거칩니다:
...
만약 이 과정에서 문제가 발견되면, 브라우저는 요청을 차단하고 개발자 도구에 다음과 같은 오류를 표시합니다:
...
CORS는 HTTP 헤더를 통해 구현됩니다. 주요 헤더들은 다음과 같습니다:
...
**상황**: React 앱(`http://localhost:3000`)에서 Express API 서버(`http://localhost:8080`)로 데이터를 요청하려고 합니다. 브라우저 개발자 도구를 열어보니 다음과 같은 오류가 나타납니다:
...
이 오류의 의미를 정확히 이해하는 것이 중요합니다. 브라우저는 "서버가 이 요청을 허용한다고 명시적으로 말하지 않았으니 차단하겠다"는 것입니다.
...
많은 개발자들이 놓치는 부분이 바로 Preflight 요청입니다. 
...
이런 요청들은 브라우저가 실제 요청 전에 OPTIONS 메서드로 "이런 요청을 보내도 되나요?"라고 먼저 물어봅니다. 서버가 이 OPTIONS 요청에 적절히 응답해야만 실제 요청이 진행됩니다.
...
CORS 문제 해결의 핵심은 **서버에서 적절한 헤더를 설정하는 것**입니다. 클라이언트(브라우저) 측에서는 CORS를 우회할 방법이 없습니다. 이는 보안상 의도된 설계입니다.
...
Preflight Request는 브라우저의 **방어적 프로그래밍** 철학을 보여주는 대표적인 사례입니다. 브라우저는 "이 요청이 서버에 부담을 줄 수 있으니 미리 확인해보자"는 생각으로 작동합니다.
...
**1단계: 브라우저의 판단**
브라우저가 요청을 분석하여 Preflight가 필요한지 결정합니다. 위의 조건 중 하나라도 해당되면 Preflight를 시작합니다.

**2단계: OPTIONS 요청 전송**
실제 요청 대신 OPTIONS 메서드로 사전 요청을 보냅니다. 이 요청에는 다음 정보가 포함됩니다:
- `Access-Control-Request-Method`: 실제 요청에서 사용할 메서드
- `Access-Control-Request-Headers`: 실제 요청에서 사용할 헤더들
- `Origin`: 요청을 보내는 출처

**3단계: 서버의 응답**
서버는 OPTIONS 요청에 대해 허용 여부를 응답합니다:
- `Access-Control-Allow-Methods`: 허용할 메서드들
- `Access-Control-Allow-Headers`: 허용할 헤더들
- `Access-Control-Max-Age`: 이 정보를 캐시할 시간

**4단계: 실제 요청 진행**
서버가 허용한다고 응답하면, 브라우저는 실제 요청을 보냅니다. 거부한다면 요청을 차단합니다.
...
#### 1단계: 오류 메시지 분석
브라우저 콘솔의 CORS 오류 메시지는 매우 구체적입니다. 메시지를 정확히 읽으면 문제의 원인을 파악할 수 있습니다.
...
#### 2단계: 네트워크 탭 분석
개발자 도구의 네트워크 탭에서 실제 HTTP 요청과 응답을 확인합니다.
...
#### 3단계: 서버 로그 확인
서버 측에서 CORS 관련 요청과 응답을 로깅하여 문제를 추적합니다.
...
CORS를 완전히 이해하고 마스터하려면 다음 사항들을 숙지해야 합니다:

...
```text
> /review src/payment 디렉터리 변경분
→ 변경된 파일만 추려서 분석
→ Critical / High / Medium / Low / Info 5단계로 분류
→ 각 항목마다 "이 코드를 이렇게 바꾸면 된다" 제안 + Quick Fix 적용 버튼
```

실제로 잡히는 것과 안 잡히는 것을 구분해 두면 기대치 설정이 쉽다.

- **잘 잡는 것**: 하드코딩된 시크릿(AWS Access Key, JWT, OAuth 토큰), SQL 인젝션 가능성이 있는 문자열 결합, 민감 정보 로깅(`log.info("password=" + password)`), AWS SDK 사용 시 `s3:*` 같은 와일드카드 IAM 정책, try/finally 누락으로 인한 리소스 누수.
- **놓치는 것**: 비즈니스 로직 버그(권한 체크 누락, 인가 우회), race condition, 프로젝트 전체에 걸친 의도된 패턴 위반(예: "이 모듈은 직접 DB 접근 금지"), 외부 라이브러리의 알려진 CVE는 탐지하지만 transitive dependency 깊은 곳까지는 약하다.
- **로컬 변경분만 본다**: 커밋 안 된 워킹 디렉터리 기준이다. 원격 main과 비교하는 건 아니다. PR 단위 리뷰를 원하면 별도로 git diff 결과를 채팅에 붙여 일반 채팅 모드로 돌리는 편이 낫다.

PR 게이트로 박을 수 있냐는 질문이 자주 나오는데, 2026년 5월 기준 `/review`는 IDE 안에서만 돌고 CLI/CI 훅은 없다. CI에서 강제하려면 보안 스캔(`q chat /security-scan` 또는 별도 CodeGuru Security)을 쓰는 게 정공법이다.

## 코드 생성에서 자주 부딪히는 한계

Q Developer를 6개월 이상 써보면 반복적으로 같은 패턴에서 깨진다. 도입 전에 알아두면 사용자 신고를 미리 차단할 수 있는 항목들을 모았다.

### 라이브러리 버전 환각

가장 잦은 신고다. Q가 제안하는 `pom.xml`, `package.json`, `build.gradle` 의존성 버전이 실제로 존재하지 않거나, 존재하지만 호환성이 깨진 조합인 경우가 흔하다.
...
<dependency>
    ...
    <version>3.4.7</version>   <!-- 실제 존재하지 않음 -->
</dependency>
```

학습 컷오프 이후 새로 나온 버전을 짐작으로 채우는 경우와, 컷오프 이전이지만 메이저 버전을 잘못 매핑한 경우가 섞여 있다. 빌드 한 번 돌려보면 바로 잡히지만, "추천된 버전이 실제로 존재하는지 확인 후 사용한다"는 규칙을 사내에 공지해 두는 게 안전하다.

### 사내 라이브러리 추론 한계

커스터마이제이션을 안 켠 상태에서 사내 공통 라이브러리를 쓰는 코드를 요청하면, Q는 표준 SDK 흉내를 낸다. 예를 들어 사내에 `MyCorpHttpClient`가 있어도 `OkHttpClient`나 `RestTemplate`을 제안한다. 사용자에게 이름을 알려줘도 시그니처는 추측한다. 결국 컴파일 에러가 나거나, 더 나쁘게는 컴파일은 되지만 동작 의미가 다른 코드가 된다.

해결은 두 갈래다. 한 번만 쓸 코드면 채팅에서 사내 라이브러리의 인터페이스 정의 파일을 직접 첨부한다. 반복적으로 등장하면 커스터마이제이션을 켜는 게 답이다.

### 비동기 코드 패턴 약점

Java의 `CompletableFuture`, Kotlin의 `coroutine`, Node.js의 `Promise.all` 같은 비동기 조합 코드에서 미묘하게 틀린 패턴을 잡는다.

```java
// 사용자가 "여러 외부 API를 병렬 호출하고 결과 합치기" 요청 시 자주 나오는 잘못된 패턴
List<CompletableFuture<Result>> futures = ids.stream()
    .map(id -> CompletableFuture.supplyAsync(() -> callApi(id)))
    .collect(Collectors.toList());

// 여기서 Q가 .join()을 stream 안에서 호출하는 패턴을 자주 만든다
List<Result> results = futures.stream()
    .map(CompletableFuture::join)   // 순차 블로킹이 됨, 의도와 다름
    .collect(Collectors.toList());
```

`CompletableFuture.allOf()`로 모은 후 join 해야 진짜 병렬인데, 위 형태가 일반적인 학습 데이터에 많아서 그대로 베끼는 경우가 잦다. 동시성·병렬성 코드는 사람이 한 번 더 본다는 원칙을 지킨다.

### 테스트 데이터 날조

`/test`로 테스트를 만들 때, 입력 데이터를 그럴듯하게 지어낸다. 도메인 모델에 명시되지 않은 필드값을 추측하거나, 실제 운영에서 나올 수 없는 조합을 만들기도 한다.

```java
// User 엔티티에 age 필드만 있는데 Q가 만든 테스트
User user = new User();
user.setAge(150);     // 도메인 검증 규칙으로는 0~120만 가능, 통과되지만 비현실적
user.setName(null);   // @NotNull인데 무시
```

도메인 검증 어노테이션을 안 보고 단위 테스트만 컴파일이 되도록 만든다. 통합 테스트로 가면 깨진다. 실제 fixture 데이터가 있으면 그걸 가리키는 편이 낫다.

### 한국어 변수명·주석 처리

사내 코딩 규약이 영문 변수명·영문 주석이라도, 사용자가 한국어로 질문하면 Q가 한국어 식별자를 슬쩍 끼워넣는 경우가 있다.

```typescript
// 사용자 질문: "주문 합계 계산하는 함수 만들어줘"
function 주문합계계산(주문목록: 주문[]): number {  // 사내 규약 위반
    return 주문목록.reduce((acc, 주문) => acc + 주문.가격, 0);
}
```

특히 인라인 자동완성에서 잦다. 채팅창에선 "영문 식별자 유지"를 명시하면 잘 따르지만, 인라인 제안엔 그 컨텍스트가 안 전달된다. `.amazonq/rules/` 디렉터리에 사내 코딩 규약을 적어두면 인라인에도 일정 부분 반영된다.

### 컨텍스트 망각

같은 채팅 세션에서 메시지가 20~30개를 넘어가면, 초반 지시("이 프로젝트는 Java 17 + Spring Boot 3 기반")를 잊고 Java 8 시절 문법을 제안하기 시작한다. Claude Code 같은 도구에 비해 컨텍스트 유지가 짧은 편이다.

긴 작업을 시작할 때는 새 세션을 열고 핵심 제약(언어 버전, 사용 금지 라이브러리, 명명 규약)을 시스템 프롬프트처럼 첫 메시지에 넣는다. `.amazonq/rules/project.md`에 적어두면 매 세션 자동으로 주입된다.

### 모노레포 의존성 추적 실패

`/dev`로 한 서비스에 기능을 추가할 때, 같은 모노레포의 다른 서비스가 그 인터페이스를 import하는 상황을 못 본다. 변경된 함수 시그니처 때문에 다른 서비스 빌드가 깨지는 사고가 흔하다.

`grep`이나 `ripgrep`으로 영향 범위를 먼저 사람이 확인한 뒤 `/dev`에 "다음 파일도 같이 수정해야 한다"고 알려주는 식으로 우회한다.

### 인프라 코드의 미묘한 오류

CDK·Terraform·CloudFormation 코드를 만들 때 큰 틀은 맞지만 세부 옵션을 자주 틀린다. 예를 들어 RDS 파라미터 그룹의 family 이름(`postgres15` vs `postgres-15`), Lambda의 `architecture`와 layer 호환성, IAM 정책의 condition key 표기(`aws:SourceVpc` vs `aws:SourceVpce`) 등은 한 글자 차이로 배포가 깨진다.

생성된 코드를 그대로 `cdk deploy`로 던지지 말고, `cdk diff`나 `terraform plan`을 먼저 돌려 변경 항목을 사람이 훑는다. AWS 특화 작업에서도 Q가 100% 정확한 건 아니다.
...
- **데이터가 수십만 페이지 이상이고 검색 정확도가 핵심**: Kendra retriever. 키워드 매칭 튜닝이 가능해 사내 약어·코드 검색에 강하다.
...
Pro 플랜의 핵심 기능인데 가격 표에서만 한 줄 언급되는 경우가 많다. 실제로 도입할 때 절차와 한계를 정리해 둔다.

### 무엇을 학습시키나

사내 GitHub/GitLab/Bitbucket 저장소의 코드를 읽고, Q 모델이 그 스타일로 코드를 제안하게 만든다. 학습 자체는 모델 파라미터를 바꾸는 게 아니라 RAG 형태로 동작한다고 보면 된다. 즉 매 요청마다 사내 코드를 검색해 컨텍스트로 주입하는 방식이다.

```text
1. Q Developer 콘솔 → Customizations → "Create customization"
2. 데이터 소스 선택 (S3 / GitHub / GitLab / Bitbucket)
3. 코드 저장소 인덱싱 (수십 GB까지 가능, 시간은 수 시간 단위)
4. 평가 단계: Q가 자동으로 샘플 시나리오를 만들어 기본 모델 vs 커스터마이제이션 비교
5. 평가 결과 OK → 그룹/사용자에 활성화
```

### 잘 먹히는 경우

- **사내 공통 라이브러리 사용을 학습시킬 때**: 사내에 `MyCorpHttpClient`, `MyCorpLogger` 같은 래퍼가 있고 표준 SDK 대신 이걸 써야 하는 환경이라면 커스터마이제이션이 거의 필수다. 안 쓰면 Q가 매번 표준 `OkHttpClient`를 제안한다.
- **사내 도메인 모델·DTO 명명 규약**: "Order는 항상 OrderAggregate로, Repository는 패키지 `repo.order`에" 같은 사내 컨벤션이 있을 때 자동으로 따른다.
- **반복되는 보일러플레이트**: 컨트롤러 - 서비스 - 리포지토리 골격이 일정하면 패턴을 그대로 재현한다.

### 잘 안 먹히는 경우

- **저장소가 너무 잡다하다**: PoC 코드, 사용 안 하는 옛 모듈, 외주 코드가 같은 저장소에 섞여 있으면 잘못된 패턴까지 학습한다. 평가 점수가 기본 모델보다 떨어지는 경우가 실제로 나온다. "이 저장소 폴더만 학습하라"는 화이트리스트를 만들고 시작해야 한다.
- **언어가 너무 다양하다**: 한 인스턴스에 Java, Python, Go, TypeScript를 같이 학습시키면 도메인 적중률이 떨어진다. 언어별로 커스터마이제이션을 따로 만드는 게 낫다(인스턴스당 추가 비용은 없다).
- **저장소가 빠르게 변한다**: 인덱스는 자동으로 갱신되지 않는다. 수동 또는 스케줄로 재인덱싱을 걸어야 한다. 큰 리팩토링 후에는 한 번 재학습이 필요하다.

### 비용·운영상의 주의

- 커스터마이제이션 자체는 추가 요금이 없다(Pro 구독에 포함). 단, S3에 저장되는 인덱스 데이터의 스토리지 비용은 발생한다.
- **데이터는 사내 AWS 계정에 머문다**: 커스터마이제이션 결과 인덱스는 고객 계정의 S3에 저장되고, 모델 학습에 안 쓰인다. 법무 팀이 가장 자주 묻는 부분이다.
- **시크릿 필터링은 사용자가 책임**: Q가 사내 저장소를 학습하면서 코드에 박힌 자격 증명까지 통째로 인덱싱하는 사고가 가능하다. 학습 전에 git-secrets, gitleaks 같은 도구로 저장소를 한 번 청소하고 시작해야 한다.
- 활성화는 그룹 단위가 가능하다. 보안 등급이 다른 사내 코드를 별도 커스터마이제이션으로 분리하고, 접근 권한 있는 그룹에만 노출하는 식으로 다룬다.
...
- **모델 선택 폭이 좁다**: 2026년 들어 Q 채팅에서 Claude Sonnet 계열 일부와 AWS 자체 모델을 고를 수 있게 됐지만, Cursor처럼 Claude·GPT·Gemini를 자유롭게 오가는 수준은 아니다. 특정 작업(예: 긴 컨텍스트 분석, 초저지연 자동완성)에 가장 잘 맞는 외부 최신 모델을 그때그때 끌어다 쓸 수는 없다.

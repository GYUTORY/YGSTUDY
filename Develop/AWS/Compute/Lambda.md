...
전통적인 웹 애플리케이션 개발에서는 개발자가 서버를 직접 관리해야 했습니다.
...
**전통적 방식의 문제점:**
- 개발자가 비즈니스 로직에 집중하는 것을 방해
- 높은 운영 비용 발생
- 복잡한 인프라 관리로 인한 부담

**Lambda의 해결책:**
AWS Lambda는 이러한 문제를 해결하기 위해 등장했습니다. 개발자는 코드 작성에만 집중하고, AWS가 인프라 관리의 모든 측면을 담당합니다. 이를 통해 개발 생산성 향상과 운영 비용 절감을 동시에 달성할 수 있습니다.
...
AWS Lambda는 마이크로 가상화 기술을 기반으로 작동합니다.

**기본 동작 원리:**
- 각 함수 실행은 격리된 실행 환경에서 이루어짐
- AWS가 자동으로 인프라를 관리
- 개발자가 함수를 업로드하면, AWS가 패키징하고 실행 환경을 준비

**함수 실행 과정:**

1. **함수 호출 시:**
   - AWS가 새로운 실행 컨테이너를 생성하거나 기존의 워밍된 컨테이너를 재사용
   - 함수 코드가 로드되고 초기화
   - 이벤트 데이터가 전달됨

2. **함수 실행 완료 후:**
   - 결과를 반환
   - 일정 시간 후 컨테이너는 종료되거나 다른 요청을 위해 대기 상태로 전환
...
동시성 관리는 Lambda의 핵심 기능 중 하나입니다.

**동시성 제한 레벨:**

AWS는 두 가지 레벨에서 동시성 제한을 제공합니다:

1. **계정 레벨 동시성:**
   - 전체 계정에서 동시에 실행될 수 있는 Lambda 함수의 총 개수를 제한
   - 기본값은 리전별로 다르며, 필요시 증가 요청 가능

2. **함수 레벨 동시성:**
   - 특정 함수의 동시 실행 수를 제한
   - 각 함수별로 독립적으로 설정 가능
   - 함수별로 다른 동시성 제한 설정 가능

...
```mermaid
flowchart LR
    A["CPU 사용률 측정"] --> B{"baseline 이하?"}
    B -- "Yes" --> C["남는 CPU 시간 → 크레딧 적립"]
    C --> D["크레딧 잔액 증가\n(최대치까지)"]
    B -- "No" --> E["초과분만큼 크레딧 소비"]
    E --> F{"크레딧 잔액 > 0?"}
    F -- "Yes" --> G["크레딧 차감\nCPU 100%까지 사용 가능"]
    F -- "No" --> H{"모드 확인"}
    H -- "Standard" --> I["CPU를 baseline으로\n강제 제한"]
    H -- "Unlimited" --> J["초과 크레딧 발생\n→ 추가 과금"]
```

baseline 이하로 쓰면 크레딧이 쌓이고, 넘기면 소비된다. 크레딧이 바닥났을 때 동작이 Standard와 Unlimited에서 갈린다. 이 흐름을 머릿속에 넣어두면 나머지 내용이 다 연결된다.
...
위 세 가지 예시를 하루 단위로 비교하면 이렇다:

```mermaid
gantt
    title 크레딧 흐름 비교 (하루 기준)
    dateFormat HH:mm
    axisFormat %H시

    section t3.micro (70%)
    크레딧 순감 60/h   :crit, 00:00, 24:00

    section t3.small (40%)
    적립 = 소비 (균형)   :active, 00:00, 24:00

    section t3.medium (10%→80%)
    적립 구간 (+24/h)    :done, 00:00, 22:00
    소비 구간 (-48/h)    :crit, 22:00, 24:00
```

t3.micro(70% 사용)는 하루 종일 크레딧이 줄어든다. 이런 워크로드에 T 시리즈를 쓰면 안 된다. t3.small(40% 사용)은 적립과 소비가 맞아서 크레딧이 안정적이다. t3.medium(대부분 10%, 피크 2시간 80%)은 적립 시간이 길어서 크레딧이 남는다. 이게 T 시리즈에 맞는 워크로드다.

### 24시간 크레딧 잔액 변화 (시나리오별)

세 가지 예시를 크레딧 잔액 관점에서 보면 어떤 워크로드가 T 시리즈에 맞는지 한눈에 보인다.
...
300 |──────────────────────────·─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ (10%→80%)     ╲ 480
    |╲                                                                              │
200 |  ╲            t3.small ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 288 유지
    |    ╲           (40% 사용 = baseline 2배, 적립=소비)
100 |      ╲
    |        ╲  t3.micro (70% 사용, 시간당 -60)
  0 |──────────━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  ← 4.8h에 바닥
    └──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬──┬→ 시간
...
t3.micro는 5시간도 안 돼서 크레딧이 바닥난다. t3.small은 적립과 소비가 같아서 잔액이 변하지 않는다. t3.medium은 22시간 동안 크레딧이 꾸준히 쌓이다가 피크 2시간 동안 소비해도 480 크레딧이 남는다. T 시리즈를 쓸지 말지는 이 그래프 모양이 결정한다.
...
### Standard vs Unlimited: 크레딧 소진 후 동작 비교

크레딧이 바닥났을 때 두 모드의 차이를 나란히 놓고 보면 이렇다.

```
     Standard 모드 (T2 기본)                    Unlimited 모드 (T3 기본)
     ─────────────────────                    ──────────────────────
...
100  │    ┏━━━━┛    ┗━━━━┓              100  │    ┏━━━━┛
 ...
 10  │                   ┗━━━━━━━━━━     10  │
     └────┬─────┬────┬────┬────→ 시간        └────┬─────┬────┬────┬────→ 시간
          ↑     ↑    ↑    ↑                       ↑     ↑    ↑
        부하   크레딧 CPU    계속                  부하   크레딧 CPU
        증가   바닥  baseline 제한               증가   바닥  제한 없음
                     고정(10%)                              (초과 과금 시작)
...
Standard는 성능으로 대가를 치르고, Unlimited는 돈으로 대가를 치른다. 운영 서비스는 Unlimited가 안전하고, 개발/테스트 환경은 Standard가 비용 사고를 막아준다.
...
이 예시에서 `8.2 st`는 CPU 시간의 8.2%를 steal당하고 있다는 뜻이다. `us`(user)나 `sy`(system)보다 `st`가 먼저 눈에 들어와야 한다.
...
크레딧이 소진되면 CloudWatch에서 다음 메트릭들이 움직인다:
...
### CloudWatch 그래프 패턴 예시

실제 CloudWatch에서 크레딧 소진 과정이 어떤 모양으로 나타나는지 정리했다. t3.micro(baseline 10%) Standard 모드 기준이다.

```
[CPUCreditBalance]                    [CPUUtilization]
크레딧                                 CPU %
288 |━━━━━━━━━╲                       100|
    |          ╲                          |         ┏━━┓
200 |           ╲                      70 |    ┏━━━━┛  ┗━━━┓
...
100 |             ╲                    30 |━━━━┛            ┃
  ...
  0 |───────────────━━━━━━━━          10 |─────────────────━━━━━━━━━━
    └──────────────────────→ 시간         └──────────────────────────→ 시간
         크레딧 감소 → 바닥                  부하 증가 → 강제 제한(10%)
```

왼쪽 그래프에서 `CPUCreditBalance`가 0에 도달하는 시점과, 오른쪽 그래프에서 `CPUUtilization`이 baseline(10%)에 딱 붙는 시점이 일치한다. 이 두 그래프를 같이 놓고 보면 throttling이 발생했는지 바로 알 수 있다.

Unlimited 모드에서는 패턴이 다르다:
...
100 |        ╱                            |    ┃
 ...
 50 |    ╱                                |
  ...
  0 |━━━━━━━━━━━                        0 |
    └──────────────────────→ 시간         └──────────────────────────→ 시간
         크레딧 바닥 후 초과분 누적           CPU 제한 없이 계속 사용
```

Unlimited 모드에서는 CPU가 제한되지 않는 대신 `CPUSurplusCreditBalance`가 계속 올라간다. 이 그래프가 우상향하고 있으면 초과 과금이 쌓이고 있다는 뜻이다. 이 메트릭에 알람을 걸어두면 예상 못한 과금을 잡을 수 있다.
...
여기가 T2와 T3의 중요한 차이다. 아래 다이어그램으로 먼저 보자.

```mermaid
flowchart TD
    subgraph T2["T2 인스턴스"]
        A2["Running\n크레딧 288"] -- "Stop" --> B2["Stopped\n크레딧 → 0"]
        B2 -- "Start" --> C2["Running\n크레딧 0부터 시작"]
    end
    subgraph T3["T3 / T3a / T4g 인스턴스"]
        A3["Running\n크레딧 288"] -- "Stop" --> B3["Stopped\n크레딧 288 유지\n(7일까지)"]
        B3 -- "7일 이내 Start" --> C3["Running\n크레딧 288 유지"]
        B3 -- "7일 초과" --> D3["크레딧 소멸"]
    end
```

### T2 vs T3: 일주일 운영 시 크레딧 변화

주말에 인스턴스를 끄는 운영을 할 때 T2와 T3의 차이가 극명하게 드러난다.

```
[T2 인스턴스 - Stop하면 크레딧 초기화]
...
144 |     ╲╱       ╲╱       ╲╱       ╲╱       ╲
  ...
  0 |──────────────────────────────────────────━━━━━━━━━━━━━━━━
    └──┬──────┬──────┬──────┬──────┬──────┬──────────────┬──→
      월(Start) 화     수     목     금   금저녁(Stop)  월(Start)
                  평일: 크레딧 변동                      ↑
                                                    크레딧 0에서 재시작
                                                    월요일 아침 느림

[T3 인스턴스 - Stop해도 크레딧 보존 (7일 이내)]
...
T2는 주말 Stop/Start 후 월요일 아침에 크레딧 0으로 시작해서 baseline 이하로만 동작한다. 크레딧이 어느 정도 쌓이기까지 1~2시간 동안 성능이 떨어진다. T3는 금요일 저녁에 Stop해도 크레딧이 보존되어 월요일 아침부터 정상 성능을 낸다.
...
```mermaid
flowchart TD
    A["CloudWatch 메트릭 확인"] --> B{"CPUCreditBalance가\n하루 4시간 이상 0?"}
    B -- "Yes" --> G["M 시리즈 전환 검토"]
    B -- "No" --> C{"CPUSurplusCreditBalance가\n꾸준히 증가?"}
    C -- "Yes" --> D{"Unlimited 추가 비용이\n기본 비용의 50% 이상?\n(2개월 연속)"}
    D -- "Yes" --> G
    D -- "No" --> E["더 큰 T 시리즈로\n업사이징 검토"]
    C -- "No" --> F{"평균 CPUUtilization이\nbaseline × 2 이상?\n(1주일 이상 지속)"}
    F -- "Yes" --> G
    F -- "No" --> H["현재 인스턴스 유지"]
```

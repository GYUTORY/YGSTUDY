...
AWS EC2 T 시리즈는 버스트 성능(Burstable Performance)을 제공하는 범용 인스턴스 패밀리입니다.

**핵심 특징:**
- 평상시에는 기본적인 CPU 성능을 유지
- 필요할 때만 CPU 성능을 일시적으로 높여 사용 가능
- 간헐적이거나 예측 가능한 워크로드에 최적화
- 비용 효율성을 중시하는 사용자들에게 인기

**적합한 워크로드:**
- 웹 서버 (대부분의 시간은 낮은 CPU 사용률)
- 개발 환경 (코딩 시간과 빌드/테스트 시간의 성능 차이)
- 간헐적인 트래픽을 받는 애플리케이션

### T 시리즈가 등장한 배경

클라우드 컴퓨팅 환경에서 많은 애플리케이션들이 24시간 내내 최고 성능을 요구하지 않습니다.

**실제 워크로드 특성:**

**웹 서버:**
- 대부분의 시간 동안 낮은 CPU 사용률
- 특정 시간대에만 트래픽이 급증하여 높은 성능 필요
- 예: 점심 시간, 저녁 시간대의 트래픽 증가

**개발 환경:**
- 개발자가 코딩하는 시간: 낮은 CPU 사용률
- 빌드나 테스트를 실행하는 시간: 높은 CPU 사용률
- 명확한 성능 요구사항의 차이

**기존 인스턴스의 문제점:**
- 고정 성능 인스턴스는 이러한 워크로드 특성을 고려하지 않음
- 필요하지 않은 시간에도 높은 비용을 지불해야 함
- 비용 효율성이 떨어짐

**T 시리즈의 해결책:**
T 시리즈는 이러한 문제를 해결하기 위해 도입된 혁신적인 솔루션입니다. 평상시에는 기본 성능만 제공하고, 필요할 때만 높은 성능을 사용할 수 있어 비용 효율성을 크게 향상시킵니다.

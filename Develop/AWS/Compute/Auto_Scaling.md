...
```mermaid
flowchart LR
    subgraph ASG["Auto Scaling Group (Desired: 3)"]
        direction TB
        I1["Instance A<br/>Healthy"]
        I2["Instance B<br/>Unhealthy"]
        I3["Instance C<br/>Healthy"]
    end

    HC["Health Check"] -->|"장애 감지"| I2
    I2 -->|"종료"| TERM["Terminate"]
    ASG -->|"새 인스턴스 생성"| I4["Instance D<br/>Launching"]
    I4 -->|"Health Check 통과"| ALB["ALB에 등록"]

    style I2 fill:#ef4444,color:#fff
    style I4 fill:#22c55e,color:#fff
    style TERM fill:#6b7280,color:#fff
```
...
flowchart TB
    ...
    style AZ_A fill:#dbeafe,stroke:#3b82f6
    style AZ_B fill:#dcfce7,stroke:#22c55e
    style AZ_C fill:#fef9c3,stroke:#eab308
```
...
flowchart TB
    ...
    CW --> TT["Target Tracking<br/>목표값 유지 방식"]
    CW --> SS["Step Scaling<br/>임계값 단계별 대응"]
    CW --> SC["Scheduled Scaling<br/>시간 기반 예약"]

    TT -->|"CPU 70% 목표"| ASG["Auto Scaling Group"]
    SS -->|"CPU 80% → +1<br/>CPU 90% → +2"| ASG
    SC -->|"09시 → 10대<br/>18시 → 3대"| ASG

    ASG --> OUT["Scale Out<br/>인스턴스 추가"]
    ASG --> IN["Scale In<br/>인스턴스 제거"]

    style TT fill:#dbeafe,stroke:#3b82f6
    style SS fill:#fef9c3,stroke:#eab308
...
**Target Tracking vs Step Scaling 비교:**

```mermaid
flowchart LR
    subgraph TT["Target Tracking"]
        direction TB
        TT1["CPU 85% 감지"]
        TT2["목표 70%와 비교"]
        TT3["필요 인스턴스 계산<br/>85÷70×4 ≈ 5"]
        TT4["1개 추가"]
        TT1 --> TT2 --> TT3 --> TT4
    end

    subgraph SS["Step Scaling"]
        direction TB
        SS1["CPU 85% 감지"]
        SS2["알람 임계값 70%<br/>초과량: 15%"]
        SS3["구간 판단<br/>10~20% 구간"]
        SS4["2개 추가"]
        SS1 --> SS2 --> SS3 --> SS4
    end
...
| 항목 | Target Tracking | Step Scaling |
|------|----------------|--------------|
| 설정 난이도 | 목표값 하나만 지정 | CloudWatch 알람 + 구간별 설정 필요 |
| 반응 속도 | 점진적 조절 | 급증 시 한번에 여러 개 추가 가능 |
| 적합한 상황 | 트래픽이 완만하게 변하는 서비스 | 트래픽이 급격하게 튀는 서비스 |
| 관리 비용 | 낮음 | 높음 (구간별 튜닝 필요) |

대부분의 서비스는 Target Tracking으로 충분하다. Step Scaling은 이벤트성 트래픽 폭주가 잦은 서비스에서 쓴다.
...
sequenceDiagram
    ...
    participant ASG
    participant OLD as Instance B (Unhealthy)
    participant NEW as Instance D (신규)

    HC->>OLD: GET /health
    OLD-->>HC: 503 Error
    HC->>OLD: GET /health (재확인)
    OLD-->>HC: 503 Error
    HC->>ASG: Unhealthy 보고
    ASG->>ALB: Instance B 트래픽 차단
    ASG->>NEW: 새 인스턴스 시작
    NEW-->>ASG: 부팅 완료
    HC->>NEW: GET /health
    NEW-->>HC: 200 OK
    ASG->>ALB: Instance D 트래픽 등록
    ASG->>OLD: 종료
```
...
flowchart TB
    ...
    AASS["Application Auto Scaling"]

    subgraph CLUSTER["ECS Cluster"]
        direction TB
        SVC["ECS Service<br/>(Desired: 4)"]
    ...
    CW -->|"CPU > 70%"| AASS
    AASS -->|"DesiredCount 증가"| SVC
    SVC -->|"Task 추가"| T5["Task 5 (신규)"]
    ALB["ALB / Target Group"] --> TASKS
    ALB --> T5

    style T5 fill:#22c55e,color:#fff
    style AASS fill:#dbeafe,stroke:#3b82f6
```
...
```mermaid
flowchart TB
    SVC["ECS Service<br/>Task 배치 요청"]
    CP["Capacity Provider"]
    ASG["Auto Scaling Group"]

    subgraph CLUSTER["ECS Cluster"]
        direction LR
        subgraph EC2_1["EC2 Instance A"]
            T1["Task 1"]
            T2["Task 2"]
        end
        subgraph EC2_2["EC2 Instance B"]
            T3["Task 3"]
            T4["Task 4"]
        end
        subgraph EC2_3["EC2 Instance C (신규)"]
            T5["Task 5"]
        end
    end
    ...
    CP -->|"2. 인스턴스 추가 요청"| ASG
    ASG -->|"3. EC2 시작"| EC2_3
    EC2_3 -->|"4. 클러스터 등록"| CLUSTER
    SVC -->|"5. Task 배치"| T5

    style EC2_3 fill:#dcfce7,stroke:#22c55e
    style T5 fill:#22c55e,color:#fff

...
tags: [nodejs, testing]
updated: 2025-09-17
description: "테스트 환경에서 외부 HTTP 요청을 가로채 안정적으로 모킹하는 기법"
---

# 외부 API 모킹
## 외부 API 모킹의 개념과 필요성
### 외부 API 모킹이란?
외부 API 모킹은 테스트 환경에서 외부 서비스로 나가는 실제 HTTP 요청을 가로채 미리 정해둔 응답을 돌려주는 기술이다. 외부 의존성 없이 안정적이고 빠르게 테스트를 돌릴 수 있다.

### 외부 API 모킹의 필요성
- **테스트 안정성**: 외부 서비스 장애나 변경에 영향받지 않음
- **테스트 속도**: 네트워크 지연 없이 빠른 테스트 실행
- **예측 가능성**: 일관된 응답으로 테스트 결과 보장
- **비용 절약**: 외부 API 호출 비용과 호출 제한 회피
- **에러 시나리오 테스트**: 여러 에러 상황을 직접 재현

### 기본 개념
- **HTTP 인터셉션**: 네트워크 요청을 가로채 모킹된 응답 반환
- **요청 매칭**: URL, 메서드, 헤더, 바디로 요청 식별
- **응답 시뮬레이션**: 성공, 실패, 지연 같은 응답 상황 구현
- **상태 관리**: 모킹 상태의 활성화/비활성화 관리

## 요청 인터셉션과 응답 시뮬레이션 원리
### 모킹으로 얻는 것과 잃는 것

모킹은 공짜가 아니다. 위에 적은 장점의 대가로 **실제 API 가 바뀌어도 테스트는 계속 통과한다**는 위험을 떠안는다.

모킹 테스트가 검증하는 것은 "우리 코드가 **우리가 상상한 응답**을 잘 처리하는가"다. 상대가 필드 이름을 바꾸거나, 에러 형식을 고치거나, 응답이 배열에서 객체로 변해도 우리 목은 옛 모양 그대로 답한다. 초록불을 보면서 운영에서만 깨진다.

그래서 실무에서는 두 겹으로 간다.

- **모킹 테스트** — 분기·에러 처리·재시도 로직을 빠르게 전수 검증한다. CI 에서 매번 돈다.
- **실제 호출 스모크 테스트** — 응답의 모양이 여전한지만 확인한다. 소수의 케이스로, 별도 잡에서 주기적으로 돈다.

상대가 스펙 문서를 제공하면 계약 테스트(contract test)로 자동화할 수 있지만, 그게 없어도 **"진짜로 한 번은 쳐보는 테스트"를 어딘가에 두는 것**이 핵심이다.

### 1. nock 으로 HTTP 요청 모킹하기
...
아래 설정에서 `nock.disableNetConnect()` 가 핵심이다. **이걸 켜지 않으면 모킹이 걸리지 않은 요청은 실제 네트워크로 나간다.** 테스트가 조용히 외부 서비스를 때리고, 상대 서버 상태에 따라 결과가 흔들린다. 켜 두면 미처 모킹하지 못한 호출이 즉시 에러로 드러나 빠뜨린 곳을 찾을 수 있다.

`localhost` 만 예외로 여는 이유는 테스트용 DB 나 앱 서버가 그쪽에 있기 때문이다.
...
#### 자주 걸리는 것 — 인터셉터는 한 번 쓰면 사라진다

nock 의 인터셉터는 **기본이 1회용이다.** 한 번 매칭되면 소모되고, 같은 주소로 다시 부르면 매칭 실패로 떨어진다.

```javascript
nock('http://api.test').get('/v1').reply(200, { ok: true });

await get('http://api.test/v1');   // 200
await get('http://api.test/v1');   // 실패: Nock: No match for request
```

평소에는 문제가 안 되지만 **재시도 로직을 테스트할 때 반드시 걸린다.** 3회 재시도를 검증하려는데 두 번째 호출부터 목이 없어서, 코드가 아니라 테스트 설정 때문에 실패한다. 원인이 안 보여서 시간을 꽤 잡아먹는다.

호출 횟수만큼 명시하거나, 횟수를 안 정할 거면 `persist()` 를 쓴다.

```javascript
nock('http://api.test').get('/v2').times(2).reply(200, { ok: true });   // 2회
nock('http://api.test').get('/v3').persist().reply(200, { ok: true });  // 무제한
```

`times(n)` 은 **호출 횟수까지 검증**해주는 셈이라 오히려 유용하다. 재시도가 2번이어야 하는데 3번 돌면 세 번째에서 실패하니 바로 잡힌다. `persist()` 는 편하지만 이 검증력을 포기하는 것이고, 테스트가 끝나면 `nock.cleanAll()` 로 반드시 지워야 다음 테스트에 새지 않는다.
...
### 결론
외부 API 모킹은 안정적이고 빠른 테스트에 빠질 수 없는 기술이다.
nock 과 axios-mock-adapter 를 적절히 쓰면 외부 의존성 없이 테스트를 온전히 갖출 수 있다.
장애 시나리오를 여러 개 흉내 내 둘수록 애플리케이션은 튼튼해진다.
모킹 데이터를 한군데서 관리하면 테스트가 일관되고 유지보수도 쉬워진다.

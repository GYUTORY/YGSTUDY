...
권한 모델은 Node 20에 `--experimental-permission` 이라는 이름으로 처음 들어왔고, **22부터 `--permission` 으로 굳었다.** 아래 예시는 22 이상 기준이라 20에서 따라 하려면 플래그 이름을 바꿔야 한다 — 20에서 `--permission` 을 주면 `bad option` 이 난다(실측). 24 에서는 반대로 옛 이름이 제거됐다. 한 마디로 "프로세스가 무엇을 만질 수 있는지" 를 부팅 시점에 화이트리스트로 묶어두는 기능이다. 파일 시스템·네트워크·자식 프로세스·워커 스레드를 따로따로 잠근다. 실험적이라는 꼬리표가 22 LTS 시점에도 붙어 있고, 23~24를 거치며 깎이고는 있지만 아직 "운영 첫 줄에 켜고 끝" 수준은 아니다.

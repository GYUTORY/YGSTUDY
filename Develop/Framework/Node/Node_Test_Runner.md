...
스냅샷이 22.3부터 들어왔다. 그 이전에는 sinon-chai snapshot이나 jest-snapshot을 끌어다 썼지만, 이제 빌트인으로 해결된다.

들어오는 자리를 헷갈리기 쉽다. `node:assert`에는 `snapshot`이 없다. 테스트 컨텍스트에 달린 `t.assert.snapshot()`을 쓴다.
...
스냅샷 파일은 테스트 파일과 같은 디렉터리에 `<테스트파일명>.snapshot`으로 저장된다. 확장자를 바꾸는 게 아니라 뒤에 붙이므로 `api.test.mjs`면 `api.test.mjs.snapshot`이 된다.

**첫 실행은 자동 생성이 아니라 실패한다.** Jest에 익숙하면 여기서 걸린다.

```
not ok 1 - 응답 형식 스냅샷
  error: "Invalid state: Cannot read snapshot file '...api.test.mjs.snapshot.'
          Missing snapshots can be generated by rerunning the command with
          the --test-update-snapshots flag."
  code: 'ERR_INVALID_STATE'
```

만들 때도 갱신할 때도 `--test-update-snapshots`를 붙여야 한다. 파일이 생긴 뒤로는 플래그 없이 비교만 한다. CI에서 이 플래그를 켜두면 스냅샷이 매번 현재 출력으로 덮여서 검증이 통째로 무의미해지니, 로컬 전용으로 둔다.

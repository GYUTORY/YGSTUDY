...
프레임워크를 고를 때 "기본으로 뭘 해주는가"와 "뭘 직접 붙여야 하는가"를 구분하는 게 중요하다. 아래 다이어그램은 각 프레임워크가 어디까지 내장하고, 어디서부터 외부 패키지가 필요한지를 보여준다.

```mermaid
block-beta
    columns 6
    space header1["라우팅"] header2["유효성 검사"] header3["직렬화"] header4["인증 체계"] header5["로깅"]

    express["Express"]:1 e1["내장"]:1 e2["외부\nJoi, Zod 등"]:1 e3["외부\nJSON.stringify"]:1 e4["외부\nPassport 등"]:1 e5["외부\nmorgan+winston"]:1

    fastify["Fastify"]:1 f1["내장"]:1 f2["내장\nAjv"]:1 f3["내장\nfast-json-stringify"]:1 f4["외부\n@fastify/auth"]:1 f5["내장\npino"]:1

    nestjs["NestJS"]:1 n1["내장"]:1 n2["내장\nclass-validator"]:1 n3["하부 엔진 의존"]:1 n4["내장\n@nestjs/passport"]:1 n5["내장\nLogger"]:1

    koa["Koa"]:1 k1["외부\n@koa/router"]:1 k2["외부"]:1 k3["외부"]:1 k4["외부"]:1 k5["외부"]:1

    hapi["Hapi"]:1 h1["내장"]:1 h2["내장\nJoi"]:1 h3["내장"]:1 h4["내장\n@hapi/bell"]:1 h5["외부\nhapi-pino"]:1

    style e1 fill:#4ade80,color:#000
    style e2 fill:#fbbf24,color:#000
    style e3 fill:#fbbf24,color:#000
    style e4 fill:#fbbf24,color:#000
    style e5 fill:#fbbf24,color:#000
    style f1 fill:#4ade80,color:#000
    style f2 fill:#4ade80,color:#000
    style f3 fill:#4ade80,color:#000
    style f4 fill:#fbbf24,color:#000
    style f5 fill:#4ade80,color:#000
    style n1 fill:#4ade80,color:#000
    style n2 fill:#4ade80,color:#000
    style n3 fill:#94a3b8,color:#000
    style n4 fill:#4ade80,color:#000
    style n5 fill:#4ade80,color:#000
    style k1 fill:#fbbf24,color:#000
    style k2 fill:#fbbf24,color:#000
    style k3 fill:#fbbf24,color:#000
    style k4 fill:#fbbf24,color:#000
    style k5 fill:#fbbf24,color:#000
    style h1 fill:#4ade80,color:#000
    style h2 fill:#4ade80,color:#000
    style h3 fill:#4ade80,color:#000
    style h4 fill:#4ade80,color:#000
    style h5 fill:#fbbf24,color:#000
```

초록색이 내장, 노란색이 외부 패키지 필요를 뜻한다. Koa는 라우팅조차 직접 붙여야 하고, Hapi와 NestJS는 대부분 내장이다. Express는 라우팅만 내장이고 나머지는 전부 조합해야 한다. 이 차이가 초기 셋업 시간과 의존성 관리 부담에 직접 영향을 준다.

### 런타임 메모리 사용량 비교

성능을 req/sec만으로 판단하면 안 된다. 컨테이너 환경에서는 메모리 사용량이 비용에 직결된다. 아래는 단순 JSON 응답 서버를 띄웠을 때 RSS(Resident Set Size) 기준 메모리 사용량이다.

```mermaid
xychart-beta
    title "유휴 상태 메모리 사용량 (MB, 낮을수록 좋음)"
    x-axis ["Koa", "Express", "Fastify", "Hapi", "NestJS (Express)"]
    y-axis "RSS (MB)" 0 --> 120
    bar [35, 45, 55, 60, 110]
```

NestJS는 DI 컨테이너와 메타데이터 리플렉션 때문에 유휴 상태에서도 메모리를 많이 먹는다. 모듈 수가 늘어나면 이 수치가 더 올라간다. AWS Lambda나 Cloud Run처럼 메모리 기준으로 과금되는 환경에서는 이 차이가 월 비용에 직접 반영된다.

Koa와 Express는 프레임워크 자체가 가벼워서 메모리 사용이 적다. Fastify는 스키마 컴파일 결과를 메모리에 캐싱하기 때문에 Express보다 약간 높지만, 그 대신 처리량에서 이점을 가져간다.
...
프레임워크 간 전환이 모두 같은 난이도는 아니다. 아래 다이어그램은 실무에서 흔히 발생하는 전환 경로와 각각의 작업량을 보여준다.
...
graph LR
    ...
    Express -- "높은 난이도\n구조 전면 재설계" --> NestJS
    Koa -- "낮은 난이도\n시그니처 유사" --> Express
    NestJS -- "어댑터 교체만\n난이도 낮음" --> NestJS_F["NestJS + Fastify 엔진"]
    Hapi -- "높은 난이도\n생태계 완전히 다름" --> Express

    style Express fill:#68a063,color:#fff
    style Fastify fill:#f5a623,color:#fff
    style NestJS fill:#4a9eff,color:#fff
    style NestJS_F fill:#4a9eff,color:#fff
    style Koa fill:#33d6a6,color:#fff
    style Hapi fill:#ed6c63,color:#fff
```

가장 많이 하는 전환은 Express → Fastify와 Express → NestJS다. NestJS 내부에서 Express 엔진을 Fastify로 바꾸는 건 어댑터만 교체하면 되기 때문에 상대적으로 작업량이 적다. Hapi에서 다른 프레임워크로 옮기는 건 자체 생태계(`@hapi/*` 패키지)를 전부 교체해야 해서 난이도가 높다.

...
- **로드 밸런싱**: 마스터가 요청을 워커 프로세스에 분산 (아래 참고)

### 시작하기 전에 짚을 것 세 가지

**분산은 OS 가 아니라 마스터가 한다.** Node 는 라운드로빈을 기본으로 쓴다. 확인해보면 이렇다.

```javascript
const cluster = require('cluster');
cluster.schedulingPolicy;      // 2 === cluster.SCHED_RR
```

`SCHED_RR` 은 마스터가 커넥션을 받아 워커에 차례로 넘기는 방식이다. Windows 에서는 기본값이 `SCHED_NONE` 이라 OS 커널이 분배하는데, 이 경우 특정 워커에 요청이 몰리는 쏠림이 생길 수 있다. 정책은 `cluster.schedulingPolicy` 로 바꿀 수 있고, **워커를 만들기 전에** 정해야 한다.

**워커끼리 메모리를 공유하지 않는다.** 워커는 스레드가 아니라 별개의 프로세스다. 그래서 이런 것들이 전부 워커마다 따로 논다.

- 인메모리 세션 → 요청이 다른 워커로 가면 로그인이 풀린 것처럼 보인다
- 인메모리 캐시 → 적중률이 워커 수만큼 떨어지고, 무효화가 한 워커에만 반영된다
- 요청 수 제한 카운터 → 실제 허용량이 워커 수배로 늘어난다
- `setInterval` 스케줄러 → 워커 수만큼 중복 실행된다

Cluster 를 붙이는 순간 이 네 가지는 Redis 같은 외부 저장소로 빼거나, 마스터에서만 돌게 옮겨야 한다. **단일 프로세스에서 잘 돌던 코드가 조용히 틀리기 시작하는 지점이 여기다.**

**`os.cpus().length` 를 그대로 쓰면 컨테이너에서 과하게 뜬다.** 이 값은 머신의 코어 수라서, CPU 1~2개로 제한된 컨테이너 안에서도 호스트 기준 숫자를 돌려준다. 워커 수십 개가 뜨면 컨텍스트 스위칭과 메모리만 늘고 처리량은 오히려 떨어진다.

```javascript
// 환경변수로 명시하고, 없을 때만 자동값을 쓴다
const workers = Number(process.env.WEB_CONCURRENCY)
  || (os.availableParallelism?.() ?? os.cpus().length);
```

`os.availableParallelism()` 은 CPU 친화도(affinity)를 반영하므로 `os.cpus().length` 보다는 낫지만, cgroup 의 CPU 할당량은 별개 장치라 이것도 완벽하지 않다. **오케스트레이터에서 값을 주입하는 쪽이 확실하다.**

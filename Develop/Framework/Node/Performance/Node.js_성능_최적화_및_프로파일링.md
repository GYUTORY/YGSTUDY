...
성능 최적화는 애플리케이션의 응답 시간, 처리량, 리소스 사용률을 개선해 더 나은 사용자 경험과 비용 효율성을 얻는 작업이다.
...
Node.js 성능 최적화는 애플리케이션의 응답 시간, 처리량, 리소스 사용률을 개선해 더 나은 사용자 경험과 비용 효율성을 얻는 작업이다.

### 성능 최적화 프로세스

최적화는 순서를 지켜야 효과가 난다. 아래 플로우차트가 성능 최적화의 전체 프로세스다.
...
Clinic.js는 설정 파일로 프로파일링 강도를 조절한다. 주요 설정 항목은 다음과 같다:
...
**참고:**
- 프로덕션 환경에서는 duration을 짧게 잡고 조금씩 늘린다
- 샘플링 간격이 너무 짧으면 오버헤드가 붙으니 적당히 조정한다
- 메모리 프로파일링은 시간을 충분히 두고 돌려야 패턴이 보인다
...
0x는 프로파일링 결과를 HTML 형식으로 내놓는다. 여기서 볼 정보는 다음과 같다:
...
**참고:**
- 0x 결과의 플레임 그래프로 호출 체인을 눈으로 확인한다
- 상위 10개 핫스팟에 집중해 최적화를 진행한다
- 메모리 누수는 시간에 따른 메모리 증가 패턴으로 확인한다
...
Node.js의 내장 프로파일링만 써도 외부 도구 없이 기본적인 성능 분석은 된다.

**메모리 사용량 추적**:
- `process.memoryUsage()`를 주기적으로 찍어 메모리 사용 패턴을 본다
...
**CPU 사용률 추적**:
- `process.cpuUsage()`로 CPU 사용 패턴을 분석한다
- **user**: 사용자 모드 CPU 시간
...
- 데이터 저장: JSON 형식으로 저장해 나중에 다시 분석할 수 있게 구성

**참고:**
- 프로덕션 환경에서는 오버헤드를 줄이려고 샘플링 간격을 늘린다
- 메모리 증가 추세를 그래프로 그리면 누수 패턴이 드러난다
- CPU 사용률이 계속 높은 구간을 골라 최적화 대상으로 삼는다

## 메모리 누수 탐지 및 해결

### 메모리 누수 패턴 개요

메모리 누수는 애플리케이션이 더 이상 쓰지 않는 메모리를 해제하지 않아 생기는 문제다. Node.js에서 흔한 누수 패턴부터 알아 두는 게 좋다.
...
heapdump는 Node.js 애플리케이션의 메모리 힙 스냅샷을 떠서 메모리 누수를 분석하게 해 주는 도구다.
...
**메모리 모니터링:**
- **임계값 설정**: 보통 50MB를 넘으면 한 번 들여다봐야 한다
- **모니터링 간격**: 5초 간격이면 충분하며, 더 자세한 분석이 필요하면 1초 간격 사용
- **스냅샷 비교**: 시간에 따른 여러 스냅샷을 비교해 메모리 증가 패턴을 파악한다

**스냅샷 분석 방법:**
- Chrome DevTools의 Memory 프로파일러로 `.heapsnapshot` 파일을 열어 분석한다
- Comparison 모드로 스냅샷 간 차이를 확인한다
- Retainers에서 메모리를 붙잡고 있는 참조를 찾는다

**참고:**
- 프로덕션 환경에서는 스냅샷 생성이 성능을 건드리니 신중하게 쓴다
- 메모리 사용량이 계속 증가하는 패턴을 확인한다
- 큰 객체나 배열이 메모리에 남아있는지 확인한다

### 3. 메모리 누수 해결

#### WeakMap과 WeakSet 활용

WeakMap과 WeakSet은 객체를 키로 쓰면서 그 객체를 강하게 붙잡지는 않는다. 메모리 누수를 막는 데 아주 요긴하다.

**WeakMap 활용 시나리오:**

**메모리 효율적인 캐싱:**
- 객체를 키로 쓰는 캐시를 구현할 때 WeakMap을 쓴다
- 객체가 가비지 컬렉션되면 캐시에서도 자동으로 빠진다
- 별도 정리 로직이 필요 없어 메모리 관리가 자동화된다

**메타데이터 저장:**
- 객체에 추가 정보를 저장해야 할 때 WeakMap을 쓴다
- 원본 객체를 건드리지 않고도 추가 정보를 연결한다
- 객체가 사라지면 메타데이터도 자동으로 정리된다

**WeakSet 활용 시나리오:**

**객체 추적:**
- 특정 객체를 추적해야 할 때 WeakSet을 쓴다
- 객체가 가비지 컬렉션되면 추적 목록에서도 자동으로 빠진다
- 순회가 필요 없는 경우 WeakSet이 적합하다

**중복 체크:**
- 객체의 중복 여부를 확인할 때 WeakSet을 쓴다
- 메모리 효율이 좋고 자동 정리까지 된다

**사용 시 주의사항:**
- WeakMap/WeakSet은 객체만 키로 쓸 수 있다 (원시 타입 불가)
- 키로 쓴 객체가 다른 곳에서 참조되지 않으면 자동으로 제거된다
- 순회(iteration)가 불가능하므로 모든 키를 확인할 수 없다

#### 메모리 풀링 패턴

메모리 풀링은 객체를 재사용해 메모리 할당과 해제의 오버헤드를 줄이는 기법이다. 자주 만들고 버리는 객체일수록 효과가 크다.
...
- 풀 크기를 모니터링해 최적값을 찾는다
- 과도한 풀링은 오히려 메모리를 낭비하니 신중하게 적용한다
...
Worker Threads는 CPU 집약적 작업을 별도 스레드로 넘겨 메인 스레드가 막히지 않게 해 준다.
...
**워커 수 결정:**
- 보통 CPU 코어 수와 똑같이 잡는다
- I/O 작업이 섞여 있으면 워커 수를 더 늘려도 된다
- 메모리 제한을 보고 워커 수를 조정한다
...
**워커 풀 관리:**
- 워커를 미리 만들어 초기화 오버헤드를 줄인다
- 작업 큐를 둬서 워커가 모두 바쁠 때 대기시킨다
- 워커 오류가 나면 다시 만들거나 대체 워커를 쓴다
...
**주의사항:**
- 워커끼리 메모리를 공유하지 못하니 데이터 복사 오버헤드가 붙는다
- 작은 작업에는 워커 오버헤드가 더 클 수 있다
- I/O 작업에는 Worker Threads보다 비동기 I/O가 더 효율적이다

### 2. 알고리즘 최적화

#### 효율적인 데이터 구조 선택

알고리즘 최적화는 결국 데이터 구조를 고르는 일이다. 각 데이터 구조의 시간 복잡도를 알고 상황에 맞게 골라야 한다.
...
**메모리 고려사항:**
- Set과 Map은 해시 테이블을 쓰니 메모리 오버헤드가 있다
- 작은 데이터셋에서는 배열이 더 효율적일 수 있다
- 데이터 크기와 접근 패턴을 보고 고른다

#### 메모이제이션을 통한 계산 최적화

메모이제이션은 함수 결과를 캐시해 같은 입력에 대한 재계산을 막는 최적화 기법이다.
...
- 캐시 크기를 모니터링해 메모리 사용량을 관리한다

### 3. 비동기 처리 최적화

#### Promise.all을 사용한 병렬 처리

비동기 작업을 병렬로 돌리면 전체 실행 시간이 크게 줄어든다. 그렇다고 모든 상황에서 병렬 처리가 최선은 아니다.
...
**참고:**
- I/O 바운드 작업은 병렬 처리 효과가 크다
- CPU 바운드 작업은 Worker Threads를 고려한다
- 외부 API 호출은 Rate Limit을 보고 제한된 병렬 처리를 쓴다
...
**통합된 기존 파일들**: 이 섹션은 다음 기존 파일들의 내용을 합쳤다.
...
JavaScript는 기본적으로 싱글 스레드 언어다. 한 번에 하나의 작업만 수행한다. 그런데 웹 애플리케이션에서는 네트워크 요청, 파일 읽기, 타이머 같은 여러 작업을 동시에 처리해야 한다.
...
이벤트 루프가 JavaScript 비동기 처리를 떠받치는 핵심 메커니즘이다. Node.js의 이벤트 루프 구조를 알아 두면 성능 최적화에 큰 도움이 된다.
...
콜백 함수는 다른 함수에 인자로 전달되어 나중에 실행되는 함수다. Node.js는 비동기 작업의 결과를 처리하려고 콜백 패턴을 광범위하게 쓴다.
...
콜백을 겹겹이 쌓으면 코드가 복잡해지고 가독성이 떨어진다. 흔히 말하는 "콜백 지옥"이다.
...
Promise는 비동기 작업의 최종 완료(또는 실패)와 그 결과값을 나타내는 객체다.
...
#### 논블로킹 I/O의 핵심 개념
Node.js는 기본적으로 **논블로킹 I/O(Non-Blocking I/O) 모델**을 써서 싱글 스레드에서도 높은 성능을 유지한다.
...
Node.js의 Event Loop는 다음 단계를 밟는다. 각 단계의 역할과 실행 순서를 알면 비동기 코드의 동작을 정확히 예측할 수 있다.
...
1. **Timers**: `setTimeout()`, `setInterval()` 콜백 실행
   - 타이머가 만료된 콜백들을 실행한다
   - 이 단계에서 실행할 콜백이 없으면 다음 단계로 넘어간다

2. **Pending Callbacks**: 완료된 비동기 I/O 콜백 실행
   - 이전 루프에서 지연된 콜백들을 실행한다
   - 대부분의 I/O 콜백은 Poll 단계에서 처리된다

3. **Idle, Prepare**: 내부 정리 작업
   - Node.js 내부에서만 쓰는 단계다
   - 개발자가 직접 제어할 일은 없다

4. **Poll**: 새로운 I/O 이벤트 대기 및 처리
   - 새로운 I/O 이벤트를 가져와 관련 콜백을 실행한다
   - 실행할 콜백이 없으면 Check 단계로 넘어간다
   - 이 단계에서 대부분의 I/O 작업이 처리된다

5. **Check**: `setImmediate()` 콜백 실행
   - Poll 단계가 끝난 뒤 곧바로 실행된다
   - Poll 단계에서 대기 중인 콜백이 없으면 이 단계로 넘어간다

6. **Close Callbacks**: 소켓 종료 등 작업 처리
   - 소켓이나 핸들이 갑자기 닫힌 경우의 콜백을 처리한다
   - 예: `socket.on('close', ...)`

#### 이벤트 큐의 종류와 우선순위

이벤트 루프는 여러 종류의 큐를 관리하고, 큐마다 우선순위가 다르다. 이 우선순위를 알면 비동기 코드의 실행 순서가 그대로 읽힌다.
...
async/await는 Promise를 더 쉽게 쓰게 해 주는 문법적 설탕이다.
...
JavaScript는 기본적으로 **싱글 스레드**로 동작한다. 한 번에 하나의 작업만 처리한다. Web Workers를 쓰면 **별도의 스레드에서 작업을 수행**하니 메인 스레드가 멈추지 않는다.
...
- Worker와 메인 스레드 간 통신은 `postMessage()`로만 가능하다
- 복사 가능한 데이터만 전송할 수 있다 (함수, DOM 객체 등은 전송 불가)
...
## 요약
Node.js 성능 최적화는 계속 재고 고쳐 나가야 하는 작업이다.

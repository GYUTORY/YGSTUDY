...
#### 응답 스키마는 보안 장치이자 함정이다

위에서 `preSerialization` 과 `onSend` 를 가르며 지나간 것 — 스키마에 없는 필드가 사라지는 동작 — 은 따로 볼 값이 있다. **이점과 사고가 같은 뿌리에서 나온다.**

```javascript
app.get('/user', {
  schema: { response: { 200: { type: 'object',
    properties: { id: { type: 'integer' }, name: { type: 'string' } } } } }
}, async () => ({ id: 1, name: 'kyg', email: 'a@b.c', passwordHash: '$2b$...' }));
```

```
GET /user   →  {"id":1,"name":"kyg"}
스키마 없음  →  {"id":1,"name":"kyg","email":"a@b.c","passwordHash":"$2b$..."}
```

**이점 쪽이 먼저다.** 핸들러가 실수로 `passwordHash` 를 흘려도 스키마가 막는다. ORM 엔티티를 그대로 반환하는 흔한 패턴에서 내부 필드가 새어 나가는 사고를 구조적으로 차단한다 — `select` 를 빠뜨렸든 나중에 컬럼이 추가됐든, 응답에 나갈 수 있는 건 스키마에 적힌 것뿐이다. 중첩 객체와 배열 안까지 적용된다.

```
/nested  →  {"user":{"id":1}}              ← user.secret 이 걸러졌다
/arr     →  [{"id":1},{"id":2}]            ← 배열 항목의 name 도 걸러졌다
```

덤으로 타입도 맞춰 준다. `{ id: '42' }` 를 반환해도 `{"id":42}` 로 나간다.

#### 그런데 같은 동작이 필드를 조용히 삼킨다

같은 규칙이 반대로 물린다. **응답에 필드를 추가했는데 스키마를 안 고치면 그 필드는 나가지 않는다.**

```javascript
app.get('/added', {
  schema: { response: { 200: { type: 'object', properties: { id: { type: 'integer' } } } } }
}, async () => ({ id: 1, newFeature: '방금 추가한 필드' }));
// → {"id":1}
```

에러도 경고도 없다. 핸들러는 분명히 반환했고, 프런트는 `undefined` 를 받는다. 로그를 봐도 정상이다. **"백엔드는 넣었다는데 프런트에는 안 온다"** 가 여기서 나온다. 서버 로그에 반환값을 찍어 보면 필드가 있어서 더 헷갈린다 — 사라지는 건 직렬화 단계이기 때문이다.

#### 상태 코드마다 따로다

이게 가장 위험한 함정이다. 응답 스키마는 **상태 코드별로** 정의되고, 스키마가 없는 코드는 **필터링이 아예 안 걸린다.**

```javascript
app.post('/created', {
  schema: { response: { 200: { type: 'object', properties: { id: { type: 'integer' } } } } }
}, async (req, reply) => { reply.code(201); return { id: 1, extra: '남아있나' }; });
// → 201  {"id":1,"extra":"남아있나"}       ← 201 스키마가 없어서 그대로 나갔다
```

200 만 정의해 두고 생성 응답을 201 로 주면, **그 경로에서는 보안 필터가 통째로 꺼진다.** 200 응답만 보고 "우리는 응답 스키마로 막고 있다" 고 믿는 상태에서 201·202 로 민감 필드가 새는 구조다.

방법은 두 가지다. 코드별로 다 적거나, 범위 표기(`2xx`)를 쓴다.

```javascript
schema: {
  response: {
    '2xx': { type: 'object', properties: { id: { type: 'integer' } } },
...
| 상황 | 결과 |
|---|---|
| 스키마에 없는 필드 | **조용히 사라진다** (에러 없음) |
| 중첩 객체·배열 항목 | 같은 규칙이 적용된다 |
| 타입이 다른 값 | 스키마 타입으로 변환된다 |
| 스키마 없는 상태 코드 | **필터링 자체가 안 걸린다** |
| `additionalProperties: true` | 나머지 필드도 통과한다 |

`additionalProperties: true` 는 필터링을 포기하는 대신 유연해지는 선택지다. 다만 그러면 위의 보안 이점도 같이 사라진다는 걸 알고 써야 한다.

**응답 필드를 추가할 때 스키마를 같이 고치는 걸 잊지 않는 게 요점이고**, 잊는 걸 전제하면 통합 테스트에서 응답 본문의 키를 실제로 단언하는 편이 낫다. 타입 정의만 고치고 스키마를 안 고치면 타입 검사는 통과하는데 런타임에서 사라진다.

...
요청 수신
  │
  ▼
app.use() 미들웨어 (등록 순서대로)
  │  body-parser → cors → 커스텀 로깅 → ...
  │  각 미들웨어에서 next() 호출해야 다음으로 넘어감
  ▼
라우터 매칭 (선형 탐색)
  │  app.get('/users/:id') 같은 라우트를 등록 순서대로 검색
  ▼
라우트 미들웨어
  │  router.use()로 특정 라우터에 붙인 미들웨어
  ▼
라우트 핸들러
  │  (req, res) => { res.json(...) }
  ▼
에러 발생 시 → 에러 미들웨어 (4개 인자)
  │  (err, req, res, next) => { ... }
  ▼
응답 전송
```
...
onRequest 훅
  ...
  ▼
preParsing 훅
  │  요청 본문 파싱 전 처리 (스트림 변환 등)
  ▼
본문 파싱 (content-type 기반 자동 처리)
  ▼
preValidation 훅
  │  스키마 검증 전 데이터 가공
  ▼
스키마 검증 (JSON Schema 기반, 컴파일된 검증기)
  ▼
preHandler 훅
  │  핸들러 실행 전 마지막 처리
  ▼
...
```
요청 수신 (Express 또는 Fastify 엔진)
  │
  ▼
글로벌 미들웨어
  │  Express 호환 미들웨어 (cors, helmet 등)
  ▼
가드 (Guard)
  │  @UseGuards(AuthGuard)
  │  true/false 반환 → false면 403
  ▼
인터셉터 (Interceptor) - 전처리
  │  @UseInterceptors(LoggingInterceptor)
  │  요청 전후로 로직 추가 가능 (RxJS Observable 체인)
  ▼
파이프 (Pipe)
...
컨트롤러 핸들러
  ...
  ▼
인터셉터 (Interceptor) - 후처리
  │  응답 변환, 캐싱, 로깅
  ▼
예외 필터 (Exception Filter)
  │  에러 발생 시 @Catch()로 잡아서 처리
  ▼
응답 전송
```
...
onPreAuth
  ...
  ▼
인증 (authentication)
  │  server.auth.strategy()로 등록한 인증 로직
  ▼
onCredentials
  │  인증 성공 후, 권한 체크 전
  ▼
...
```mermaid
xychart-beta
    title "초당 요청 처리량 (req/s) - 높을수록 좋음"
    x-axis ["Fastify", "Hapi", "Express", "NestJS+Express", "NestJS+Fastify"]
    y-axis "req/s (x1000)" 0 --> 80
    bar [65, 42, 30, 22, 45]
```

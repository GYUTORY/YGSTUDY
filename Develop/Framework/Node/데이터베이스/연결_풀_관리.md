...
#### `min` 은 연결을 미리 만들지 않는다

위 생명주기 다이어그램에는 "초기화 → 최소 연결 생성"이 있지만, `pg` 에서는 그런 일이 일어나지 않는다. 풀을 만들자마자 세어보면:

```javascript
const pool = new Pool({ host: '127.0.0.1', port: 59999, min: 5, max: 20 });
console.log(pool.totalCount, pool.idleCount);   // → 0 0
// 2초 뒤에도                                    → 0 0
```

`pg-pool` 3.14.0 소스에서 `min` 이 쓰이는 곳은 딱 한 군데다.
...
if (this.options.idleTimeoutMillis && this._isAboveMin()) {
  // 유휴 타임아웃으로 연결을 닫을지 결정할 때만 본다
}
```

즉 `min` 의 뜻은 **"미리 이만큼 만들어 둔다"가 아니라 "이 아래로는 줄이지 않는다"** 이다. 연결은 첫 요청이 올 때 만들어진다.

그래서 배포 직후 첫 요청들은 **연결 수립 시간을 그대로 문다.** TLS 를 쓰거나 DB 가 다른 AZ 에 있으면 이게 꽤 크다. 워밍업이 필요하면 부팅 시 직접 만든다.

```javascript
await Promise.all(Array.from({ length: 5 }, async () => {
  const c = await pool.connect();
  c.release();          // 반납하면 idle 로 남는다
}));
```

`min` 이라는 이름만 보고 워밍업이 됐다고 믿으면 "재시작 직후에만 느리다"의 원인을 엉뚱한 데서 찾게 된다. 풀 설정을 옮겨 심을 때는 **같은 이름의 옵션이 같은 뜻인지** 라이브러리 소스에서 확인하는 편이 빠르다.
...
#### `Promise.race` 는 진 쪽을 취소하지 않는다

이 두 메서드는 "타임아웃"이라는 이름과 달리, **아무것도 중단시키지 못한다.** `Promise.race` 는 먼저 확정된 결과를 돌려줄 뿐이고 나머지 Promise 는 계속 돈다.

가장 먼저 눈에 띄는 증상은 프로세스가 안 끝나는 것이다.

```javascript
queryWithTimeout(3000);   // 쿼리는 100밀리초 만에 끝나는 상황
```

```
101밀리초: race 해결 → 쿼리 결과
3002밀리초: 프로세스 종료      ← 타이머가 살아 있다
```

쿼리가 100밀리초에 끝나 race 는 곧장 확정됐는데, 취소되지 않은 `setTimeout` 이 이벤트 루프를 3초 동안 붙잡는다. CLI 도구나 배치 작업이 "끝났는데 안 끝나는" 이유가 대개 이것이다. `clearTimeout` 을 넣으면 해결된다.

더 심각한 건 반대 경우다. **타임아웃이 이겨도 쿼리는 DB 에서 계속 실행된다.** 커넥션도 붙잡힌 상태 그대로다. 느린 쿼리에 타임아웃을 걸어 요청을 빨리 끊었더니, 커넥션은 여전히 점유돼 있어 **풀이 순식간에 마르는** 상황이 만들어진다. 타임아웃을 넣기 전보다 나빠진다.

`getConnectionWithTimeout` 은 더 나쁘다. 바깥 타임아웃이 먼저 터지면 `reject` 된 뒤에 `pool.connect()` 가 뒤늦게 성공하는데, **그 클라이언트를 아무도 `release()` 하지 않는다.** 영구 누수다.
...
// 커넥션 획득 대기는 pg 가 이미 지원한다 — 직접 만들지 않는다
const pool = new Pool({ connectionTimeoutMillis: 2000 });
```

`statement_timeout` 은 PostgreSQL 서버가 쿼리를 중단시키므로 커넥션이 실제로 풀려난다. 애플리케이션 쪽 타이머는 "언제 포기할지"만 정할 수 있고 "무엇을 멈출지"는 정하지 못한다는 차이를 기억한다.
...
**주석과 코드가 다르다.** `readQuery` 에는 `// Round Robin으로 Slave 선택` 이라고 적혀 있지만 실제로는 `Math.floor(Math.random() * length)` 다. 무작위 선택은 라운드 로빈이 아니고, 요청이 적을 때 한쪽으로 몰릴 수 있다. 그리고 슬레이브 설정마다 넣어둔 `weight: 2` / `weight: 1` 은 **이 클래스에서 한 번도 읽히지 않는다.** 설정 파일만 보면 가중치가 동작한다고 믿게 된다.

**슬레이브 실패 시 마스터로 폴백하는 설계도 다시 볼 필요가 있다.** 슬레이브가 죽는 이유가 대개 과부하인데, 그 부하를 그대로 마스터로 보내면 마스터까지 같이 넘어간다. 읽기 트래픽이 쓰기보다 훨씬 많은 구조라면 특히 그렇다. 최소한 폴백에도 회로 차단기나 별도 한도를 둔다.

**복제 지연을 다루는 코드가 없다.** 쓰기 직후 같은 데이터를 읽으면 슬레이브에는 아직 반영이 안 됐을 수 있다. "저장했는데 목록에 안 보인다"는 버그가 여기서 나온다. 방금 쓴 사용자의 후속 읽기는 일정 시간 마스터로 보내는 식의 처리가 필요하다.

...
### 가드가 거부한 요청은 Interceptor 를 타지 않는다

실행 순서에서 곧바로 따라오는 결론인데, 운영에서 사고를 낼 때까지 잘 안 보인다. 가드가 `false` 를 반환하거나 예외를 던지면 **즉시 Exception Filter 로 점프**한다. 그 사이의 Interceptor 는 실행되지 않는다.

Nest 12.0.1 에서 같은 앱에 통과 요청과 거부 요청을 한 번씩 보내고 각 단계를 찍어 봤다.

```
통과 요청 (200):            거부 요청 (401):
  가드 실행                   가드 실행
  인터셉터 진입               가드 거부 → 401 throw
  핸들러                      예외 필터
  인터셉터 종료 0ms
```

거부된 쪽에는 인터셉터가 아예 없다. 여기서 두 가지가 조용히 깨진다.

#### 1. 인증 실패가 접근 로그에 안 남는다

로깅을 Interceptor 로 구현한 앱에서는 **401·403 요청이 로그에 통째로 빠진다.** 200 과 500 은 다 남는데 인증 실패만 없다.

하필 그게 가장 남아야 하는 신호다. 같은 IP 에서 401 이 분당 수백 건 나는 것은 토큰을 긁고 있다는 뜻이고, 특정 계정에서만 403 이 몰리는 것은 권한 밖 리소스를 훑고 있다는 뜻이다. 침입 탐지의 1차 재료가 통째로 비어 있는 셈이다.

더 나쁜 건 **비어 있다는 사실 자체가 안 보인다**는 점이다. 로그가 잘 쌓이고 있으니 정상으로 보인다. "우리 서비스는 인증 실패가 거의 없네" 로 읽힌다.

#### 2. 응답 시간 지표가 실제보다 좋게 나온다

거부된 요청은 지표에서도 빠지므로 분모가 줄어든다. 그런데 이쪽보다 큰 문제가 따로 있다 — **인터셉터가 재는 시간은 가드 이후부터**다.

토큰 검증이 느려진 상황(공개키 조회, Redis 블랙리스트 확인)을 만들어 미들웨어와 인터셉터로 각각 재 봤다.

```javascript
app.use((req, res, next) => {                 // 미들웨어 = 가드보다 앞
  const t = Date.now();
  res.on('finish', () => { req.__middlewareMs = Date.now() - t; });
  next();
});
app.useGlobalGuards(new SlowGuard());          // 여기서 300ms 지연
app.useGlobalInterceptors(new Timer());        // 인터셉터는 이 뒤부터 잰다
```

```
미들웨어로 잰 시간 : 328ms   ← 사용자가 실제로 기다린 시간
인터셉터로 잰 시간 : 22ms    ← 가드 이후부터
가드에서 사라진 시간: 약 306ms
```

**328ms 중 306ms 가 지표에 안 잡힌다.** 이런 장애는 이렇게 나타난다 — 사용자는 "느리다" 고 하는데 대시보드의 p99 는 20ms 대이고, APM 도 정상이다. 애플리케이션 안에서만 보면 원인을 찾을 수가 없다. 토큰 검증에 쓰는 Redis 나 공개키 엔드포인트가 느려지는 장애가 정확히 이 모양이다.

#### 대응

**접근 로그는 미들웨어로 내린다.** 미들웨어는 가드보다 앞이라 거부된 요청도 지나간다. 상태 코드는 `res.on('finish')` 에서 읽으면 된다.

```typescript
// 모든 요청이 여기를 지난다 — 401 도, 403 도
export function accessLog(req: Request, res: Response, next: NextFunction) {
  const t = Date.now();
  res.on('finish', () => {
    logger.info({
      method: req.method,
      url: req.originalUrl,      // 마운트로 잘리지 않은 원본
      status: res.statusCode,    // 401·403 이 여기 찍힌다
      ms: Date.now() - t,        // 가드 시간이 포함된 진짜 소요 시간
    });
  });
  next();
}
...
다만 미들웨어는 어느 핸들러가 실행될지 모른다(위 "Middleware" 절 참고). 컨트롤러·메서드 이름이나 인가 결과 같은 것은 여전히 Interceptor 몫이다. **둘 다 두고 역할을 나누는 게 맞다** — 미들웨어는 "무엇이 들어왔고 어떻게 끝났나", 인터셉터는 "핸들러 안에서 무슨 일이 있었나".

그리고 애플리케이션 지표만 믿지 않는다. ALB·Nginx 같은 **프레임워크 바깥 계층의 응답 시간과 대조**하면 가드에서 사라지는 구간이 드러난다. 두 숫자가 벌어지기 시작하면 그 사이 어딘가가 느려진 것이다.

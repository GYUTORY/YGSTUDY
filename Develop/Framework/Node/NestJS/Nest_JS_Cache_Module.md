...
async findById(id: string): Promise<Product | null> {
  ...
  const promise = this.productRepo.findOne(id)
    .then(async (product) => {
...
여기서 가장 흔한 사고는 `then` 안에서만 `inflight.delete(key)`를 호출하는 것이다. DB 쿼리가 에러를 던지면 `then`이 안 돌아서 키가 영원히 맵에 남는다. 같은 ID로 다시 들어오는 모든 요청이 reject된 promise를 받게 되고, 맵 크기는 계속 늘어나서 메모리 누수가 된다. 반드시 `finally`로 정리해야 한다.
...
```typescript
private readonly pendingDeletes = new Set<NodeJS.Timeout>();
...
async update(id: string, dto: UpdateProductDto): Promise<Product> {
  ...
  const timer = setTimeout(() => {
    this.pendingDeletes.delete(timer);
    this.cache.del(key).catch((err) =>
      this.logger.warn(`delayed cache del failed: ${err.message}`),
    );
  }, 500);
  this.pendingDeletes.add(timer);

  return updated;
}

onModuleDestroy() {
  for (const timer of this.pendingDeletes) {
    clearTimeout(timer);
  }
...
`setTimeout`을 그냥 던지면 두 가지 누수가 생긴다. 첫째, 프로세스가 종료될 때 pending timer가 이벤트 루프를 붙잡아서 graceful shutdown이 안 끝난다. 둘째, 쓰기 트래픽이 많으면 동시에 살아있는 timer 객체가 누적되면서 메모리가 증가한다. Set에 핸들을 모아두고 `onModuleDestroy`에서 정리하는 게 안전하다. 더 강한 보장이 필요하면 BullMQ 같은 큐에 지연 작업으로 등록한다. 다만 대부분의 경우 짧은 TTL + 즉시 삭제로 충분하다.
...
캐시 자체가 메모리 누수의 원인이 되는 경우가 의외로 많다. 패턴별로 정리한다.

### 메모리 스토어의 무제한 키 증가

`CacheModule.register({ max: 1000 })`에서 `max`를 빼먹으면 메모리 스토어는 무한정 키를 쌓는다. 인메모리 LRU는 `max`가 있어야 evict가 동작한다. 사용자별·요청별 키를 메모리 스토어에 박는 코드가 들어가면 시간 지나 OOM이 난다.
...
`max`만으로 안심하면 안 된다. TTL이 길면 짧은 시간 안에 `max`까지 차고, 그 뒤로는 evict 비용이 매 요청마다 든다. 메모리 스토어는 키 개수가 수천 단위를 넘어가면 더 이상 적합하지 않다고 보고 Redis로 옮긴다.

### Redis 스토어인데 메모리 누수가 일어나는 케이스

Redis로 옮겼는데도 NestJS 프로세스 메모리가 계속 늘어나는 경우가 있다. 원인은 보통 다음 셋 중 하나다.

첫째, 응답을 캐시에 넣기 전에 그대로 들고 있는 클로저. cache-aside 패턴에서 `findOne` 결과에 다른 객체 그래프(부모 엔티티, 연관 캐시 등)를 매단 채로 캐시에 넣으면 직렬화 후에도 원본 참조가 어딘가에 살아있을 수 있다. DTO로 일단 평탄화한 뒤 캐시에 넣는다.

둘째, `inflight` Map의 키가 정리되지 않는 경로. 위에서 본 것처럼 `finally`를 빠뜨리면 reject된 promise와 키가 누적된다. 운영에서 메모리가 천천히 우상향하면 가장 먼저 의심한다.

셋째, `pino` 같은 로거에 캐시 hit/miss 메타데이터를 동기 transport로 쏟아붓는 경우. 캐시 호출이 잦은 핫패스에서는 로깅 자체가 메모리 버퍼를 갉아먹는다. `level: 'debug'`인 채로 운영에 올라가면 며칠 안에 RSS가 두 배가 된다.

### Redis 자체 메모리 누수 — TTL 누락

`set`에 TTL을 안 주면 키가 영원히 산다. `cache-manager` v5는 `set(key, value)` 형태로 호출하면 모듈 등록 시점의 기본 TTL을 쓰는데, 기본 TTL을 0이나 음수로 잘못 줘 두면 TTL 없는 키가 쌓인다.

```typescript
// 위험: TTL 인자를 빠뜨림
await this.cache.set('product:1', product);

// 안전: 명시적으로 TTL을 준다
await this.cache.set('product:1', product, 60_000);
```

Redis `INFO keyspace`를 보면 `expires=` 값이 키 전체 개수보다 한참 작을 때가 있다. 이게 TTL 없는 키가 쌓이고 있다는 신호다. `redis-cli --scan | xargs -L 1 redis-cli TTL`로 샘플을 떠 보고, `-1`(no expire)이 많으면 누가 어디서 TTL 없이 `SET`을 하고 있는지 추적한다.

### 큐·이벤트 리스너 누수

`Cache` 인스턴스가 내부적으로 들고 있는 Redis 클라이언트는 이벤트 emitter다. 인스턴스 종료 시 `quit()`를 안 부르면 연결이 남아서 통합 테스트에서 핸들이 누적된다. NestJS는 `onModuleDestroy`에서 자동으로 닫아주지만, 직접 `redisStore`를 만들어 쓸 때는 라이프사이클 훅에서 직접 정리한다.
...
테스트 환경에서 `--detectOpenHandles`를 켜고 돌렸을 때 Redis 소켓이 남으면 십중팔구 이 정리가 빠진 것이다.

### 캐시 키에 동적 객체를 박는 실수

`cache.set(JSON.stringify(filterDto), result)`처럼 DTO 전체를 키로 박으면 키 카디널리티가 통제 불가능해진다. Redis 메모리 누수의 가장 흔한 원인이고, 사실상 영구히 산다(TTL은 있지만 키가 다시 안 들어오므로 evict 외에는 정리 수단이 없음). 키는 의미 있는 필드 몇 개만 골라서 짧은 해시로 만든다.

```typescript
import { createHash } from 'node:crypto';

function listKey(filter: ProductFilter): string {
  const normalized = {
...
필터 필드를 화이트리스트로 명시하면 새로운 옵션이 추가돼도 키가 폭증하지 않는다. 해시 길이는 12자리면 충돌 확률이 사실상 0이다.
...
CacheModule은 30분이면 붙일 수 있지만 운영하면서 30번은 데인다. 자동 캐싱은 매력적이지만 사용자별 응답·정합성 민감 데이터에는 함정이 많다. 수동 cache-aside가 코드는 길어도 통제할 지점이 명확해서 운영이 쉽다. TTL 설계, 키 prefix·버전, 무효화 시점 이 세 가지를 결정하고 시작하면 사고가 훨씬 줄어든다. 메모리 누수는 inflight Map의 `finally` 누락, TTL 없는 `set`, 동적 키 카디널리티 폭증 — 이 세 패턴이 거의 다 잡아낸다. 적중률·메모리·에러 메트릭은 처음부터 같이 깔아두는 게 좋다. 캐시는 보이지 않으면 곧 거짓말을 시작한다.

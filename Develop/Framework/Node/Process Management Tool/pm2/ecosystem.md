...
### PM2 에코시스템 파일이란?
PM2 에코시스템 파일은 JavaScript 나 JSON 으로 쓰는 설정 파일이고, 파일명은 보통 `ecosystem.config.js` 를 쓴다. 여러 애플리케이션 설정을 여기 한꺼번에 적어두고 PM2 옵션도 이 파일에서 관리한다.

### 에코시스템 파일의 필요성
- **중앙 집중식 설정 관리**: 모든 PM2 설정을 하나의 파일에서 관리
- **환경별 설정 분리**: 개발, 스테이징, 프로덕션을 각각 따로
- **복잡한 애플리케이션 관리**: 여러 애플리케이션을 동시에 관리
- **배포 자동화**: CI/CD 파이프라인에서 설정 파일 활용

### 주요 기능
- **클러스터 모드 활성화**: 멀티코어에서 성능 최적화
- **자동 재시작 및 메모리 관리**: 안정적인 애플리케이션 운영
- **환경 변수 설정**: 환경별 설정 분리
- **여러 애플리케이션 관리**: 복잡한 시스템 구성
- **로그 관리 및 파일 감시**: 개발과 운영 편의성
...
### 결론
PM2 에코시스템 파일 하나면 Node.js 애플리케이션 설정을 한곳에 모아 관리한다.
환경별 설정 분리, 다중 애플리케이션 관리, 성능 최적화까지 여기서 다룬다.
설정을 제대로 잡아두면 그만큼 운영이 안정되고 확장도 쉬워진다.
무중단 배포, 로그 관리, 모니터링처럼 프로덕션에 필요한 것들도 이 파일에서 켠다.
CI/CD 파이프라인에 붙이면 배포 과정까지 자동화된다.

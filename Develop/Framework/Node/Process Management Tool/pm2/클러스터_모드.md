...
PM2 전반은 [PM2 Node.js](pm2.md)를 참고한다. 이 문서는 클러스터 모드만 깊게 다룬다.
### PM2 클러스터 모드란?

PM2 클러스터 모드는 애플리케이션 하나를 여러 인스턴스로 띄워서 멀티코어 CPU를 쓰는 방식이다. Node.js는 기본이 싱글 스레드라 프로세스 하나가 CPU 코어 하나밖에 못 쓴다. 클러스터 모드로 돌리면 코어를 전부 쓰니까 처리량이 크게 올라간다.

**핵심 특징:**
- **로드 밸런싱**: 요청을 각 인스턴스에 고르게 분배
- **CPU 최대 활용**: Node.js 싱글 스레드 한계 극복
- **자동 장애 복구**: 프로세스 중단 시 자동 재시작
- **간편한 관리**: 설정 파일로 하는 중앙 집중식 관리

### 왜 클러스터 모드가 필요한가?

Node.js는 이벤트 루프 기반 비동기 처리라 동시성 자체는 높다. 그런데 CPU를 많이 쓰는 작업으로 가면 코어 하나라는 한계가 그대로 드러난다. 클러스터 모드를 쓰면 이렇게 달라진다.
...
인스턴스 4개를 실행한다. CPU 코어 수보다 많이 지정해도 되긴 하는데, 보통은 코어 수와 같거나 그보다 적게 잡는 쪽이 효율이 좋다.
...
`0`을 주면 CPU 코어 수만큼 알아서 인스턴스가 뜬다. `max`와 같은 효과다.
...
설정 파일을 쓰면 클러스터 모드를 더 체계적으로 관리한다.
...
PM2 클러스터 모드는 Node.js 내장 `cluster` 모듈 위에서 돈다.

1. **마스터 프로세스**: PM2가 생성하는 메인 프로세스로, 워커 프로세스들을 관리한다.
2. **워커 프로세스**: 실제 애플리케이션 코드를 실행하는 프로세스다.

### 로드 밸런싱 방식

PM2는 요청을 라운드 로빈(Round Robin)으로 나눈다.
...
인스턴스마다 메모리 공간이 따로 잡히니까 전체 메모리 사용량은 이렇게 계산된다.

```
전체 메모리 사용량 = 인스턴스 수 × 단일 인스턴스 메모리
```

인스턴스 4개가 각각 200MB를 쓰면 총 800MB가 필요하다. 서버 메모리 용량을 보고 인스턴스 수를 조정해야 한다.

### 세션 관리

클러스터 모드에서는 워커 프로세스마다 메모리 공간이 따로 놀기 때문에, 메모리 기반 세션 스토리지를 쓰면 문제가 생긴다. 같은 사용자의 요청이 매번 다른 워커로 라우팅될 수 있어서다.
...
파일 업로드도 마찬가지다. 워커 프로세스마다 파일 시스템이 따로라서 임시 파일 관리에 주의해야 한다.
...
워커 프로세스끼리 상태를 공유해야 한다면 메모리 기반 저장소는 못 쓴다.
...
인스턴스 수는 대체로 CPU 코어 수에 맞추는 쪽이 효율이 좋다. 코어 수보다 많이 띄우면 컨텍스트 스위칭 오버헤드가 붙는다.
...
클러스터 모드에서는 워커 프로세스 전부가 같은 포트를 공유한다. TCP 연결을 각 워커에 나눠 주는 건 운영체제 몫이다. 그래서 애플리케이션 코드에서 포트를 직접 바인딩하면 안 된다. 그건 마스터 프로세스가 처리한다.

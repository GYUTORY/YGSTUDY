...
PM2(Process Manager 2)는 Node.js 애플리케이션을 위한 프로덕션 프로세스 관리 도구다. 애플리케이션의 가용성과 성능, 안정성을 높이는 기능을 여럿 제공한다.
...
시스템 부팅 시 PM2가 자동으로 시작되도록 등록해 둘 수 있다.
...
PM2는 클러스터 모드로 멀티코어 환경에서 애플리케이션 성능을 극대화한다.
동작 원리와 운영 시 주의사항은 [PM2 Cluster Mode](클러스터_모드.md)에서 자세히 다룬다.
...
1. **순차적 재시작**: PM2는 한 번에 하나씩 워커 프로세스를 재시작한다.
2. **로드 밸런싱**: 활성 상태인 다른 워커가 요청을 처리한다.
3. **헬스 체크**: 새로운 워커가 정상적으로 시작되면 다음 워커를 재시작한다.
4. **완료**: 모든 워커가 새로운 코드로 업데이트된다.

---

## 무중단 배포

PM2는 `reload` 명령으로 애플리케이션을 중단 없이 재배포한다.
...
- **reload**: 무중단 재시작. 워커 프로세스를 순차적으로 재시작해서 서비스 중단 없이 업데이트한다.
- **restart**: 일반 재시작. 모든 프로세스를 동시에 재시작하니 일시적으로 서비스가 끊길 수 있다.

---

## 로그 관리

PM2는 로그 파일을 자동으로 만들고 관리한다.
...
애플리케이션 안에서 로깅 라이브러리를 써서 로그를 관리할 수도 있다.
...
PM2 Plus를 쓰면 웹 기반 대시보드에서 애플리케이션을 모니터링할 수 있다.
...
PM2 모니터링에서 확인할 수 있는 정보는 다음과 같다:
- CPU 사용률
- 메모리 사용량
- 프로세스 상태
- 실시간 로그
...
설정 파일을 쓰면 애플리케이션을 더 체계적으로 관리할 수 있다.
...
애플리케이션에 헬스 체크 엔드포인트를 하나 추가해 두면 상태를 확인할 수 있다.
...
프로덕션 환경에서는 다음을 고려해야 한다:
- 환경 변수로 민감한 정보 관리
- 로그 파일 권한 설정
- PM2 Plus로 원격 모니터링할 때의 보안 키 관리
- 방화벽 설정
...
- **pm2-runtime**: Docker 컨테이너 안에서 PM2를 포그라운드 프로세스로 실행한다. 컨테이너가 종료되면 PM2도 함께 종료된다.
- **pm2 start**: 백그라운드 데몬으로 실행되니 Docker 컨테이너와는 별도로 동작한다.

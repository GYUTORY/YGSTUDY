...
#### `3`은 재시도 3회가 아니라 **총 시도 3회**다

위 주석의 "최대 3회 재시도"는 정확하지 않다. Feign 과 Resilience4j 모두 이 숫자에 **최초 호출을 포함**한다. 실제로 실패하는 호출을 돌려 세어보면:

```java
Retryer r = new Retryer.Default(100, 1000, 3);
// 실패할 때마다 r.continueOrPropagate(...) 를 부르며 호출 횟수를 센다

Retry retry = Retry.of("t", RetryConfig.custom().maxAttempts(3).build());
```

```
=== Feign Retryer.Default(100, 1000, 3) ===
총 호출 횟수: 3  → 최초 1회 + 재시도 2회

=== Resilience4j maxAttempts=3 ===
총 호출 횟수: 3  → 최초 1회 + 재시도 2회
```

(feign-core 13.5 / resilience4j-retry 1.7.1)

한 번 차이라 사소해 보이지만, **상대 서비스에 가해지는 부하를 계산할 때 어긋난다.** "3회 재시도"로 읽고 최악 4배를 예상했는데 실제는 3배이거나, 반대로 재시도 없이 1회만 갈 줄 알고 `max-attempts: 1` 을 줬다가 재시도가 아예 안 도는 식이다.

더 중요한 건 **재시도가 곱해진다**는 점이다. 게이트웨이 → 주문 → 사용자 3단 호출에서 각 단계가 총 3회씩 시도하면, 맨 아래 사용자 서비스는 최대 3×3=9번 두들겨 맞는다. 사용자 서비스가 느려져서 시작된 장애라면 재시도가 그 장애를 키운다.

그래서 재시도는 **한 계층에서만** 건다. 보통 가장 바깥(진입점)이 아니라 **가장 안쪽 호출 지점**에 두고, 위 계층은 그 결과를 그대로 전달한다. 아래 설정에서 "Feign Retryer 와 Resilience4j retry 중 하나만 쓴다"고 적어둔 것도 같은 이유인데, 계층 간 중복까지 함께 봐야 한다.
...
#### 하필 **결제 API** 를 예시로 든 것이 이 설정의 가장 큰 문제다

`retry-exceptions` 에 `SocketTimeoutException` 이 들어 있다. 그런데 타임아웃은 **"요청이 상대에게 안 갔다"는 뜻이 아니다.** 결제가 이미 승인됐는데 응답만 못 받았을 수도 있다. 그 상태에서 재시도하면 같은 금액이 두 번 청구된다. `IOException` 도 똑같다 — 응답을 읽는 중에 연결이 끊긴 경우가 여기 들어간다.

**돈이 오가는 호출에 자동 재시도를 걸려면 중복 방지 키가 먼저 있어야 한다.** 요청마다 고유 키를 만들어 보내고, 결제사가 같은 키의 두 번째 요청을 첫 번째 결과로 응답하게 만든다. 그게 없다면 재시도 대신 **조회 API 로 결과를 확인**하는 흐름이 맞다.

```java
// 재시도가 안전해지는 최소 조건
paymentClient.charge(request, idempotencyKey);   // 같은 키 = 같은 결제 1건
```

이건 Resilience4j 설정으로 풀 수 있는 문제가 아니라 **결제사가 지원해야 하는 기능**이다. 지원 여부를 확인하기 전에 `retry` 를 켜면 안 된다.

#### 지수 백오프 주석의 `1s → 2s → 4s` 는 실제와 다르다

`max-attempts: 3` 이면 시도가 3번이므로 **대기는 2번**뿐이다. 실제 호출 간격을 재보면:

```
호출 시각 간격(밀리초): [1, 1014, 2004]
실제 대기 횟수: 2
```

(resilience4j-retry 1.7.1)

`1s → 2s` 에서 끝난다. `4s` 는 오지 않는다. 최악 지연을 계산할 때 이 한 칸이 어긋나면 상위 타임아웃 설계가 틀어진다.

그리고 이 조합에서 **타임아웃 예산이 서로 맞물리는지** 확인해야 한다. 재시도 총 대기가 3초인데 Feign `read-timeout` 이 5초라면, 최악의 경우 한 요청이 5×3 + 3 = 18초를 붙잡는다. 그동안 `bulkhead.max-concurrent-calls: 10` 슬롯 하나가 점유된다. **바깥쪽 타임아웃(게이트웨이·클라이언트)이 그보다 짧으면 안쪽 재시도는 아무도 안 기다리는 헛일**이 된다. 안쪽 총 예산 < 바깥쪽 타임아웃이 되도록 숫자를 맞춘다.

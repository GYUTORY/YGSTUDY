...
### 2.3 Prompt Injection 공격 흐름

직접 Injection과 간접 Injection의 공격 경로를 비교하면 다음과 같다.

```mermaid
flowchart LR
    subgraph direct["직접 Prompt Injection"]
        A1[공격자] -->|악의적 입력| B1[채팅 입력 필드]
        B1 --> C1[시스템 프롬프트 + 사용자 입력]
        C1 --> D1[LLM]
        D1 -->|시스템 프롬프트 노출<br/>의도하지 않은 동작| E1[유출된 응답]
    end

    subgraph indirect["간접 Prompt Injection"]
        A2[공격자] -->|악성 지시 삽입| B2[외부 문서 / 웹페이지]
        U2[일반 사용자] -->|정상 질문| C2[LLM 서비스]
        C2 -->|문서 검색| B2
        B2 -->|오염된 컨텍스트| C2
        C2 --> D2[LLM]
        D2 -->|조작된 응답| E2[피해자가 받는 결과]
    end

    style direct fill:#fff5f5,stroke:#e53e3e
    style indirect fill:#fff5f5,stroke:#e53e3e
```

직접 공격은 공격자 본인이 결과를 받지만, 간접 공격은 공격자와 피해자가 다르다. 간접 공격이 더 위험한 이유가 여기에 있다. 공격자는 문서에 악성 지시를 심어두고 기다리기만 하면 된다.
...
사용자 입력이 LLM에 도달하기까지, 그리고 LLM 응답이 사용자에게 전달되기까지 거치는 필터링 단계를 정리하면 다음과 같다.

```mermaid
flowchart TB
    subgraph input_pipeline["입력 필터링 파이프라인"]
        direction TB
        UI[사용자 입력] --> V1{빈 입력 / 길이 초과?}
        V1 -->|차단| BLOCK1[요청 거부]
        V1 -->|통과| V2{Injection 패턴 탐지}
        V2 -->|차단| BLOCK2[요청 거부]
        V2 -->|통과| V3{Jailbreak 패턴 탐지}
        V3 -->|차단| BLOCK3[요청 거부]
        V3 -->|통과| V4[PII 마스킹]
        V4 --> CLEAN[정제된 입력]
    end
    ...
    subgraph output_pipeline["출력 필터링 파이프라인"]
        direction TB
        LLM --> O1{시스템 프롬프트 유출?}
        O1 -->|차단| BLOCK4[응답 대체]
        O1 -->|통과| O2{위험 콘텐츠 포함?}
    ...
    style input_pipeline fill:#f0fdf4,stroke:#16a34a
    style output_pipeline fill:#eff6ff,stroke:#2563eb
    style BLOCK1 fill:#fef2f2,stroke:#dc2626
    style BLOCK2 fill:#fef2f2,stroke:#dc2626
    style BLOCK3 fill:#fef2f2,stroke:#dc2626
    style BLOCK4 fill:#fef2f2,stroke:#dc2626
    style BLOCK5 fill:#fef2f2,stroke:#dc2626
```

각 단계에서 차단된 요청은 바로 거부 응답을 반환한다. PII 마스킹은 차단이 아닌 수정이라서 파이프라인을 멈추지 않고 계속 진행한다. 입력 쪽에서 5단계, 출력 쪽에서 3단계를 거치는 구조다.
...
LLM 서비스의 보안은 단일 방어선으로 해결되지 않는다. 네트워크, 애플리케이션, LLM 각 계층에서 방어를 겹겹이 쌓아야 한다.
...
flowchart TB
    ...
    subgraph APP["애플리케이션 계층"]
        INPUT_GUARD[입력 필터링<br/>Injection · Jailbreak 탐지]
        SESSION[세션 격리]
        PERMISSION[도구 호출 권한 제어]
        INPUT_GUARD --> SESSION
        SESSION --> PERMISSION
    end

    PERMISSION --> LLM_LAYER

    subgraph LLM_LAYER["LLM 계층"]
        SYSTEM_PROMPT[시스템 프롬프트 방어<br/>role 분리 · 지시 강화]
        GUARDRAILS[Guardrails<br/>NeMo · LLM Guard 등]
        SYSTEM_PROMPT --> GUARDRAILS
    end

    GUARDRAILS --> LLM[LLM 모델]
    LLM --> OUTPUT

    subgraph OUTPUT["출력 계층"]
        LEAK_CHECK[프롬프트 유출 탐지]
        CONTENT_CHECK[위험 콘텐츠 필터]
        PII_MASK[PII 마스킹]
        LEAK_CHECK --> CONTENT_CHECK
        CONTENT_CHECK --> PII_MASK
    end

    PII_MASK --> LOG

    subgraph LOG["모니터링 계층"]
        SAFE_LOG[마스킹된 로그 저장]
        ALERT[이상 패턴 알림]
        SAFE_LOG --> ALERT
    end

    ALERT --> RESPONSE[안전한 응답 반환]
    RESPONSE --> CLIENT

    style NET fill:#fef3c7,stroke:#d97706
    style APP fill:#dbeafe,stroke:#2563eb
    style LLM_LAYER fill:#f3e8ff,stroke:#9333ea
    style OUTPUT fill:#dcfce7,stroke:#16a34a
    style LOG fill:#f1f5f9,stroke:#64748b
```

각 계층은 독립적으로 동작한다. 한 계층이 뚫려도 다음 계층에서 잡는 구조다. 실무에서 흔히 빠뜨리는 부분이 모니터링 계층인데, 공격 시도 자체를 로그로 남기고 알림을 걸어야 패턴을 파악하고 방어를 개선할 수 있다.

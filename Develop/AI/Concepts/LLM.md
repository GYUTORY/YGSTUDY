...
<figure markdown="1">
  ![Decoder-Only Transformer 아키텍처 구조도](images/transformer_architecture.svg){ width="720" }
...
<figure markdown="1">
  ![Self-Attention 연산 흐름도](images/self_attention.svg){ width="780" }
...
<figure markdown="1">
  ![BPE 토크나이저 인코딩/디코딩 과정](images/bpe_tokenizer.svg){ width="780" }
...
<figure markdown="1">
  ![Temperature와 Top-p에 따른 확률 분포 비교](images/temperature_top_p.svg){ width="780" }
...
위 시퀀스에서 핵심은 LLM이 직접 함수를 실행하지 않는다는 점이다. LLM은 "이 함수를 이 인자로 호출하라"는 JSON을 생성하고, 실제 실행과 결과 전달은 백엔드가 담당한다.
...
graph TD
    ...
    style VL fill:#fef3c7,stroke:#d97706
```
...
<figure markdown="1">
  ![LLM 학습 파이프라인: Pre-training → SFT → RLHF/DPO](images/llm_training_pipeline.svg){ width="780" }
...
<figure markdown="1">
  ![임베딩 벡터 공간 시각화](images/embedding_vector_space.svg){ width="780" }
...
graph TD
    ...
    style T5 fill:#fecaca,stroke:#dc2626
```
...
graph LR
    ...
    style COST1 fill:#fecaca,stroke:#dc2626
```

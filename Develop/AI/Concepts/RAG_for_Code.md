...
📌 **권장**: 코드는 **함수/클래스 단위** 청킹이 가장 효과적이다. 전체 함수 시그니처, 주석, 본문이 함께 보존된다.
...
📌 **실무 권장**: 대부분의 경우 **RAG가 더 적합**하다. 코드베이스는 계속 변경되므로 실시간 검색이 재학습보다 효율적이다. 필요 시 RAG + Fine-tuning을 조합한다.

---

## 8. Agentic RAG (A-RAG)

2026년의 트렌드인 **Agentic RAG**는 단순 검색-생성을 넘어, AI 에이전트가 검색 전략을 **스스로 결정**한다.
...
이것이 바로 Claude Code, Cursor 등의 AI 코딩 도구가 내부적으로 수행하는 방식이다.

...
| TTL | 옵션 | 추가 비용 |
|---|---|---|
| 5분 | `ephemeral` (기본) | 캐시 쓰기 1.25× |
| 1시간 | `ephemeral` + 별도 플래그 (모델별 지원 여부 확인) | 캐시 쓰기 2× |
...
tools 정의를 동적으로 만들면 순서가 매 요청마다 달라질 수 있다. **set** 에서 변환하면 순서가 보장되지 않는다. dict 는 Python 3.7 부터 삽입 순서를 지키므로 dict 자체가 원인은 아니지만, 그 dict 를 채우는 순서가 요청마다 달라지면 결과도 같이 흔들린다.

...
인터넷에서 웹사이트에 접속할 때 우리는 보통 `google.com`이나 `naver.com` 같은 도메인 이름을 사용한다. 하지만 실제로 컴퓨터들이 서로 통신할 때는 `172.217.164.110` 같은 IP 주소를 사용해야 한다. DNS Lookup은 바로 이 도메인 이름을 IP 주소로 변환하는 과정이다.

**실무 팁:**
DNS Lookup은 웹 통신의 첫 번째 단계다. 캐싱을 통해 반복 요청 시 빠르게 처리된다.

### 왜 DNS Lookup이 필요한가?

**사람의 편의성**
- IP 주소는 숫자로만 이루어져 있어 기억하기 어렵다
- `google.com`이 `172.217.164.110`보다 훨씬 기억하기 쉽다

**유연한 서비스 운영**
- 서버를 다른 곳으로 이전해도 도메인 이름은 그대로 유지할 수 있다
- IP 주소가 바뀌어도 사용자는 같은 도메인으로 접속할 수 있다

**서비스 분산**
- 하나의 도메인에 여러 서버를 연결해 부하를 분산시킬 수 있다
- 웹 서버, 이메일 서버, FTP 서버를 각각 다른 곳에 두고 운영할 수 있다
...
**1단계: 로컬 캐시 확인**
- 브라우저나 운영체제가 이전에 조회한 결과를 기억하고 있는지 확인한다
- 캐시에 있으면 바로 그 결과를 사용한다

**2단계: DNS 리졸버에 질의**
- 캐시에 없으면 ISP(인터넷 서비스 제공업체)의 DNS 서버에 질의를 보낸다
- 이 서버가 클라이언트를 대신해서 전체 조회 과정을 수행한다

**3단계: 계층적 조회**
- 루트 서버에서 시작해서 TLD 서버, 권한 있는 서버 순서로 조회한다
- 각 단계에서 다음 단계의 서버 정보를 얻어간다

**4단계: 결과 반환**
- 최종적으로 찾은 IP 주소를 클라이언트에게 반환한다
- 이 결과는 캐시에 저장되어 다음 조회 시 빠르게 사용된다

**실무 팁:**
DNS 조회는 계층적으로 이루어진다. 루트 → TLD → 권한 있는 서버 순서로 조회한다. 각 단계에서 캐싱이 발생한다.

...
- [실제 계산 예시](#실제-계산-예시)
- [특수 주소의 이해](#특수-주소의-이해)
- [VLSM과 CIDR](#vlsm과-cidr)
- [서브넷 설계 시 고려사항](#서브넷-설계-시-고려사항)
- [실무에서의 서브넷 활용](#실무에서의-서브넷-활용)
- [서브넷의 장점](#서브넷의-장점)
...
### 네트워크 주소
네트워크 주소는 서브넷 내에서 호스트 비트가 모두 0인 주소입니다.
...
### 브로드캐스트 주소
브로드캐스트 주소는 서브넷 내에서 호스트 비트가 모두 1인 주소입니다.
...
전체 주소 공간에서 네트워크 주소와 브로드캐스트 주소를 제외한 나머지가 실제 호스트에 할당 가능합니다.
...
/30 서브넷은 사용 가능한 호스트가 2개뿐이므로 주로 라우터 간 연결에 사용됩니다.
...
**VLSM(Variable Length Subnet Mask)**은 하나의 네트워크를 서로 다른 크기의 서브넷으로 나누는 기법입니다.
...
AWS에서는 가용 영역(AZ)별로 서브넷을 분리하는 것이 일반적입니다.
...
- [서브넷 마스크 상세 설명](https://youngkyonyou.github.io/network/2022/02/05/Network-SubnetMask-02.html)

...
GSLB(Global Server Load Balancing)는 전 세계적으로 분산된 서버들의 부하를 분산시키고 가용성을 높이는 고급 DNS 서비스다. 단순한 DNS 서비스와 달리, 서버의 상태, 지리적 위치, 네트워크 상태 등 다양한 요소를 고려하여 최적의 서버를 선택한다.

**실무 팁:**
GSLB는 글로벌 서비스에서 필수다. 여러 지역에 서버를 배치한 경우 사용자에게 가장 가까운 서버로 연결한다.

## DNS의 한계점

DNS는 도메인 주소와 IP를 매핑하여 도메인으로 요청이 들어왔을 때 대상의 주소로 변환해주는 서비스다. 하나의 도메인 주소에 대해서 여러 개의 IP주소를 제공할 수 있어 가용성 구성과 로드 밸런싱 기능을 수행하기도 하지만, 근본적으로 다음 한계가 있다:
...
**문제점:**
DNS의 로드 밸런싱은 IP 목록 중 하나를 반환할 뿐, 네트워크 지연, 성능, 트래픽 유입, 서비스 실패 등은 전혀 고려하지 않는다.
...
**실무 팁:**
DNS만으로는 서버 장애 시 자동 전환이 어렵다. 고가용성이 필요한 서비스는 GSLB를 고려한다.
...
**실무 팁:**
GSLB는 Health Check를 통해 서버 상태를 모니터링한다. 장애 서버는 자동으로 제외된다.
...
**실무 팁:**
Least Connection은 연결 수가 많은 서비스에 적합하다. Response Time은 사용자 경험을 최적화할 때 사용한다.
...
**실무 팁:**
레이턴시 기반 라우팅은 실시간 서비스에 중요하다. 게임, 화상회의 등에서 사용자 경험을 크게 개선한다.
...
**실무 팁:**
위치 기반 라우팅은 GDPR, 데이터 주권 규정을 준수해야 할 때 필수다. 유럽 사용자는 유럽 서버로 연결한다.
...
**실무 팁:**
DNS 기반 GSLB는 구현이 간단하지만 TTL 캐싱으로 인한 지연이 있다. Anycast는 성능이 우수하지만 BGP 설정이 필요하다.
...
**실무 팁:**
GSLB는 비용이 높고 복잡하다. 단일 지역 서비스라면 일반 로드 밸런서로 충분하다. 여러 지역에 서버가 있을 때만 고려한다.
...
**실무 팁:**
AWS 환경에서는 Route 53을 사용한다. Health Check와 지연 시간 기반 라우팅을 지원한다. Cloudflare는 CDN과 통합되어 있어 웹 서비스에 적합하다.
...
### 2. Failover 라우팅 설정 (Active-Standby)

```json
// route53-failover.json — Primary(서울) / Secondary(도쿄) Failover
{
  "Changes": [
    {
      "Action": "UPSERT",
...
### 3. 지연 시간 기반 라우팅 (Latency-Based)

```json
// route53-latency.json — 사용자 위치에서 가장 가까운 리전으로 자동 라우팅
{
  "Changes": [
    {
      "Action": "UPSERT",
...
### 4. 지리적 라우팅 (Geolocation)

```json
// 한국 사용자 → 서울, 일본 사용자 → 도쿄, 그 외 → 버지니아
{
  "Changes": [
    {
      "Action": "UPSERT",
...
### 5. Terraform으로 Route 53 GSLB 구성

...
**MQTT (Message Queuing Telemetry Transport)**는 IoT(사물인터넷) 환경의 실시간 데이터 통신을 겨냥해 만든 **경량 메시지 전송 프로토콜**이다.
...
MQTT 5.0은 이전 버전의 한계를 걷어내고 요즘 IoT 환경이 요구하는 것들을 담으려고 기능을 대폭 늘렸다.

#### 1. **세션 만료 간격 (Session Expiry Interval)**

**문제점 (3.1.1):**
Persistent Session은 클라이언트가 영영 재연결하지 않아도 브로커에 남아 있어서 메모리를 낭비했다.
...
**문제점 (3.1.1):**
요청-응답 패턴을 구현하려면 복잡한 주제 명명 규칙과 상관관계 ID가 필요했다.

**해결 (5.0):**
응답 주제(Response Topic)와 상관관계 데이터(Correlation Data)를 표준으로 지원한다.
...
**문제점 (3.1.1):**
오래된 메시지가 영원히 큐에 남는 탓에 시간에 민감한 데이터가 무의미해지기도 했다.
...
**문제점 (3.1.1):**
같은 주제를 구독하는 클라이언트가 전부 같은 메시지를 받아서 중복 처리가 생겼다.

**해결 (5.0):**
메시지를 여러 구독자에게 나눠 보내 로드 밸런싱을 한다.
...
**문제점 (3.1.1):**
메타데이터를 페이로드에 욱여넣어야 했다. 그러다 보니 애플리케이션 로직과 뒤섞였다.

**해결 (5.0):**
키-값 쌍으로 원하는 메타데이터를 붙인다.
...
**문제점 (3.1.1):**
연결 실패나 구독 거부 시 이유를 파악하기 어려웠다.

**해결 (5.0):**
모든 응답 패킷이 상세한 이유 코드와 설명을 함께 싣는다.
...
**문제점 (3.1.1):**
클라이언트가 브로커의 제약사항을 몰라서 런타임 오류가 났다.

**해결 (5.0):**
브로커가 CONNACK에 자기 기능과 제한을 실어 알려준다.
...
**문제점 (3.1.1):**
브로커나 클라이언트가 과부하 상태여도 메시지가 계속 밀려들어서 리소스가 바닥날 수 있었다.

**해결 (5.0):**
수신 최대값(Receive Maximum)으로 동시에 처리할 QoS 1/2 메시지 수를 제한한다.
...
**문제점 (3.1.1):**
긴 주제 이름이 매번 그대로 실려 나가 대역폭을 낭비했다.

**해결 (5.0):**
주제 이름을 짧은 정수에 매핑해 대역폭을 아낀다.
...
**문제점 (3.1.1):**
여러 주제를 구독할 때 어떤 구독으로 메시지가 왔는지 알기 어려웠다.
...
Keep-Alive는 TCP 연결이 살아 있는지 확인하는 하트비트 메커니즘이다.
...
TCP는 데이터가 오가지 않으면 연결이 끊긴 걸 감지하지 못한다. Keep-Alive가 그 구멍을 메운다.
...
**주의:** Keep-Alive를 끄면 네트워크 장애를 감지하지 못한다.
...
발행/구독 패턴이 실제로 어떻게 도는지 좀 더 뜯어보면:
...
3. 각 구독자에게 메시지를 복사해 전달 (멀티캐스트 방식)
...
#### 📮 중앙 집중식 메시지 허브
브로커는 MQTT 시스템의 **심장**이다:

#### 주요 기능

**1. 메시지 라우팅:**
브로커는 발행된 메시지를 알맞은 구독자에게 넘긴다.
...
**2. 연결 관리:**
브로커는 수백, 수천 개의 클라이언트 연결을 효율적으로 건사한다.
...
MQTT는 TCP 위에서 돈다. 통신은 전부 패킷 단위다.
...
패킷이 이렇게 작으니 대역폭이 빠듯한 환경에서도 잘 돈다.

### 5. **세션 관리 (Session Management)**

세션은 클라이언트와 브로커 사이의 상태 정보를 붙잡아 두는 메커니즘이다.

#### Clean Session vs Persistent Session

**Clean Session (클린 세션) - 플래그 = 1:**

클린 세션은 말 그대로 "깨끗한 상태에서 시작"한다.
...
**Client ID의 중요성:**
- Persistent Session은 Client ID로 식별한다
- 같은 Client ID로 재연결하면 이전 세션이 살아난다
- Client ID가 다르면 완전히 새로운 세션이 생긴다

**메모리 관리:**
- Persistent Session은 브로커의 메모리/디스크를 쓴다
- 오랜 시간 연결하지 않는 클라이언트는 리소스를 낭비한다
...
Retained Message는 주제의 "마지막 알려진 좋은 값"을 저장해 두는 기능이다.

#### 💾 보유 메시지의 개념

보통 MQTT 메시지는 발행하는 그 순간 구독 중인 클라이언트에게만 간다. Retained Message는 브로커에 남아서 **나중에 구독하는 클라이언트도** 즉시 받는다.
...
**하나의 주제당 하나의 메시지:**
- 주제 하나에 Retained 메시지는 최대 1개다
- 새 Retained 메시지가 이전 것을 덮어쓴다

**브로커 재시작:**
- 브로커가 재시작될 때 Retained 메시지가 남는지는 브로커 설정에 따라 다르다
...
Last Will은 클라이언트 연결이 예고 없이 끊겼을 때 브로커가 대신 발행해 주는 메시지다.

#### 💀 유언 메시지의 개념

**비유:**
유언장과 같다. "내가 갑자기 사라지면, 이 메시지를 전해주세요"라고 미리 브로커에 등록해 둔다.
...
Last Will은 Keep-Alive 메커니즘과 딱 붙어 있다.
...
**개념:**
"Fire and Forget" 방식이다. 발행자가 메시지를 보내면 거기서 끝. 신뢰성은 TCP가 보장하는 수준까지만이다.
...
**개념:**
메시지가 적어도 한 번은 전달된다고 보장한다. 대신 네트워크 재전송 때문에 중복이 생긴다.
...
**개념:**
4단계 핸드셰이크로 메시지가 딱 한 번만 전달되도록 보장한다. 제일 안전한 대신 제일 복잡하고 느리다.
...
#### **하이브리드 접근**
요즘 시스템은 두 기술을 같이 쓰는 하이브리드가 잘 먹힌다. MQTT로 IoT 기기와 통신하고 Kafka로 대용량 데이터를 처리하는 구성이 점점 흔해진다.
...
- **예측 정비**: 데이터 분석으로 고장 예측
- **품질 관리**: 생산 과정의 품질 실시간 모니터링
- **에너지 최적화**: 전력 사용량 최적화

#### 작동 원리
1. **센서 네트워크**: 공장 내 모든 장비에 센서 설치
2. **데이터 수집**: MQTT로 실시간 데이터 전송
3. **분석 플랫폼**: 빅데이터 분석으로 인사이트 도출
...
MQTT는 단순한데 할 일은 다 하는 프로토콜이다. IoT 생태계가 커질수록 이 프로토콜이 맡는 역할도 같이 커진다.

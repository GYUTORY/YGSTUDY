...
DHCP(Dynamic Host Configuration Protocol)는 네트워크에 연결된 장치들에게 IP 주소 및 기타 네트워크 구성 정보를 자동으로 할당하는 네트워크 프로토콜이다. 네트워크 관리의 복잡성을 줄이고 IP 주소 관리를 효율화한다.

## DHCP의 정의와 목적

### 정의
- Dynamic Host Configuration Protocol의 약자
- RFC 2131에서 정의된 표준 프로토콜
...
**실무 팁:**
DHCP는 대부분의 네트워크에서 기본으로 사용한다. 서버나 프린터처럼 고정 IP가 필요한 장치는 예외 처리한다.
...
**실무 팁:**
DHCP는 IP 주소 관리를 자동화하여 관리 부담을 크게 줄인다. 대규모 네트워크에서는 필수다.
...
**실무 팁:**
DHCP 서버는 고가용성 구성을 권장한다. 보안이 중요한 네트워크에서는 DHCP 스누핑을 활성화한다.
...
**실무 팁:**
DHCP 스누핑은 악의적 DHCP 서버를 차단하는 데 필수다. 엔터프라이즈 네트워크에서는 반드시 활성화한다.
...
**실무 팁:**
클라우드 환경에서는 관리형 DHCP 서비스를 사용하는 것이 편리하다. AWS VPC, Azure Virtual Network 등에서 자동으로 제공한다.

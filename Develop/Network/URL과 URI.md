...
URL은 인터넷 상의 모든 자원을 고유하게 식별하고 위치를 지정하는 주소 체계다. 마치 우편 주소가 특정 건물의 정확한 위치를 알려주듯이, URL은 웹상의 특정 자원이 어디에 있는지 정확히 가리킨다.

### URL의 주요 원칙

URL은 자원의 위치를 통한 식별이라는 웹의 기본 원칙을 담고 있다. 웹의 창시자 팀 버너스-리가 설계한 이 체계는 다음과 같은 원칙을 따른다:

**실무 팁:**
URL은 웹 자원의 위치를 나타낸다. RESTful API 설계에서 URL 구조가 중요하다. 의미 있는 경로를 사용하여 가독성을 높인다.
...
스킴은 클라이언트가 서버와 어떤 방식으로 통신할지를 결정하는 요소다. 이는 자원에 접근하는 방법과 보안 수준을 결정한다.

**실무 팁:**
HTTPS는 HTTP의 보안 강화 버전이다. 민감한 데이터를 전송할 때는 반드시 HTTPS를 사용한다.
...
## 요약
URL과 URI는 웹의 기본 구성 요소다. 이들을 올바르게 이해하고 활용하는 것은 웹 개발의 기본이며, 더 나은 웹 서비스를 구축하는 데 필수적이다.

**실무 팁:**
URL은 자원의 위치를 나타내고, URI는 자원을 식별하는 더 넓은 개념이다. RESTful API 설계에서 URL 구조가 중요하다. 의미 있는 경로를 사용하고, 쿼리 파라미터는 필터링과 페이지네이션에 사용한다.
...
- HTTP 가이드 (David Gourley, Brian Totty)
- 웹의 원리 (김중태)
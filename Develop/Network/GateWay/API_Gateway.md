...
Gateway 없이 마이크로서비스를 운영하면 각 서비스가 인증 로직을 직접 구현해야 한다. 클라이언트는 서비스 주소를 직접 알아야 하고, 서비스 내부 주소가 바뀌면 클라이언트 코드도 함께 수정해야 한다. CORS, 로깅, 레이트 리미팅은 서비스마다 개별 구현된다.

Gateway를 두면 인증을 한 곳에서만 처리하고 클라이언트는 Gateway 주소 하나만 알면 된다. 서비스 내부 주소가 변경되어도 클라이언트에 영향이 없고, 횡단 관심사를 Gateway에서 중앙 관리한다.
...
레이트 리미팅과 인증/인가는 빠지면 안 된다. 레이트 리미팅 없이 운영하다가 DDoS를 맞으면 복구하기 전에 비용이 먼저 폭증한다. 인증은 Gateway에서 한 번만 처리하고 하위 서비스에는 X-User-Id 헤더로 전달한다.

헬스 체크와 서킷 브레이커는 세트다. 헬스 체크로 하위 서비스 상태를 파악하고, 서킷 브레이커로 장애가 다른 서비스로 퍼지는 것을 막는다. 로깅과 트레이싱은 장애 발생 시 원인을 추적하는 데 필수다. Correlation-ID를 요청마다 Gateway에서 붙이면 서비스 간 요청 흐름을 한눈에 볼 수 있다.

CORS는 초기 세팅 때 놓치는 경우가 많다. Gateway에서 일괄 설정하면 서비스마다 설정할 필요가 없다. 타임아웃은 서비스마다 다르게 설정한다. 기본값을 전체에 적용하면 응답이 빠른 서비스도 느린 서비스 기준으로 묶인다.

TLS 종료는 Gateway에서 처리하면 내부 서비스는 HTTP로만 통신하면 된다. 카나리 배포는 가중치 라우팅으로 일부 트래픽만 신규 버전으로 보내면서 점진적으로 전환한다.

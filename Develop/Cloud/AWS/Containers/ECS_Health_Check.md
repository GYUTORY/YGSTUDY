...
tags: [aws, monitoring, observability, terraform]
updated: 2026-08-05
---
...
배포하자마자 에러가 나면 대개 네트워크부터 뒤진다. 그게 가장 흔한 시간 낭비다. **502 와 503 은 원인이 다르고, 확인해야 할 순서가 정해져 있다.**

#### 502 와 503 은 다른 말이다

| | 502 Bad Gateway | 503 Service Unavailable |
|---|---|---|
| ALB 가 겪은 일 | 타겟에 **연결은 됐는데** 유효한 응답을 못 받았다 | 응답을 보낼 **타겟이 아예 없다** |
| 대개의 원인 | 앱이 중간에 죽음, 커넥션 조기 종료, 응답 형식 이상 | 등록된 타겟이 전부 unhealthy 이거나 0개 |
| 먼저 볼 곳 | 앱 로그 | 헬스체크 |

혼동하기 쉬운데 방향이 반대다. **502 는 "앱이 뭔가 응답하려다 실패" 이고, 503 은 "보낼 데가 없음" 이다.**

배포 직후 503 이 잠깐 났다가 사라지는 건 정상이다 — 새 태스크가 헬스체크를 통과하기 전 구간이다. 문제는 **503 이 지속되는 경우**로, 이건 헬스체크가 전부 실패하고 있다는 뜻이라 앱 로그가 아니라 헬스체크 설정부터 봐야 한다.

#### 확인 순서 — 위에서부터

배포 직후 502 의 대부분은 **앱이 뜨지 못한 것**이다. 그래서 순서가 이렇다.

**1) ECS 이벤트와 stoppedReason**

태스크가 아예 안 떴는지부터 본다. 이게 답이면 나머지를 볼 필요가 없다.
...
- `CannotPullContainerError` — 이미지 태그 오타, ECR 권한 없음
- `ResourceInitializationError` — Secrets Manager 접근 실패, 태스크 실행 역할 문제
- `Essential container in task exited` + `exitCode: 1` — 앱이 부팅 중 죽음. 다음 단계로
- `Task failed container health checks` — 앱은 떴는데 헬스체크 실패. 4번으로

**2) 앱 로그에 "부팅 완료" 가 찍혔는가**

`exitCode: 1` 이면 앱 로그를 본다. 여기서 갈린다.

```bash
aws logs tail /ecs/my-service --since 10m --follow
```

- 스택 트레이스가 있다 → 환경변수 누락, DB 접속 실패 등. 원인이 로그에 그대로 있다
- **"Listening on ..." 이 찍혔는데도 502** → 3번으로. 이게 가장 헷갈리는 경우다

**3) 0.0.0.0 에 바인딩했는가**

앱이 `127.0.0.1` 로 열면 **컨테이너 안에서는 되는데 밖에서는 안 닿는다.** 실제로 두 서버를 띄워 확인하면 이렇다.

```
루프백 서버 ← 루프백으로 접속     → 200
루프백 서버 ← 외부 IP 로 접속      → ECONNREFUSED     ← ALB 가 겪는 상황
전체바인딩 ← 루프백으로 접속       → 200
전체바인딩 ← 외부 IP 로 접속       → 200
```

이 사고가 어려운 건 **컨테이너에 들어가서 `curl localhost:8080/health` 를 하면 200 이 나오기 때문**이다. 앱은 멀쩡해 보이는데 ALB 만 못 닿는다. 그래서 네트워크·보안그룹을 의심하게 된다.
...
app.listen(8080);                 // 기본값이 0.0.0.0 이다
```

프레임워크 기본값이 로컬 개발 편의를 위해 `localhost` 인 경우가 있다(Vite preview, 일부 Python 프레임워크). 로컬에서 잘 되던 게 컨테이너에서만 안 되면 여기를 본다.

확인은 컨테이너 안에서 리스닝 주소를 직접 읽는 게 확실하다.

```bash
# 0.0.0.0:8080 이어야 한다. 127.0.0.1:8080 이면 이게 원인이다
ss -tlnp | grep 8080
```

**4) 헬스체크 경로가 200 을 주는가**

여기서 고전적인 사고가 하나 있다. **전역 인증 가드가 헬스체크 경로까지 막는 것.** 앱은 정상이고 바인딩도 맞는데 `/health` 가 401 을 준다. ALB 는 200 만 성공으로 치므로 계속 unhealthy 다.

```typescript
// 전역 가드를 걸었다면 헬스체크는 명시적으로 열어야 한다
@Public()
@Get('health')
health() { return { status: 'ok' }; }
```

컨테이너 안에서 직접 쳐서 **상태 코드**를 확인한다. 본문이 아니라 코드다.
...
여기까지 다 맞는데 `Request timed out` 이면 그때 보안 그룹이다. Task SG 인바운드에 ALB SG 를 source 로 하는 규칙이 있어야 한다. (위 "ALB 헬스체크 경로/포트 불일치" 절 참고.)

**6) 타임아웃**

앱이 응답은 하는데 느려서 헬스체크 타임아웃에 걸리는 경우. `timeout` 보다 오래 걸리는 헬스 엔드포인트라면 그 안에서 DB 를 치고 있을 가능성이 크다. 헬스체크는 가볍게 유지한다 — 의존성 점검은 별도 경로(`/health/deep`)로 분리하고 ALB 는 가벼운 쪽을 친다.

#### 요약

```
502 → 앱이 못 떴다고 먼저 의심한다
  1. stoppedReason        (아예 안 떴나)
  2. 앱 로그 부팅 완료      (뜨다 죽었나)
  3. 0.0.0.0 바인딩        ← 컨테이너 안 curl 은 200 이라 헷갈린다
  4. 헬스체크 경로 200      ← 전역 인증 가드에 막히는 고전
  5. 보안 그룹
  6. 타임아웃

503 이 지속 → 타겟이 전부 unhealthy. 4번부터 본다
503 이 잠깐 → 배포 중 정상. deregistration delay 구간
```

1·2번이 배포 직후 502 의 대부분이고, 그 둘은 이 문서 앞쪽(startPeriod, 헬스체크 불일치)에 이미 적혀 있다. **네트워크부터 보기 시작하는 것이 가장 흔한 시간 낭비다.**

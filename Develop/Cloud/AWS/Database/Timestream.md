...
Timestream 은 유연 스키마라 새 Dimension·Measure 는 쓰기 시점에 자동으로 잡힌다. 앞으로 들어올 데이터에 컬럼을 더하는 건 그냥 된다. 문제는 소급이다 — 이미 쓴 레코드의 Dimension 구조는 바꿀 수 없어서, 과거 데이터까지 새 구조로 맞추려면 새 테이블에 다시 써야 한다.

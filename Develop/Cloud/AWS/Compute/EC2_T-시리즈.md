...
### CloudWatch 대시보드 4-패널 구성

운영 환경에서는 위 메트릭을 하나씩 보지 않고, 대시보드에 4개 패널을 한 화면에 놓고 본다. 아래는 t3.micro Standard 모드에서 오전 9시에 부하가 걸려서 오후 2시경 크레딧이 바닥나는 상황이다. 4개 메트릭이 시간축을 공유하므로 같은 시점에서 각 메트릭이 어떻게 움직이는지 비교할 수 있다.

```
시각    06:00     09:00     12:00     14:00     17:00     20:00
         │         │         │         │         │         │
         ▼         ▼         ▼         ▼         ▼         ▼
...
┌─ CPUSurplusCreditBalance (Standard 모드) ───────────────────────┐
│                                                                 │
│    0 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━  │
│      Standard 모드에서는 항상 0.                                  │
│      이 값이 0 이상이면 Unlimited 모드가 켜져 있다는 뜻.             │
└─────────────────────────────────────────────────────────────────┘
```

CloudWatch 콘솔에서 대시보드를 만들 때 이 4개 메트릭을 세로로 배치하고 시간축을 동기화(Linked timerange)하면 된다. **14:00 시점의 세로 선** 하나가 모든 패널을 관통하는 걸 보면 — 크레딧 0 도달, 소비량 급감, CPU가 baseline 고정 — 이 세 가지가 동시에 일어났다는 걸 한눈에 확인할 수 있다.

### Unlimited 모드 대시보드 패턴

같은 상황인데 Unlimited 모드면 패턴이 완전히 다르다. CPU 그래프는 제한 없이 유지되고, 대신 Surplus 크레딧이 쌓인다.
...
│    0 ━━━━━━━━━━━━━━╱                                            │
│                    ↑ 크레딧 0 이후부터 초과분 누적                   │
│                      이 기울기 × $0.05/vCPU-hour = 추가 비용       │
└─────────────────────────────────────────────────────────────────┘

┌─ 추정 추가 비용 (CloudWatch Custom Metric으로 구성 가능) ──────────┐
│                                                                 │
│ $3.0                                              ╱━━━━━━━━━━  │
│ $2.0                                    ╱━━━━━━━━╱              │
│ $1.0                          ╱━━━━━━━━╱                        │
│                     ╱━━━━━━━━╱                                  │
│ $0.0 ━━━━━━━━━━━━━━╱                                            │
│                    ↑ SurplusCredit × $0.05 / 60                  │
│                      이 기울기가 가파르면 M 시리즈 전환 검토          │
└─────────────────────────────────────────────────────────────────┘
```

Standard와 Unlimited의 핵심 차이가 대시보드에서 보인다. Standard는 CPUUtilization이 baseline으로 뚝 떨어지고 SurplusCredit은 0을 유지한다. Unlimited는 CPUUtilization이 유지되는 대신 SurplusCredit이 우상향한다. 운영 중에 이 두 패턴 중 어떤 쪽이 보이는지에 따라 현재 모드를 바로 판별할 수 있다.
...
### Auto Scaling 환경에서 크레딧 문제

Auto Scaling 그룹에서 T 시리즈를 쓰면 크레딧 문제가 복합적으로 발생한다. 아래는 트래픽 급증 시 스케일아웃이 일어나는 상황이다.
...
```
                     트래픽 급증
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│  Auto Scaling Group (T3 Unlimited)                              │
│                                                                 │
│  ┌──────────────┐   시간 경과                                    │
│  │ Instance A   │   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━   │
│  │ 크레딧: 200  │   크레딧 충분. 버스트 가능.                       │
│  └──────────────┘                                               │
│                                                                 │
│  ┌──────────────┐   스케일아웃으로 새로 투입                       │
│  │ Instance B   │   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━             │
│  │ 크레딧: 0    │   크레딧 0 시작. Unlimited이라 CPU 제한은 없지만  │
│  └──────────────┘   → 초과 과금 즉시 시작                         │
│                                                                 │
│  ┌──────────────┐   추가 스케일아웃                               │
│  │ Instance C   │   ━━━━━━━━━━━━━━━━━━━━━━━━                   │
│  │ 크레딧: 0    │   역시 크레딧 0 시작. 초과 과금.                  │
│  └──────────────┘                                               │
│                                                                 │
│         ┌─────────────────────────────────────────┐             │
│         │ 스케일인으로 B, C 종료                     │             │
...
│             100   ╲━━━━╱   트래픽 분산 후 회복                      │
│                                                                   │
│  Instance B (신규, 11:00 투입)                                     │
│  크레딧   0 ─────────━━━╱━━━━━╲                                    │
│                   ╱━━━╱        ╲  ← 12:30 스케일인으로 종료         │
│          초과 ╱━━╱               크레딧 소멸, 비용만 남음             │
│                                                                   │
│  Instance C (신규, 11:15 투입)                                     │
│  크레딧   0 ──────────────━━╱━╲                                    │
│                        ╱━━╱    ╲  ← 12:15 스케일인으로 종료         │
│                 초과 ╱━╱                                           │
│                                                                   │
│  시각    10:00    11:00    12:00    13:00    14:00                 │
└───────────────────────────────────────────────────────────────────┘
```

이 패턴이 반복되면 새 인스턴스가 뜰 때마다 크레딧 0에서 초과 과금이 시작되고, 스케일인으로 종료되면 그동안 쌓인 크레딧은 사라진다. 하루에 스케일아웃/인이 여러 번 발생하는 환경이면 초과 비용이 꽤 쌓인다. 이런 패턴이 보이면 Warm Pool을 쓰거나, 아예 M 시리즈로 바꾸는 게 낫다.
...
T 시리즈에 Unlimited 추가 비용이 붙으면 어느 시점에서 M 시리즈가 더 싸진다. 아래는 t3.large vs m6i.large 기준이다.

```
월 비용($)
     │
  90 │                                              ╱━━  t3.large + Unlimited
     │                                          ╱━━╱
  80 │                                      ╱━━╱
     │                                  ╱━━╱
  70 │─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ━━━━━━━━━━━━━━━━━━━━━  m6i.large ($70 고정)
     │                          ╱━━╱     ↑
  60 │━━━━━━━━━━━━━━━━━━━━━━━━━╱         여기서부터 M 시리즈가 더 싸다
     │  t3.large 기본 $60   ╱━━╱          Unlimited 추가분 $10/월 = 역전 시점
  50 │                  ╱━━╱
     │              ╱━━╱
  40 │          ╱━━╱
     │      ╱━━╱
  30 │━━━━━╱     ← t3.large 기본 비용 $60에서
     │              Unlimited 추가 비용이 선형으로 쌓임
  20 │
     │
  10 │
     │
   0 └──┬──────┬──────┬──────┬──────┬──────┬──────┬──→
        0%    20%    40%    60%    80%   100%        평균 CPU 사용률
             baseline                               (baseline 초과분에 대해
             (30%)                                   Unlimited 과금 발생)

      ◄─── 이 구간은 T 시리즈 ──►◄─── 이 구간은 M 시리즈가 유리 ──►
           크레딧 내에서 해결            상시 부하 = 고정 비용이 나음
```

baseline(30%) 이하에서는 T 시리즈가 압도적으로 싸다. baseline을 넘기기 시작하면 Unlimited 비용이 선형으로 증가하고, 어느 시점에서 M 시리즈 고정 비용과 만난다. 이 교차점이 전환 시점이다. 실제 워크로드의 평균 CPU 사용률을 CloudWatch에서 확인하고 이 그래프에 대입하면 지금 쓰는 인스턴스가 적절한지 판단할 수 있다.

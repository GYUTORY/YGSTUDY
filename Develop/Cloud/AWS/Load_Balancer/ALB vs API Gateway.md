...
또 ALB 는 Sticky Session(쿠키 기반)을 지원하지만 API Gateway 에는 이 개념 자체가 없다. 세션 고정은 타깃 그룹에 등록된 여러 타깃 중 하나로 사용자를 묶는 기능인데, API Gateway 는 메서드마다 통합 엔드포인트 하나로 요청을 넘기는 구조라 묶을 타깃 집합이 없다. 세션 어피니티에 의존하던 백엔드는 세션 저장소(Redis 등)로 옮겨야 한다. 다만 뒤에 VPC Link 로 ALB 를 두는 구성이라면 고정은 그 ALB 의 타깃 그룹에서 그대로 설정할 수 있다.

...
엔디언이나 2의 보수, `0.1 + 0.2` 같은 기초는 [엔디언과 2의 보수, IEEE 754 부동소수점](엔디언과%202의%20보수,%20IEEE%20754%20부동소수점.md) 문서에서 다뤘다. 여기서는 그 다음 단계 — 왜 그런 결과가 나오는지, 어떻게 통제하는지 — 를 본다.
...
<div class="yg-figure" markdown="0">
...
print(s1)  # 1.001
print(s2)  # 1.0010000000827404 (오차 누적)
```
...
# 주의: 요즘 Python 의 sum() 은 이 예제로 문제를 보여주지 못한다.
# 부동소수 합산에 보상 알고리즘이 들어가 있어서 순진한 루프보다 정확하다.
# 실측 — 같은 입력에 3.10 은 1.000100000008274, 3.13 은 1.0001 이 나온다.
naive = 0.0
for x in values:
    naive += x
print(naive)               # 1.0010000000827404 (흡수로 쌓인 오차)
print(sum(values))         # 1.001  — sum() 은 이미 보상한다

...
### 인덱스가 무엇인가?
인덱스는 데이터베이스에서 데이터를 빠르게 찾기 위한 별도의 자료구조다. 마치 도서관에서 책을 찾을 때 사용하는 카드 목록과 같은 역할을 한다.

실제 데이터는 테이블에 저장되어 있지만, 인덱스는 그 데이터의 위치 정보를 별도로 관리한다. 이렇게 분리된 구조 덕분에 전체 테이블을 뒤지지 않고도 원하는 데이터를 빠르게 찾을 수 있게 된다.

**실무 팁:**
인덱스는 데이터를 빠르게 찾기 위한 자료구조다. 인덱스가 없으면 전체 테이블을 스캔해야 한다.
...
**실무 팁:**
인덱스가 없으면 전체 테이블을 스캔해야 한다. 인덱스가 있으면 빠르게 데이터를 찾을 수 있다.

### 인덱스의 물리적 구조 이해
데이터베이스는 디스크에 페이지 단위로 데이터를 저장한다:

**실무 팁:**
데이터베이스는 페이지 단위로 데이터를 저장한다. 인덱스 페이지와 데이터 페이지가 분리되어 있다.
...
### B-Tree 인덱스
B-Tree 인덱스는 현재 대부분의 관계형 데이터베이스에서 기본으로 사용하는 인덱스 구조다.

**실무 팁:**
B-Tree 인덱스는 범위 검색에 효율적이다. MySQL InnoDB, PostgreSQL에서 기본으로 사용한다. 
...
### Hash 인덱스 - 정확한 매칭에 특화
해시 함수를 사용해서 데이터를 저장하는 방식이다.

**실무 팁:**
해시 인덱스는 등호 검색에만 효율적이다. 범위 검색은 불가능하다.
...
**중요한 오해 해결:**
많은 사람들이 인덱스가 해시 형태로 저장된다고 생각하지만, 실제로는 대부분의 관계형 데이터베이스에서 B-Tree 구조로 저장된다.
...
**결론:**
인덱스는 해시 형태가 아닌 B-Tree 구조로 저장되며, 이는 범위 검색, 정렬, 디스크 효율성 등의 이유 때문이다. Hash 인덱스는 특수한 경우에만 사용되는 예외적인 구조다.
...
무분별하게 인덱스를 생성하면 조회는 빨라질지언정, 데이터 변경 작업에서 심각한 성능 저하가 발생한다.

**실무 팁:**
인덱스는 조회 성능을 향상시키지만, INSERT, UPDATE, DELETE 성능을 저하시킨다. 필요한 인덱스만 생성한다.
...
**B-Tree의 정렬 특성:**
- 복합 인덱스는 사전식 정렬(Lexicographic Order)로 저장된다
- 첫 번째 컬럼(age)이 우선적으로 정렬되고, 그 다음 두 번째 컬럼(created_at), 마지막에 세 번째 컬럼(active)이 정렬된다

**실무 팁:**
복합 인덱스는 첫 번째 컬럼부터 순서대로 정렬된다. 첫 번째 컬럼만으로도 인덱스를 사용할 수 있다.
...
**핵심 답변:**
해시 인덱스에서는 A만으로 조회가 불가능하다. 이것이 해시 인덱스의 근본적인 한계점이다.
...
**핵심 답변:**
해시 인덱스에서는 A, B, C를 통째로 저장하고, A만으로는 조회가 불가능하다. 이것이 해시 인덱스의 근본적인 한계점이며, 그래서 대부분의 관계형 데이터베이스에서 B-Tree 인덱스를 기본으로 사용하는 이유다.
...
2. hash("25,2023-01-02,false") 계산 → 해시값2 (다름!)
...
인덱스는 데이터베이스 성능 최적화의 주요 도구다. 적절한 인덱스 설계와 관리를 통해 검색 성능을 향상시킬 수 있지만, 과도한 인덱스는 오히려 성능을 저하시킬 수 있다.

실제 애플리케이션의 쿼리 패턴을 분석하고, 선택도가 높은 컬럼을 우선적으로 인덱싱하며, 정기적인 모니터링을 통해 인덱스의 효율성을 지속적으로 관리하는 것이 중요하다.

**실무 팁:**
인덱스는 쿼리 패턴에 따라 설계한다. 자주 사용되는 컬럼에 인덱스를 생성하고, 정기적으로 인덱스 사용률을 모니터링한다.

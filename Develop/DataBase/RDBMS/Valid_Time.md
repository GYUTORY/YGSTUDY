...
```
[2024-01-01, 2024-04-01) price=19900
[2024-04-01, 2024-07-01) price=19900   -- 인접, 같은 값
[2024-07-01, 9999-12-31) price=19900   -- 인접, 같은 값
```

세 레코드를 `[2024-01-01, 9999-12-31) price=19900` 하나로 합칠 수 있다.

**끝값을 하루 앞으로 적으면 안 된다.** 이 문서가 쓰는 구간은 반열림
(`valid_to > :date`)이라 `valid_to` 가 곧 다음 구간의 시작이다. `03-31`/`04-01`
처럼 하루 벌려 적으면 두 구간 사이에 3월 31일이 어디에도 속하지 않는 틈이
생기고, 아래 병합 쿼리의 그룹 판정(`prev_to < valid_from`)이 매 행을 새 그룹으로
본다. 실제로 돌려 보면 합쳐지지 않는다.

```
-- 끝값을 하루 앞으로 적었을 때
1|19900|2024-01-01|2024-03-31
1|19900|2024-04-01|2024-06-30      ← 3행 그대로
1|19900|2024-07-01|9999-12-31

-- 반열림으로 적었을 때
1|19900|2024-01-01|9999-12-31      ← 1행으로 합쳐짐
```

---
title: 데이터베이스 성능 튜닝
tags: [database, performance-tuning, query-optimization, indexing, connection-pool, benchmarking]
updated: 2025-11-26
---

# 데이터베이스 성능 튜닝

## 📋 목차
1. [성능 튜닝 개요](#성능-튜닝-개요)
2. [쿼리 최적화 및 실행 계획 분석](#쿼리-최적화-및-실행-계획-분석)
3. [인덱스 설계 및 최적화 전략](#인덱스-설계-및-최적화-전략)
4. [데이터베이스 파티셔닝 및 샤딩](#데이터베이스-파티셔닝-및-샤딩)
5. [연결 풀 관리 및 최적화](#연결-풀-관리-및-최적화)
6. [실제 성능 튜닝 사례 및 벤치마킹](#실제-성능-튜닝-사례-및-벤치마킹)
7. [참고 자료](#참고-자료)

### 📌 통합된 기존 파일들
이 문서는 다음 기존 파일들의 내용을 통합한 것입니다:
- **RDBMS에서의 index**: 인덱스 종류, 설계 원칙, 모니터링 및 최적화
- **옵티마이저**: 쿼리 분석, 실행 계획 생성, 비용 기반 최적화
- **DB Proxy**: 연결 풀링, 자동 재연결, 성능 최적화
- **메시지 큐 및 분산 락**: 트랜잭션 최적화, 락 성능 최적화
- **데이터베이스 통합 테스트**: 성능 고려사항, 모니터링

---

## 성능 튜닝 개요

### 데이터베이스 성능 튜닝이란?

데이터베이스 성능 튜닝은 데이터베이스 시스템이 더 빠르고 효율적으로 동작하도록 최적화하는 과정입니다. 단순히 쿼리 속도를 높이는 것을 넘어, 전체 시스템의 처리량, 응답 시간, 리소스 사용률을 개선하는 것이 목표입니다.

#### 성능 튜닝이 필요한 이유

**1. 사용자 경험 향상**
```
느린 데이터베이스 → 느린 애플리케이션 → 나쁜 사용자 경험

예시:
- 상품 검색이 5초 걸림 → 사용자 이탈
- 결제 처리가 10초 걸림 → 거래 실패
- 대시보드 로딩이 30초 걸림 → 업무 비효율
```

**2. 비용 절감**
```
성능 최적화로 동일한 하드웨어에서 더 많은 요청 처리 가능

최적화 전: 서버 10대로 1,000 RPS 처리
최적화 후: 서버 5대로 1,000 RPS 처리 → 50% 비용 절감
```

**3. 확장성 확보**
```
성능이 좋은 시스템은 확장이 쉽습니다

비효율적 시스템: 트래픽 2배 증가 → 서버 5배 필요
효율적 시스템: 트래픽 2배 증가 → 서버 2배 필요
```

**4. 비즈니스 연속성**
```
느린 데이터베이스는 전체 시스템의 병목이 됩니다

데이터베이스 병목 → 애플리케이션 대기 → 타임아웃 → 서비스 장애
```

### 성능 문제의 일반적인 원인

#### 1. 비효율적인 쿼리

**문제 상황:**
```sql
-- ❌ 전체 테이블 스캔
SELECT * FROM orders WHERE YEAR(created_at) = 2024;
-- 1억 건 테이블에서 10초 소요

-- ✅ 인덱스 활용
SELECT * FROM orders WHERE created_at >= '2024-01-01' AND created_at < '2025-01-01';
-- 같은 테이블에서 0.1초 소요
```

**왜 문제인가?**
- 함수를 컬럼에 적용하면 인덱스를 사용할 수 없습니다
- 전체 테이블을 읽어야 하므로 디스크 I/O가 급증합니다
- 데이터가 증가하면 성능이 선형적으로 악화됩니다

#### 2. 잘못된 인덱스 설계

**문제 상황:**
```sql
-- 테이블: users (1천만 건)
-- 쿼리: SELECT * FROM users WHERE status = 'active' AND city = 'Seoul';

-- ❌ 인덱스 없음 → 전체 스캔 (10초)
-- ❌ INDEX(status) → 선택도 낮음 (5초)
-- ✅ INDEX(city, status) → 복합 인덱스 (0.05초)
```

**왜 문제인가?**
- 선택도가 낮은 컬럼(status)에만 인덱스를 만들면 효과가 적습니다
- 인덱스가 없으면 매번 전체 데이터를 읽어야 합니다
- 잘못된 순서의 복합 인덱스는 일부 조건에만 사용됩니다

#### 3. 과도한 데이터 조회

**문제 상황:**
```sql
-- ❌ 필요 없는 데이터까지 조회
SELECT * FROM users WHERE id = 123;
-- 50개 컬럼 모두 조회

-- ✅ 필요한 컬럼만 조회
SELECT id, name, email FROM users WHERE id = 123;
-- 3개 컬럼만 조회 → 메모리 사용량 90% 감소
```

**왜 문제인가?**
- 불필요한 데이터 전송으로 네트워크 대역폭 낭비
- 메모리 사용량 증가로 캐시 효율 저하
- 직렬화/역직렬화 오버헤드 증가

#### 4. 연결 관리 문제

**문제 상황:**
```
동시 접속 1,000명
각 요청마다 새 DB 연결 생성/종료

연결 생성 시간: 50ms
쿼리 실행 시간: 10ms
연결 종료 시간: 10ms

실제 DB 작업: 10ms (14%)
연결 관리 오버헤드: 60ms (86%) ← 낭비!
```

**왜 문제인가?**
- 연결 생성/종료는 비용이 큰 작업입니다
- 데이터베이스 최대 연결 수 제한에 빠르게 도달합니다
- 실제 쿼리 실행보다 연결 관리에 더 많은 시간 소요

### 성능 튜닝의 접근 방법

#### 1. 측정 먼저, 최적화는 나중에

```
추측이 아닌 데이터 기반 의사결정

단계:
1. 현재 성능 측정 (베이스라인 설정)
2. 병목 지점 식별
3. 목표 설정 (예: 응답 시간 50% 단축)
4. 최적화 수행
5. 측정 및 검증
6. 개선 효과가 있으면 적용, 없으면 롤백
```

#### 2. 우선순위 결정

**Pareto 원칙 (80/20 법칙) 적용:**
```
전체 성능 문제의 80%는 상위 20% 쿼리에서 발생

최적화 우선순위:
1. 가장 자주 실행되는 쿼리
2. 가장 느린 쿼리
3. 리소스를 가장 많이 사용하는 쿼리

예시:
- 메인 페이지 쿼리 (하루 100만 번 실행)
  → 0.1초 개선 = 하루 27시간 절약
- 관리자 리포트 쿼리 (하루 10번 실행)
  → 1초 개선 = 하루 10초 절약
```

#### 3. 단계적 접근

```
작은 것부터 시작하여 점진적으로 개선

Level 1: Quick Wins (빠른 개선)
├─ SELECT * 제거
├─ WHERE 절 인덱스 추가
└─ 불필요한 JOIN 제거

Level 2: 구조적 개선
├─ 복합 인덱스 설계
├─ 쿼리 재작성
└─ 파티셔닝 적용

Level 3: 아키텍처 변경
├─ 읽기 전용 복제본 추가
├─ 캐싱 레이어 도입
└─ 샤딩 적용

각 단계마다 측정하고 효과를 검증
```

### 성능 측정 지표

#### 1. 응답 시간 (Response Time)

```
쿼리 시작부터 결과 반환까지 걸리는 시간

측정 방법:
- 평균 (Average): 전체 평균 시간
- 중앙값 (Median/P50): 50% 사용자가 경험하는 시간
- P95: 95% 사용자가 경험하는 시간 (상위 5%는 더 느림)
- P99: 99% 사용자가 경험하는 시간

예시:
평균: 100ms
P95: 500ms ← 20명 중 1명은 500ms 이상 경험
P99: 2000ms ← 100명 중 1명은 2초 이상 경험

P95, P99가 중요한 이유:
- 평균은 이상치에 영향을 받지만, 실제 사용자 경험을 반영 못함
- 느린 경험을 한 사용자들이 불만을 가장 많이 표출
```

#### 2. 처리량 (Throughput)

```
단위 시간당 처리할 수 있는 쿼리/트랜잭션 수

측정 단위:
- QPS (Queries Per Second): 초당 쿼리 수
- TPS (Transactions Per Second): 초당 트랜잭션 수

예시:
현재: 1,000 QPS
목표: 5,000 QPS (5배 향상)

병목 확인:
- CPU 100% 사용 → CPU 병목
- 디스크 I/O 100% → 디스크 병목
- 네트워크 대역폭 100% → 네트워크 병목
```

#### 3. 리소스 사용률

```
하드웨어 자원 사용 현황

모니터링 항목:
├─ CPU 사용률
│   - 70% 미만: 여유 있음
│   - 70-80%: 주의 필요
│   - 80% 이상: 확장 필요
│
├─ 메모리 사용률
│   - Buffer Pool Hit Rate > 95%: 좋음
│   - < 90%: 메모리 부족
│
├─ 디스크 I/O
│   - IOPS (Input/Output Per Second)
│   - Latency (읽기/쓰기 지연)
│
└─ 네트워크
    - 대역폭 사용률
    - 패킷 손실률
```

#### 4. 동시성 지표

```
동시에 처리 중인 요청 수

모니터링 항목:
- Active Connections: 활성 연결 수
- Waiting Queries: 대기 중인 쿼리 수
- Lock Waits: 락 대기 시간

예시:
최대 연결 수: 200
현재 연결 수: 180 (90% 사용) ← 위험!
대기 중인 연결: 50 ← 병목 발생

해결 방법:
1. 연결 풀 크기 증가
2. 쿼리 최적화로 처리 시간 단축
3. 읽기 전용 복제본 추가
```

### 성능 튜닝의 일반 원칙

#### 1. 조기 최적화는 피하라

```
"Premature optimization is the root of all evil" - Donald Knuth

문제:
- 실제로 병목이 아닌 부분을 최적화
- 코드 복잡도만 증가
- 유지보수 비용 증가

올바른 접근:
1. 먼저 작동하는 코드 작성
2. 성능 문제 발생 시 측정
3. 실제 병목만 최적화
```

#### 2. 캐싱은 마지막 수단이 아닌 우선 고려사항

```
캐싱의 효과:
- DB 부하 감소
- 응답 속도 향상
- 비용 절감

하지만:
- 데이터 일관성 문제
- 캐시 무효화 복잡성
- 메모리 관리 필요

캐싱 적용 기준:
├─ 읽기가 쓰기보다 10배 이상 많은 데이터
├─ 자주 변경되지 않는 데이터
└─ 일시적 불일치를 허용할 수 있는 데이터
```

#### 3. 수직 확장 vs 수평 확장

```
수직 확장 (Scale Up):
장점: 구현 간단, 데이터 일관성 유지
단점: 비용 급증, 물리적 한계

수평 확장 (Scale Out):
장점: 비용 효율적, 무제한 확장
단점: 복잡도 증가, 일관성 관리 어려움

일반적 전략:
1. 먼저 쿼리 최적화
2. 캐싱 도입
3. 읽기 전용 복제본 추가 (수평 확장)
4. 데이터베이스 사양 업그레이드 (수직 확장)
5. 샤딩 도입 (수평 확장)
```

---

## 쿼리 최적화 및 실행 계획 분석

### 쿼리 최적화란?

쿼리 최적화는 동일한 결과를 반환하면서도 더 빠르고 효율적으로 실행되도록 SQL 쿼리를 개선하는 과정입니다. 데이터베이스 성능 튜닝에서 가장 즉각적이고 효과적인 방법입니다.

#### 쿼리 최적화가 중요한 이유

**1. 즉각적인 성능 향상**
```
코드 몇 줄 수정으로 10배 이상 성능 향상 가능

예시:
기존 쿼리: 10초
최적화 후: 0.1초 (100배 향상)

비용: 개발자 30분 투자
효과: 하루 수천 명의 사용자 경험 개선
```

**2. 하드웨어 비용 절감**
```
비효율적 쿼리 최적화 → 서버 증설 불필요

예시:
최적화 전: DB 서버 10대 필요 (월 1,000만원)
최적화 후: DB 서버 3대로 충분 (월 300만원)
→ 연간 8,400만원 절감
```

**3. 확장성 확보**
```
효율적 쿼리 → 동일 리소스로 더 많은 트래픽 처리

최적화 전: 1,000 RPS 처리
최적화 후: 5,000 RPS 처리 (같은 하드웨어)
```

### 실행 계획이란?

실행 계획(Execution Plan)은 데이터베이스가 쿼리를 실행하기 위해 선택한 구체적인 방법입니다. 쿼리 최적화의 첫 번째 단계는 실행 계획을 이해하는 것입니다.

#### 실행 계획의 구성 요소

**1. 접근 방법 (Access Method)**
```
데이터를 읽는 방식

종류:
├─ Table Scan (전체 테이블 스캔)
│   - 모든 행을 순차적으로 읽음
│   - 느리지만 인덱스가 없을 때 유일한 방법
│
├─ Index Scan (인덱스 스캔)
│   - 인덱스를 순차적으로 읽음
│   - 범위 검색에 효율적
│
├─ Index Seek (인덱스 탐색)
│   - 인덱스에서 특정 값만 찾음
│   - 가장 빠른 방법
│
└─ Index Full Scan (인덱스 전체 스캔)
    - 인덱스 전체를 읽지만 테이블은 안 읽음
    - 커버링 인덱스일 때 효율적
```

**2. 조인 방법 (Join Method)**
```
여러 테이블을 결합하는 방식

Nested Loop Join:
├─ 작동: 첫 번째 테이블의 각 행에 대해 두 번째 테이블 검색
├─ 적합: 작은 결과 집합
└─ 비효율: 큰 테이블 조인

Hash Join:
├─ 작동: 해시 테이블 생성 후 조인
├─ 적합: 큰 테이블 조인
└─ 단점: 메모리 사용량 높음

Merge Join:
├─ 작동: 정렬된 데이터를 병합
├─ 적합: 이미 정렬된 데이터
└─ 단점: 정렬 오버헤드
```

**3. 비용 (Cost)**
```
쿼리 실행에 필요한 예상 리소스

구성:
- CPU Cost: CPU 연산 비용
- I/O Cost: 디스크 읽기/쓰기 비용
- Network Cost: 데이터 전송 비용

예시:
쿼리 A: Cost = 100
쿼리 B: Cost = 10
→ 쿼리 B가 10배 효율적
```

### 1. 실행 계획 분석 기초

#### MySQL 실행 계획 분석
```sql
-- 기본 실행 계획 확인
EXPLAIN SELECT * FROM users WHERE email = 'user@example.com';

-- 상세한 실행 계획 확인
EXPLAIN FORMAT=JSON SELECT * FROM users WHERE email = 'user@example.com';

-- 실행 계획과 함께 실제 실행
EXPLAIN ANALYZE SELECT * FROM users WHERE email = 'user@example.com';
```

#### PostgreSQL 실행 계획 분석
```sql
-- 기본 실행 계획 확인
EXPLAIN SELECT * FROM users WHERE email = 'user@example.com';

-- 상세한 실행 계획 확인
EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON) 
SELECT * FROM users WHERE email = 'user@example.com';

-- 실행 계획 캐싱
EXPLAIN (ANALYZE, BUFFERS, COSTS, VERBOSE, FORMAT JSON)
SELECT * FROM users WHERE email = 'user@example.com';
```

### 2. 실행 계획 해석

#### 주요 실행 계획 요소
```sql
-- 실행 계획 결과 해석 예시
EXPLAIN SELECT u.*, p.name as profile_name 
FROM users u 
JOIN profiles p ON u.id = p.user_id 
WHERE u.created_at > '2024-01-01';

-- 결과 해석:
-- 1. type: ALL (전체 테이블 스캔) vs index (인덱스 스캔)
-- 2. key: 사용된 인덱스
-- 3. rows: 예상 행 수
-- 4. Extra: 추가 정보 (Using index, Using temporary, Using filesort)
```

#### 성능 문제 진단
```javascript
class QueryAnalyzer {
  constructor() {
    this.slowQueries = [];
    this.queryStats = new Map();
  }

  // 느린 쿼리 감지
  analyzeSlowQuery(query, executionTime) {
    if (executionTime > 1000) { // 1초 이상
      this.slowQueries.push({
        query,
        executionTime,
        timestamp: new Date(),
        suggestions: this.generateSuggestions(query)
      });
    }
  }

  // 쿼리 최적화 제안
  generateSuggestions(query) {
    const suggestions = [];
    
    if (query.includes('SELECT *')) {
      suggestions.push('SELECT * 대신 필요한 컬럼만 선택하세요');
    }
    
    if (query.includes('ORDER BY') && !query.includes('LIMIT')) {
      suggestions.push('ORDER BY와 함께 LIMIT을 사용하세요');
    }
    
    if (query.includes('LIKE \'%')) {
      suggestions.push('LIKE \'%...\' 패턴은 인덱스를 사용할 수 없습니다');
    }
    
    return suggestions;
  }

  // 쿼리 통계 수집
  collectQueryStats(query, executionTime) {
    const normalizedQuery = this.normalizeQuery(query);
    
    if (!this.queryStats.has(normalizedQuery)) {
      this.queryStats.set(normalizedQuery, {
        count: 0,
        totalTime: 0,
        avgTime: 0,
        maxTime: 0
      });
    }
    
    const stats = this.queryStats.get(normalizedQuery);
    stats.count++;
    stats.totalTime += executionTime;
    stats.avgTime = stats.totalTime / stats.count;
    stats.maxTime = Math.max(stats.maxTime, executionTime);
  }

  // 쿼리 정규화 (매개변수 제거)
  normalizeQuery(query) {
    return query.replace(/\d+/g, '?')
                .replace(/'[^']*'/g, '?')
                .replace(/"[^"]*"/g, '?');
  }
}
```

### 3. 쿼리 최적화 기법

#### 인덱스 힌트 사용
```sql
-- MySQL 인덱스 힌트
SELECT * FROM users USE INDEX (idx_email) WHERE email = 'user@example.com';
SELECT * FROM users FORCE INDEX (idx_email) WHERE email = 'user@example.com';
SELECT * FROM users IGNORE INDEX (idx_old) WHERE email = 'user@example.com';

-- PostgreSQL 인덱스 힌트
SELECT * FROM users WHERE email = 'user@example.com';
-- PostgreSQL에서는 힌트 대신 통계 정보 업데이트
ANALYZE users;
```

#### 쿼리 재작성
```sql
-- ❌ 비효율적인 쿼리
SELECT * FROM users WHERE YEAR(created_at) = 2024;

-- ✅ 최적화된 쿼리
SELECT * FROM users WHERE created_at >= '2024-01-01' AND created_at < '2025-01-01';

-- ❌ 비효율적인 쿼리
SELECT * FROM users WHERE email LIKE '%@gmail.com';

-- ✅ 최적화된 쿼리 (가능한 경우)
SELECT * FROM users WHERE email LIKE 'user%@gmail.com';
```

#### 서브쿼리 최적화
```sql
-- ❌ 비효율적인 서브쿼리
SELECT * FROM users WHERE id IN (
  SELECT user_id FROM orders WHERE total_amount > 1000
);

-- ✅ 최적화된 JOIN
SELECT DISTINCT u.* FROM users u
JOIN orders o ON u.id = o.user_id
WHERE o.total_amount > 1000;

-- ❌ 비효율적인 EXISTS
SELECT * FROM users WHERE EXISTS (
  SELECT 1 FROM orders WHERE user_id = users.id AND total_amount > 1000
);

-- ✅ 최적화된 EXISTS (인덱스 활용)
SELECT * FROM users WHERE EXISTS (
  SELECT 1 FROM orders WHERE user_id = users.id AND total_amount > 1000
);
```

---

## 인덱스 설계 및 최적화 전략

### 인덱스란?

인덱스는 데이터베이스에서 데이터를 빠르게 찾기 위한 자료구조입니다. 책의 색인처럼, 전체 내용을 읽지 않고도 원하는 정보를 빠르게 찾을 수 있게 해줍니다.

#### 인덱스의 필요성

**인덱스가 없는 경우:**
```
사용자 테이블 (1,000만 건)에서 특정 이메일 검색:

SELECT * FROM users WHERE email = 'john@example.com';

작동 방식:
1. 첫 번째 행부터 시작
2. 1,000만 개의 모든 행을 순차적으로 확인
3. 일치하는 이메일 발견
4. 계속 검색 (중복 가능성)

소요 시간: 10초
읽은 행 수: 1,000만 건
디스크 I/O: 매우 높음
```

**인덱스가 있는 경우:**
```
email 컬럼에 인덱스 생성:

CREATE INDEX idx_email ON users(email);

작동 방식:
1. 인덱스 트리에서 'john@example.com' 검색
2. 3-4번의 노드 탐색으로 위치 파악
3. 해당 데이터 직접 접근

소요 시간: 0.001초 (10,000배 빠름)
읽은 행 수: 1건
디스크 I/O: 최소화
```

#### 인덱스의 작동 원리

**책의 색인과 유사한 구조:**
```
책 (데이터베이스 테이블):
- 1,000페이지의 내용
- 순서대로 읽으면 모든 페이지를 확인해야 함

색인 (인덱스):
- 주제별로 정렬된 목록
- 각 항목은 해당 페이지 번호를 가리킴
- 원하는 주제를 색인에서 찾으면 즉시 페이지로 이동

예시:
"데이터베이스"를 찾고 싶을 때
1. 색인에서 "데이터베이스" 검색 → "p.234"
2. 234페이지로 바로 이동

인덱스 없이: 1,000페이지를 순서대로 확인
인덱스 사용: 색인에서 찾고 해당 페이지로 바로 이동
```

#### 인덱스의 트레이드오프

**장점:**
```
1. 검색 속도 향상
   - 전체 스캔 대신 인덱스 탐색
   - O(n) → O(log n) 복잡도

2. 정렬 성능 향상
   - ORDER BY 절의 성능 개선
   - 이미 정렬된 구조 활용

3. 조인 성능 향상
   - 조인 키에 인덱스가 있으면 빠른 매칭
   - Nested Loop Join 최적화
```

**단점:**
```
1. 저장 공간 증가
   - 인덱스 자체가 디스크 공간 차지
   - 테이블 크기의 10-20% 추가 공간 필요

2. 쓰기 성능 저하
   - INSERT: 데이터 + 인덱스 모두 추가
   - UPDATE: 인덱스 키 변경 시 재정렬
   - DELETE: 인덱스에서도 제거

3. 유지보수 비용
   - 인덱스 조각화 관리
   - 통계 정보 업데이트
   - 불필요한 인덱스 식별 및 제거
```

**읽기 vs 쓰기 비율에 따른 결정:**
```
읽기 위주 시스템 (읽기 90% : 쓰기 10%):
→ 인덱스를 적극적으로 활용
→ 검색 성능 향상 이득 > 쓰기 성능 저하 손실

쓰기 위주 시스템 (읽기 20% : 쓰기 80%):
→ 최소한의 필수 인덱스만 유지
→ 쓰기 성능 우선

균형 잡힌 시스템 (읽기 50% : 쓰기 50%):
→ 자주 사용되는 쿼리에만 인덱스 생성
→ 정기적인 성능 모니터링 필요
```

### 인덱스를 언제 사용해야 하는가?

#### 인덱스가 필요한 경우

```
1. WHERE 절에서 자주 사용되는 컬럼
   SELECT * FROM orders WHERE user_id = 123;
   → user_id에 인덱스 필수

2. JOIN의 키 컬럼
   SELECT * FROM orders o JOIN users u ON o.user_id = u.id;
   → 양쪽 조인 키에 인덱스 필요

3. ORDER BY 절의 컬럼
   SELECT * FROM products ORDER BY created_at DESC LIMIT 10;
   → created_at에 인덱스 있으면 정렬 생략

4. GROUP BY 절의 컬럼
   SELECT category, COUNT(*) FROM products GROUP BY category;
   → category에 인덱스 있으면 효율적

5. 선택도가 높은 컬럼 (유일한 값이 많음)
   - 이메일 (거의 모두 유일)
   - 주문 번호 (유일)
   - 사용자 ID (유일)
```

#### 인덱스가 불필요한 경우

```
1. 선택도가 낮은 컬럼 (반복되는 값이 많음)
   - 성별 (남/여 2가지)
   - 상태 (활성/비활성 2가지)
   - 불린 값 (true/false)
   
   예외: 한 쪽 값의 비율이 매우 낮으면 부분 인덱스 고려
   
2. 작은 테이블 (수천 건 이하)
   - 전체 스캔이 더 빠를 수 있음
   - 인덱스 탐색 오버헤드가 더 클 수 있음

3. 자주 변경되는 컬럼
   - 쓰기 성능 저하가 크게 나타남
   - 인덱스 재구성 비용 증가

4. 테이블의 상당 부분을 조회하는 경우
   - 전체의 20% 이상 조회 시 전체 스캔이 더 효율적
   - 인덱스 + 테이블 접근이 2배의 I/O 발생
```

### 인덱스 설계 원칙

#### 1. 선택도 (Selectivity)

```
선택도 = 고유한 값의 수 / 전체 행 수

높은 선택도 (좋은 인덱스):
- 이메일: 100만 개 / 100만 행 = 1.0 (100%)
- 주문 번호: 500만 개 / 500만 행 = 1.0 (100%)

낮은 선택도 (나쁜 인덱스):
- 성별: 2개 / 100만 행 = 0.000002 (0.0002%)
- 상태: 5개 / 100만 행 = 0.000005 (0.0005%)

권장 사항:
- 선택도 > 0.1 (10%): 인덱스 효과적
- 선택도 < 0.01 (1%): 인덱스 효과 미미
```

#### 2. 카디널리티 (Cardinality)

```
카디널리티 = 컬럼의 고유한 값 개수

높은 카디널리티 (인덱스 적합):
- 사용자 ID: 1,000,000 (각 사용자마다 고유)
- 이메일: 1,000,000 (각 사용자마다 고유)
- 주문 번호: 5,000,000 (각 주문마다 고유)

낮은 카디널리티 (인덱스 부적합):
- 성별: 2 (남/여)
- 국가: 200 (전 세계 국가 수)
- 상태: 5 (대기/진행/완료/취소/실패)

복합 인덱스 순서 결정:
높은 카디널리티 컬럼을 앞에 배치하는 것이 일반적으로 효과적
```

#### 3. 복합 인덱스 컬럼 순서

```
쿼리: SELECT * FROM orders WHERE user_id = 123 AND status = 'completed';

옵션 1: INDEX(user_id, status)
- user_id로 먼저 필터링 (사용자의 모든 주문)
- 그 중에서 status로 필터링 (완료된 주문만)
- 효율적: user_id는 고유도가 높음

옵션 2: INDEX(status, user_id)
- status로 먼저 필터링 (모든 완료 주문)
- 그 중에서 user_id로 필터링 (해당 사용자의 것만)
- 비효율적: status는 고유도가 낮음

일반 규칙:
1. 등호(=) 조건이 있는 컬럼을 앞에
2. 선택도가 높은 컬럼을 앞에
3. 범위 조건(<, >, BETWEEN)은 뒤에
```

### 1. 인덱스 종류 및 특성

#### B-Tree 인덱스
```sql
-- 단일 컬럼 인덱스
CREATE INDEX idx_email ON users(email);

-- 복합 인덱스
CREATE INDEX idx_user_status ON users(status, created_at);

-- 부분 인덱스 (PostgreSQL)
CREATE INDEX idx_active_users ON users(email) WHERE status = 'active';

-- 함수 기반 인덱스
CREATE INDEX idx_lower_email ON users(LOWER(email));
```

#### Hash 인덱스
```sql
-- MySQL Memory 엔진에서 사용
CREATE TABLE user_cache (
  id INT PRIMARY KEY,
  data VARCHAR(255)
) ENGINE=MEMORY;

CREATE INDEX idx_hash ON user_cache(id) USING HASH;
```

#### 전문 검색 인덱스
```sql
-- MySQL Full-Text 인덱스
CREATE FULLTEXT INDEX idx_content ON articles(title, content);

-- 전문 검색 쿼리
SELECT * FROM articles 
WHERE MATCH(title, content) AGAINST('database performance' IN NATURAL LANGUAGE MODE);
```

### 2. 인덱스 설계 원칙

#### 선택도 기반 인덱스 설계
```javascript
class IndexDesigner {
  constructor() {
    this.columnStats = new Map();
  }

  // 컬럼 선택도 계산
  calculateSelectivity(tableName, columnName) {
    // 실제로는 데이터베이스에서 통계 정보를 가져옴
    const totalRows = 1000000;
    const distinctValues = 100000;
    
    return distinctValues / totalRows; // 0.1 (10%)
  }

  // 인덱스 효율성 평가
  evaluateIndexEfficiency(tableName, columns) {
    const selectivities = columns.map(col => 
      this.calculateSelectivity(tableName, col)
    );
    
    // 선택도가 높을수록 좋은 인덱스
    const avgSelectivity = selectivities.reduce((a, b) => a + b, 0) / selectivities.length;
    
    return {
      efficiency: avgSelectivity,
      recommendation: avgSelectivity > 0.1 ? 'Good' : 'Poor'
    };
  }

  // 복합 인덱스 컬럼 순서 결정
  determineColumnOrder(tableName, columns) {
    // 1. 등호 조건이 있는 컬럼을 앞에 배치
    // 2. 선택도가 높은 컬럼을 앞에 배치
    // 3. 범위 검색이 있는 컬럼을 뒤에 배치
    
    return columns.sort((a, b) => {
      const selectivityA = this.calculateSelectivity(tableName, a);
      const selectivityB = this.calculateSelectivity(tableName, b);
      return selectivityB - selectivityA; // 내림차순
    });
  }
}
```

#### 인덱스 모니터링
```sql
-- MySQL 인덱스 사용 현황
SELECT 
  TABLE_NAME,
  INDEX_NAME,
  CARDINALITY,
  INDEX_TYPE,
  COMMENT
FROM information_schema.STATISTICS 
WHERE TABLE_SCHEMA = 'your_database'
ORDER BY TABLE_NAME, INDEX_NAME;

-- PostgreSQL 인덱스 사용 현황
SELECT 
  schemaname,
  tablename,
  indexname,
  idx_scan,
  idx_tup_read,
  idx_tup_fetch
FROM pg_stat_user_indexes
WHERE schemaname = 'public'
ORDER BY idx_scan DESC;

-- 사용되지 않는 인덱스 찾기
SELECT 
  schemaname,
  tablename,
  indexname,
  idx_scan
FROM pg_stat_user_indexes
WHERE idx_scan = 0
ORDER BY tablename;
```

### 3. 인덱스 최적화 전략

#### 인덱스 조각화 관리
```sql
-- MySQL 인덱스 조각화 확인
SELECT 
  TABLE_NAME,
  INDEX_NAME,
  CARDINALITY,
  SUB_PART,
  PACKED,
  NULLABLE,
  INDEX_TYPE
FROM information_schema.STATISTICS 
WHERE TABLE_SCHEMA = 'your_database';

-- 인덱스 재구성
OPTIMIZE TABLE users;

-- PostgreSQL 인덱스 재구성
REINDEX INDEX idx_email;
REINDEX TABLE users;
```

#### 인덱스 통계 업데이트
```sql
-- MySQL 통계 업데이트
ANALYZE TABLE users;

-- PostgreSQL 통계 업데이트
ANALYZE users;

-- 특정 컬럼 통계 업데이트
ANALYZE users (email, created_at);
```

---

## 데이터베이스 파티셔닝 및 샤딩

### 1. 파티셔닝 전략

#### 범위 기반 파티셔닝
```sql
-- 날짜 기반 파티셔닝
CREATE TABLE user_activities (
    id BIGINT,
    user_id BIGINT,
    activity_type VARCHAR(50),
    created_at TIMESTAMP,
    data JSONB
) PARTITION BY RANGE (created_at);

-- 월별 파티션 생성
CREATE TABLE user_activities_2024_01 PARTITION OF user_activities
    FOR VALUES FROM ('2024-01-01') TO ('2024-02-01');

CREATE TABLE user_activities_2024_02 PARTITION OF user_activities
    FOR VALUES FROM ('2024-02-01') TO ('2024-03-01');

-- 파티션 자동 생성 함수
CREATE OR REPLACE FUNCTION create_monthly_partition(table_name text, start_date date)
RETURNS void AS $$
DECLARE
    partition_name text;
    end_date date;
BEGIN
    partition_name := table_name || '_' || to_char(start_date, 'YYYY_MM');
    end_date := start_date + interval '1 month';
    
    EXECUTE format('CREATE TABLE %I PARTITION OF %I FOR VALUES FROM (%L) TO (%L)',
                   partition_name, table_name, start_date, end_date);
END;
$$ LANGUAGE plpgsql;
```

#### 해시 기반 파티셔닝
```sql
-- 사용자 ID 기반 해시 파티셔닝
CREATE TABLE users (
    id BIGINT,
    username VARCHAR(50),
    email VARCHAR(100),
    created_at TIMESTAMP
) PARTITION BY HASH (id);

-- 4개 파티션 생성
CREATE TABLE users_0 PARTITION OF users
    FOR VALUES WITH (modulus 4, remainder 0);

CREATE TABLE users_1 PARTITION OF users
    FOR VALUES WITH (modulus 4, remainder 1);

CREATE TABLE users_2 PARTITION OF users
    FOR VALUES WITH (modulus 4, remainder 2);

CREATE TABLE users_3 PARTITION OF users
    FOR VALUES WITH (modulus 4, remainder 3);
```

### 2. 샤딩 구현

#### 수평 샤딩
```javascript
class HorizontalSharding {
  constructor(shardCount) {
    this.shardCount = shardCount;
    this.shards = new Array(shardCount).fill(null).map(() => ({
      connection: null,
      data: new Map()
    }));
  }

  // 해시 기반 샤딩
  getShardId(key) {
    const hash = this.hashFunction(key);
    return hash % this.shardCount;
  }

  hashFunction(str) {
    let hash = 0;
    for (let i = 0; i < str.length; i++) {
      const char = str.charCodeAt(i);
      hash = ((hash << 5) - hash) + char;
      hash = hash & hash;
    }
    return Math.abs(hash);
  }

  async insert(key, value) {
    const shardId = this.getShardId(key);
    const shard = this.shards[shardId];
    
    shard.data.set(key, value);
    console.log(`Inserted ${key} into shard ${shardId}`);
  }

  async get(key) {
    const shardId = this.getShardId(key);
    const shard = this.shards[shardId];
    
    return shard.data.get(key);
  }

  // 크로스 샤드 쿼리
  async crossShardQuery(predicate) {
    const results = [];
    
    for (let i = 0; i < this.shardCount; i++) {
      const shard = this.shards[i];
      const shardResults = Array.from(shard.data.values())
        .filter(predicate);
      results.push(...shardResults);
    }
    
    return results;
  }
}
```

#### 수직 샤딩
```javascript
class VerticalSharding {
  constructor() {
    this.shards = {
      userProfile: new Map(),    // 사용자 프로필 정보
      userActivity: new Map(),   // 사용자 활동 로그
      userSettings: new Map()    // 사용자 설정
    };
  }

  async insertUserProfile(userId, profile) {
    this.shards.userProfile.set(userId, profile);
  }

  async insertUserActivity(userId, activity) {
    if (!this.shards.userActivity.has(userId)) {
      this.shards.userActivity.set(userId, []);
    }
    this.shards.userActivity.get(userId).push(activity);
  }

  async insertUserSettings(userId, settings) {
    this.shards.userSettings.set(userId, settings);
  }

  async getUserData(userId) {
    const profile = this.shards.userProfile.get(userId);
    const activities = this.shards.userActivity.get(userId) || [];
    const settings = this.shards.userSettings.get(userId);

    return {
      profile,
      activities,
      settings
    };
  }
}
```

---

## 연결 풀 관리 및 최적화

### 1. 연결 풀 설정

#### Node.js 연결 풀 설정
```javascript
const mysql = require('mysql2/promise');

class DatabaseConnectionPool {
  constructor() {
    this.pool = mysql.createPool({
      host: process.env.DB_HOST,
      user: process.env.DB_USER,
      password: process.env.DB_PASSWORD,
      database: process.env.DB_NAME,
      
      // 연결 풀 설정
      connectionLimit: 20,           // 최대 연결 수
      acquireTimeoutMillis: 5000,    // 연결 획득 타임아웃
      timeout: 30000,                // 쿼리 타임아웃
      idleTimeoutMillis: 300000,     // 유휴 연결 타임아웃 (5분)
      
      // 재연결 설정
      reconnect: true,
      maxReconnects: 3,
      reconnectDelay: 1000,
      
      // 성능 설정
      multipleStatements: false,
      dateStrings: true,
      supportBigNumbers: true,
      bigNumberStrings: true
    });
    
    this.setupEventHandlers();
  }

  setupEventHandlers() {
    this.pool.on('connection', (connection) => {
      console.log('New connection established:', connection.threadId);
    });

    this.pool.on('acquire', (connection) => {
      console.log('Connection acquired:', connection.threadId);
    });

    this.pool.on('release', (connection) => {
      console.log('Connection released:', connection.threadId);
    });

    this.pool.on('error', (err) => {
      console.error('Pool error:', err);
    });
  }

  async query(sql, params = []) {
    let connection;
    try {
      connection = await this.pool.getConnection();
      const [rows] = await connection.execute(sql, params);
      return rows;
    } catch (error) {
      console.error('Query error:', error);
      throw error;
    } finally {
      if (connection) {
        connection.release();
      }
    }
  }

  async withTransaction(operation) {
    let connection;
    try {
      connection = await this.pool.getConnection();
      await connection.beginTransaction();
      
      const result = await operation(connection);
      
      await connection.commit();
      return result;
    } catch (error) {
      if (connection) {
        await connection.rollback();
      }
      throw error;
    } finally {
      if (connection) {
        connection.release();
      }
    }
  }

  // 연결 풀 상태 모니터링
  getPoolStatus() {
    return {
      totalConnections: this.pool._allConnections.length,
      freeConnections: this.pool._freeConnections.length,
      queuedRequests: this.pool._connectionQueue.length
    };
  }
}
```

#### PostgreSQL 연결 풀 설정
```javascript
const { Pool } = require('pg');

class PostgreSQLConnectionPool {
  constructor() {
    this.pool = new Pool({
      host: process.env.DB_HOST,
      port: process.env.DB_PORT,
      database: process.env.DB_NAME,
      user: process.env.DB_USER,
      password: process.env.DB_PASSWORD,
      
      // 연결 풀 설정
      max: 20,                    // 최대 연결 수
      min: 5,                     // 최소 연결 수
      idleTimeoutMillis: 30000,   // 유휴 연결 타임아웃
      connectionTimeoutMillis: 5000, // 연결 타임아웃
      
      // 성능 설정
      statement_timeout: 30000,   // 쿼리 타임아웃
      query_timeout: 30000,       // 쿼리 타임아웃
      application_name: 'myapp',  // 애플리케이션 이름
      
      // SSL 설정
      ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false
    });
    
    this.setupEventHandlers();
  }

  setupEventHandlers() {
    this.pool.on('connect', (client) => {
      console.log('New client connected');
    });

    this.pool.on('error', (err, client) => {
      console.error('Unexpected error on idle client', err);
    });
  }

  async query(text, params = []) {
    const start = Date.now();
    try {
      const result = await this.pool.query(text, params);
      const duration = Date.now() - start;
      console.log('Query executed', { text, duration, rows: result.rowCount });
      return result;
    } catch (error) {
      console.error('Query error:', error);
      throw error;
    }
  }
}
```

### 2. 연결 풀 최적화

#### 동적 연결 풀 크기 조정
```javascript
class AdaptiveConnectionPool {
  constructor() {
    this.pool = null;
    this.metrics = {
      activeConnections: 0,
      queuedRequests: 0,
      avgResponseTime: 0,
      errorRate: 0
    };
    
    this.startMonitoring();
  }

  startMonitoring() {
    setInterval(() => {
      this.adjustPoolSize();
    }, 60000); // 1분마다
  }

  adjustPoolSize() {
    const { activeConnections, queuedRequests, avgResponseTime, errorRate } = this.metrics;
    
    // 큐에 대기 중인 요청이 많으면 연결 수 증가
    if (queuedRequests > 10) {
      this.increasePoolSize();
    }
    
    // 평균 응답 시간이 길면 연결 수 증가
    if (avgResponseTime > 1000) {
      this.increasePoolSize();
    }
    
    // 에러율이 높으면 연결 수 감소
    if (errorRate > 0.05) {
      this.decreasePoolSize();
    }
    
    // 유휴 연결이 많으면 연결 수 감소
    if (activeConnections < this.pool.options.connectionLimit * 0.3) {
      this.decreasePoolSize();
    }
  }

  increasePoolSize() {
    const currentLimit = this.pool.options.connectionLimit;
    const newLimit = Math.min(currentLimit + 5, 50); // 최대 50개
    
    if (newLimit > currentLimit) {
      this.pool.options.connectionLimit = newLimit;
      console.log(`Pool size increased to ${newLimit}`);
    }
  }

  decreasePoolSize() {
    const currentLimit = this.pool.options.connectionLimit;
    const newLimit = Math.max(currentLimit - 2, 5); // 최소 5개
    
    if (newLimit < currentLimit) {
      this.pool.options.connectionLimit = newLimit;
      console.log(`Pool size decreased to ${newLimit}`);
    }
  }
}
```

#### 연결 풀 모니터링
```javascript
class ConnectionPoolMonitor {
  constructor(pool) {
    this.pool = pool;
    this.metrics = {
      totalConnections: 0,
      activeConnections: 0,
      idleConnections: 0,
      queuedRequests: 0,
      totalQueries: 0,
      failedQueries: 0,
      avgQueryTime: 0
    };
    
    this.startMonitoring();
  }

  startMonitoring() {
    setInterval(() => {
      this.collectMetrics();
      this.logMetrics();
    }, 10000); // 10초마다
  }

  collectMetrics() {
    this.metrics.totalConnections = this.pool._allConnections.length;
    this.metrics.activeConnections = this.pool._allConnections.filter(c => c._inUse).length;
    this.metrics.idleConnections = this.pool._freeConnections.length;
    this.metrics.queuedRequests = this.pool._connectionQueue.length;
  }

  logMetrics() {
    console.log('Connection Pool Metrics:', {
      total: this.metrics.totalConnections,
      active: this.metrics.activeConnections,
      idle: this.metrics.idleConnections,
      queued: this.metrics.queuedRequests,
      utilization: (this.metrics.activeConnections / this.metrics.totalConnections * 100).toFixed(2) + '%'
    });
  }

  // 알림 설정
  checkThresholds() {
    const utilization = this.metrics.activeConnections / this.metrics.totalConnections;
    
    if (utilization > 0.8) {
      console.warn('High connection pool utilization:', utilization);
    }
    
    if (this.metrics.queuedRequests > 20) {
      console.warn('High queued requests:', this.metrics.queuedRequests);
    }
  }
}
```

---

## 실제 성능 튜닝 사례 및 벤치마킹

### 1. 성능 튜닝 사례

#### 사례 1: 느린 사용자 검색 쿼리 최적화
```sql
-- ❌ 최적화 전 (2.5초)
SELECT u.*, p.name as profile_name 
FROM users u 
LEFT JOIN profiles p ON u.id = p.user_id 
WHERE u.email LIKE '%@gmail.com' 
ORDER BY u.created_at DESC 
LIMIT 20;

-- ✅ 최적화 후 (0.1초)
-- 1. 인덱스 생성
CREATE INDEX idx_users_email ON users(email);
CREATE INDEX idx_users_created_at ON users(created_at);

-- 2. 쿼리 최적화
SELECT u.id, u.email, u.created_at, p.name as profile_name
FROM users u 
LEFT JOIN profiles p ON u.id = p.user_id 
WHERE u.email LIKE 'user%@gmail.com'  -- 앞쪽 와일드카드 제거
ORDER BY u.created_at DESC 
LIMIT 20;
```

#### 사례 2: 대량 데이터 삽입 최적화
```javascript
// ❌ 최적화 전 (1000개 삽입에 10초)
async function insertUsersSlow(users) {
  for (const user of users) {
    await db.query('INSERT INTO users (name, email) VALUES (?, ?)', [user.name, user.email]);
  }
}

// ✅ 최적화 후 (1000개 삽입에 0.5초)
async function insertUsersFast(users) {
  const values = users.map(user => [user.name, user.email]);
  const placeholders = users.map(() => '(?, ?)').join(', ');
  
  await db.query(`INSERT INTO users (name, email) VALUES ${placeholders}`, values.flat());
}

// ✅ 배치 삽입 최적화
async function insertUsersBatch(users, batchSize = 1000) {
  for (let i = 0; i < users.length; i += batchSize) {
    const batch = users.slice(i, i + batchSize);
    await insertUsersFast(batch);
  }
}
```

#### 사례 3: 복잡한 조인 쿼리 최적화
```sql
-- ❌ 최적화 전 (5초)
SELECT u.name, u.email, COUNT(o.id) as order_count, SUM(o.total) as total_spent
FROM users u
LEFT JOIN orders o ON u.id = o.user_id
LEFT JOIN order_items oi ON o.id = oi.order_id
WHERE u.created_at > '2024-01-01'
GROUP BY u.id, u.name, u.email
HAVING COUNT(o.id) > 5
ORDER BY total_spent DESC
LIMIT 100;

-- ✅ 최적화 후 (0.3초)
-- 1. 인덱스 생성
CREATE INDEX idx_users_created_at ON users(created_at);
CREATE INDEX idx_orders_user_id ON orders(user_id);
CREATE INDEX idx_orders_total ON orders(total);

-- 2. 쿼리 최적화 (서브쿼리 활용)
SELECT u.name, u.email, o.order_count, o.total_spent
FROM users u
JOIN (
  SELECT user_id, COUNT(*) as order_count, SUM(total) as total_spent
  FROM orders
  WHERE created_at > '2024-01-01'
  GROUP BY user_id
  HAVING COUNT(*) > 5
) o ON u.id = o.user_id
WHERE u.created_at > '2024-01-01'
ORDER BY o.total_spent DESC
LIMIT 100;
```

### 2. 벤치마킹 도구

#### 성능 벤치마킹 클래스
```javascript
class DatabaseBenchmark {
  constructor() {
    this.results = [];
    this.metrics = {
      totalQueries: 0,
      totalTime: 0,
      avgTime: 0,
      minTime: Infinity,
      maxTime: 0,
      errorCount: 0
    };
  }

  // 단일 쿼리 벤치마크
  async benchmarkQuery(name, query, params = [], iterations = 100) {
    console.log(`Benchmarking: ${name}`);
    
    const times = [];
    let errors = 0;
    
    for (let i = 0; i < iterations; i++) {
      const start = process.hrtime.bigint();
      
      try {
        await this.executeQuery(query, params);
        const end = process.hrtime.bigint();
        const duration = Number(end - start) / 1000000; // ms
        times.push(duration);
      } catch (error) {
        errors++;
        console.error(`Query error: ${error.message}`);
      }
    }
    
    const result = this.calculateStats(times, errors, iterations);
    this.results.push({ name, ...result });
    
    console.log(`Results for ${name}:`, result);
    return result;
  }

  // 동시성 벤치마크
  async benchmarkConcurrency(name, query, params = [], concurrency = 10, duration = 10000) {
    console.log(`Concurrency benchmark: ${name}`);
    
    const startTime = Date.now();
    const results = [];
    const promises = [];
    
    for (let i = 0; i < concurrency; i++) {
      promises.push(this.runConcurrentQueries(query, params, startTime, duration));
    }
    
    const allResults = await Promise.all(promises);
    const combinedResults = allResults.flat();
    
    const result = this.calculateStats(combinedResults, 0, combinedResults.length);
    this.results.push({ name: `${name}_concurrent`, ...result });
    
    console.log(`Concurrency results for ${name}:`, result);
    return result;
  }

  async runConcurrentQueries(query, params, startTime, duration) {
    const results = [];
    
    while (Date.now() - startTime < duration) {
      const start = process.hrtime.bigint();
      
      try {
        await this.executeQuery(query, params);
        const end = process.hrtime.bigint();
        const queryTime = Number(end - start) / 1000000;
        results.push(queryTime);
      } catch (error) {
        // 에러는 무시하고 계속
      }
    }
    
    return results;
  }

  calculateStats(times, errors, total) {
    if (times.length === 0) {
      return {
        avgTime: 0,
        minTime: 0,
        maxTime: 0,
        p95Time: 0,
        p99Time: 0,
        errorRate: errors / total,
        throughput: 0
      };
    }
    
    const sortedTimes = times.sort((a, b) => a - b);
    const avgTime = times.reduce((a, b) => a + b, 0) / times.length;
    const minTime = Math.min(...times);
    const maxTime = Math.max(...times);
    const p95Time = sortedTimes[Math.floor(sortedTimes.length * 0.95)];
    const p99Time = sortedTimes[Math.floor(sortedTimes.length * 0.99)];
    const errorRate = errors / total;
    const throughput = times.length / (times.reduce((a, b) => a + b, 0) / 1000); // queries per second
    
    return {
      avgTime: Math.round(avgTime * 100) / 100,
      minTime: Math.round(minTime * 100) / 100,
      maxTime: Math.round(maxTime * 100) / 100,
      p95Time: Math.round(p95Time * 100) / 100,
      p99Time: Math.round(p99Time * 100) / 100,
      errorRate: Math.round(errorRate * 10000) / 100, // percentage
      throughput: Math.round(throughput * 100) / 100
    };
  }

  // 결과 비교
  compareResults() {
    console.log('\n=== Benchmark Results Comparison ===');
    
    this.results.forEach(result => {
      console.log(`\n${result.name}:`);
      console.log(`  Average Time: ${result.avgTime}ms`);
      console.log(`  Min Time: ${result.minTime}ms`);
      console.log(`  Max Time: ${result.maxTime}ms`);
      console.log(`  95th Percentile: ${result.p95Time}ms`);
      console.log(`  99th Percentile: ${result.p99Time}ms`);
      console.log(`  Error Rate: ${result.errorRate}%`);
      console.log(`  Throughput: ${result.throughput} queries/sec`);
    });
  }

  async executeQuery(query, params) {
    // 실제 데이터베이스 쿼리 실행
    // 이 부분은 실제 데이터베이스 연결에 맞게 구현
    throw new Error('executeQuery method must be implemented');
  }
}
```

#### 실제 벤치마킹 예시
```javascript
// 벤치마킹 실행 예시
async function runBenchmarks() {
  const benchmark = new DatabaseBenchmark();
  
  // 단일 쿼리 벤치마크
  await benchmark.benchmarkQuery(
    'Simple SELECT',
    'SELECT * FROM users WHERE id = ?',
    [1],
    1000
  );
  
  await benchmark.benchmarkQuery(
    'Complex JOIN',
    `SELECT u.name, COUNT(o.id) as order_count 
     FROM users u 
     LEFT JOIN orders o ON u.id = o.user_id 
     GROUP BY u.id, u.name 
     LIMIT 100`,
    [],
    100
  );
  
  // 동시성 벤치마크
  await benchmark.benchmarkConcurrency(
    'Concurrent SELECT',
    'SELECT * FROM users WHERE email = ?',
    ['test@example.com'],
    20,
    10000
  );
  
  // 결과 비교
  benchmark.compareResults();
}

runBenchmarks().catch(console.error);
```

### 3. 성능 모니터링 대시보드

```javascript
class PerformanceDashboard {
  constructor() {
    this.metrics = {
      queries: [],
      connections: [],
      errors: []
    };
    
    this.startMonitoring();
  }

  startMonitoring() {
    setInterval(() => {
      this.collectMetrics();
      this.updateDashboard();
    }, 5000); // 5초마다
  }

  collectMetrics() {
    // 쿼리 성능 메트릭 수집
    const queryMetrics = {
      timestamp: new Date(),
      avgResponseTime: this.getAverageResponseTime(),
      totalQueries: this.getTotalQueries(),
      slowQueries: this.getSlowQueries(),
      errorRate: this.getErrorRate()
    };
    
    this.metrics.queries.push(queryMetrics);
    
    // 연결 풀 메트릭 수집
    const connectionMetrics = {
      timestamp: new Date(),
      activeConnections: this.getActiveConnections(),
      idleConnections: this.getIdleConnections(),
      queuedRequests: this.getQueuedRequests()
    };
    
    this.metrics.connections.push(connectionMetrics);
  }

  updateDashboard() {
    console.log('\n=== Performance Dashboard ===');
    console.log(`Time: ${new Date().toLocaleTimeString()}`);
    
    const latestQuery = this.metrics.queries[this.metrics.queries.length - 1];
    const latestConnection = this.metrics.connections[this.metrics.connections.length - 1];
    
    console.log('\nQuery Performance:');
    console.log(`  Average Response Time: ${latestQuery.avgResponseTime}ms`);
    console.log(`  Total Queries: ${latestQuery.totalQueries}`);
    console.log(`  Slow Queries: ${latestQuery.slowQueries}`);
    console.log(`  Error Rate: ${latestQuery.errorRate}%`);
    
    console.log('\nConnection Pool:');
    console.log(`  Active Connections: ${latestConnection.activeConnections}`);
    console.log(`  Idle Connections: ${latestConnection.idleConnections}`);
    console.log(`  Queued Requests: ${latestConnection.queuedRequests}`);
    
    // 알림 체크
    this.checkAlerts(latestQuery, latestConnection);
  }

  checkAlerts(queryMetrics, connectionMetrics) {
    if (queryMetrics.avgResponseTime > 1000) {
      console.warn('⚠️  High average response time detected!');
    }
    
    if (queryMetrics.errorRate > 5) {
      console.warn('⚠️  High error rate detected!');
    }
    
    if (connectionMetrics.queuedRequests > 20) {
      console.warn('⚠️  High queued requests detected!');
    }
  }

  // 실제 구현은 데이터베이스 연결에 맞게 수정
  getAverageResponseTime() { return Math.random() * 100; }
  getTotalQueries() { return Math.floor(Math.random() * 1000); }
  getSlowQueries() { return Math.floor(Math.random() * 10); }
  getErrorRate() { return Math.random() * 2; }
  getActiveConnections() { return Math.floor(Math.random() * 20); }
  getIdleConnections() { return Math.floor(Math.random() * 10); }
  getQueuedRequests() { return Math.floor(Math.random() * 5); }
}
```

---

## 결론

데이터베이스 성능 튜닝은 시스템의 전반적인 성능에 직접적인 영향을 미치는 중요한 작업입니다.

### 핵심 원칙 요약

1. **쿼리 최적화**: 실행 계획 분석을 통한 효율적인 쿼리 작성
2. **인덱스 설계**: 선택도 기반의 적절한 인덱스 설계 및 관리
3. **파티셔닝/샤딩**: 대용량 데이터 처리를 위한 분산 전략
4. **연결 풀 최적화**: 효율적인 연결 관리 및 모니터링
5. **지속적인 모니터링**: 성능 지표 추적 및 최적화

### 실무 적용 가이드

- **개발 단계**: 쿼리 최적화 및 인덱스 설계에 집중
- **테스트 단계**: 벤치마킹을 통한 성능 검증
- **운영 단계**: 모니터링 및 지속적인 튜닝

이러한 원칙들을 바탕으로 데이터베이스 성능을 최적화하여 안정적이고 확장 가능한 시스템을 구축하세요.

---

## 통합 최적화 예제

### 예제: 전자상거래 주문 시스템 성능 최적화

**시나리오**: 대용량 주문 데이터를 처리하는 전자상거래 시스템의 성능을 종합적으로 최적화

**초기 상태:**
- 주문 테이블: 1억 건
- 평균 쿼리 응답 시간: 2.5초
- 동시 사용자: 1,000명
- 데이터베이스 CPU 사용률: 85%

**최적화 전략:**

#### 1. 인덱스 최적화

```sql
-- 기존 인덱스 분석
SHOW INDEX FROM orders;

-- 복합 인덱스 추가 (쿼리 패턴 분석 후)
CREATE INDEX idx_orders_user_date_status ON orders(user_id, created_at DESC, status);

-- 커버링 인덱스 (자주 조회되는 컬럼만 포함)
CREATE INDEX idx_orders_list_covering ON orders(user_id, created_at, status, total_amount) 
INCLUDE (order_id, shipping_address);

-- 부분 인덱스 (활성 주문만)
CREATE INDEX idx_orders_active ON orders(user_id, created_at) 
WHERE status IN ('pending', 'processing', 'shipped');
```

#### 2. 쿼리 최적화

```javascript
// 최적화 전
async function getOrders(userId, page = 1) {
  const offset = (page - 1) * 20;
  
  // ❌ 비효율적: OFFSET 사용, 전체 스캔
  const query = `
    SELECT * FROM orders 
    WHERE user_id = ? 
    ORDER BY created_at DESC 
    LIMIT 20 OFFSET ${offset}
  `;
  
  return await db.query(query, [userId]);
}

// 최적화 후
async function getOrders(userId, lastOrderId = null, limit = 20) {
  if (lastOrderId) {
    // ✅ 커서 기반 페이지네이션
    const query = `
      SELECT order_id, user_id, created_at, status, total_amount, shipping_address
      FROM orders 
      WHERE user_id = ? 
        AND (created_at, order_id) < (
          SELECT created_at, order_id 
          FROM orders 
          WHERE order_id = ?
        )
      ORDER BY created_at DESC, order_id DESC
      LIMIT ?
    `;
    return await db.query(query, [userId, lastOrderId, limit]);
  } else {
    // 첫 페이지
    const query = `
      SELECT order_id, user_id, created_at, status, total_amount, shipping_address
      FROM orders 
      WHERE user_id = ? 
      ORDER BY created_at DESC, order_id DESC
      LIMIT ?
    `;
    return await db.query(query, [userId, limit]);
  }
}
```

#### 3. 연결 풀 최적화

```javascript
// database-pool.js
const mysql = require('mysql2/promise');

// 최적화된 연결 풀 설정
const pool = mysql.createPool({
  host: process.env.DB_HOST,
  user: process.env.DB_USER,
  password: process.env.DB_PASSWORD,
  database: process.env.DB_NAME,
  
  // 연결 풀 설정
  connectionLimit: 20,        // 최대 연결 수
  queueLimit: 0,              // 대기열 제한 없음
  waitForConnections: true,   // 연결 대기
  
  // 연결 최적화
  enableKeepAlive: true,
  keepAliveInitialDelay: 0,
  
  // 타임아웃 설정
  connectTimeout: 10000,
  acquireTimeout: 10000,
  timeout: 60000,
  
  // 재연결 설정
  reconnect: true,
  
  // 쿼리 최적화
  typeCast: function (field, next) {
    if (field.type === 'TINY' && field.length === 1) {
      return field.string() === '1';
    }
    return next();
  }
});

// 연결 풀 모니터링
setInterval(async () => {
  const stats = {
    total: pool.pool._allConnections.length,
    free: pool.pool._freeConnections.length,
    queued: pool.pool._connectionQueue.length
  };
  
  console.log('Connection Pool Stats:', stats);
  
  // 메트릭 전송
  await sendMetrics('db.pool.total', stats.total);
  await sendMetrics('db.pool.free', stats.free);
  await sendMetrics('db.pool.queued', stats.queued);
}, 60000);
```

#### 4. 캐싱 전략 통합

```javascript
// order-service.js
const Redis = require('ioredis');
const redis = new Redis(process.env.REDIS_URL);

class OrderService {
  // Cache-Aside 패턴
  async getOrder(orderId) {
    // 1. 캐시 확인
    const cacheKey = `order:${orderId}`;
    const cached = await redis.get(cacheKey);
    
    if (cached) {
      return JSON.parse(cached);
    }
    
    // 2. 데이터베이스 조회
    const order = await db.query(
      'SELECT * FROM orders WHERE order_id = ?',
      [orderId]
    );
    
    if (order) {
      // 3. 캐시 저장 (TTL: 5분)
      await redis.setex(cacheKey, 300, JSON.stringify(order));
    }
    
    return order;
  }
  
  // Write-Through 패턴
  async createOrder(orderData) {
    // 트랜잭션 시작
    const connection = await db.getConnection();
    await connection.beginTransaction();
    
    try {
      // 1. 데이터베이스에 저장
      const [result] = await connection.query(
        'INSERT INTO orders (user_id, items, total_amount, status) VALUES (?, ?, ?, ?)',
        [orderData.userId, JSON.stringify(orderData.items), orderData.total, 'pending']
      );
      
      const orderId = result.insertId;
      
      // 2. 캐시에 즉시 저장
      const cacheKey = `order:${orderId}`;
      await redis.setex(cacheKey, 300, JSON.stringify({
        order_id: orderId,
        ...orderData
      }));
      
      // 3. 사용자 주문 목록 캐시 무효화
      await redis.del(`user:${orderData.userId}:orders`);
      
      await connection.commit();
      return orderId;
    } catch (error) {
      await connection.rollback();
      throw error;
    } finally {
      connection.release();
    }
  }
  
  // 사용자 주문 목록 캐싱
  async getUserOrders(userId, page = 1) {
    const cacheKey = `user:${userId}:orders:${page}`;
    
    // 캐시 확인
    const cached = await redis.get(cacheKey);
    if (cached) {
      return JSON.parse(cached);
    }
    
    // 데이터베이스 조회
    const orders = await this.getOrders(userId, null, 20);
    
    // 캐시 저장 (TTL: 1분)
    await redis.setex(cacheKey, 60, JSON.stringify(orders));
    
    return orders;
  }
}
```

#### 5. 파티셔닝 적용

```sql
-- 주문 테이블을 날짜별로 파티셔닝
ALTER TABLE orders 
PARTITION BY RANGE (YEAR(created_at) * 100 + MONTH(created_at)) (
  PARTITION p202401 VALUES LESS THAN (202402),
  PARTITION p202402 VALUES LESS THAN (202403),
  PARTITION p202403 VALUES LESS THAN (202404),
  -- ... 계속 추가
  PARTITION p_future VALUES LESS THAN MAXVALUE
);

-- 파티션 프루닝 확인
EXPLAIN PARTITIONS 
SELECT * FROM orders 
WHERE created_at >= '2024-01-01' AND created_at < '2024-02-01';
```

#### 6. 통합 모니터링

```javascript
// performance-monitor.js
class DatabasePerformanceMonitor {
  constructor() {
    this.metrics = {
      queryTimes: [],
      slowQueries: [],
      connectionPool: {}
    };
  }
  
  // 쿼리 성능 추적
  async trackQuery(query, params, executionTime) {
    // 느린 쿼리 감지 (1초 이상)
    if (executionTime > 1000) {
      this.metrics.slowQueries.push({
        query,
        params,
        executionTime,
        timestamp: Date.now()
      });
      
      // 알림 전송
      await this.alertSlowQuery(query, executionTime);
    }
    
    // 메트릭 수집
    this.metrics.queryTimes.push(executionTime);
    
    // P95, P99 계산
    const sorted = [...this.metrics.queryTimes].sort((a, b) => a - b);
    const p95 = sorted[Math.floor(sorted.length * 0.95)];
    const p99 = sorted[Math.floor(sorted.length * 0.99)];
    
    // CloudWatch 또는 Prometheus로 전송
    await this.sendMetrics({
      'db.query.duration': executionTime,
      'db.query.p95': p95,
      'db.query.p99': p99,
      'db.query.slow_count': this.metrics.slowQueries.length
    });
  }
  
  // 실행 계획 분석
  async analyzeQueryPlan(query, params) {
    const explainQuery = `EXPLAIN ${query}`;
    const plan = await db.query(explainQuery, params);
    
    // 전체 테이블 스캔 감지
    if (plan[0].type === 'ALL') {
      console.warn('Full table scan detected:', query);
      await this.alertFullTableScan(query);
    }
    
    // 인덱스 미사용 감지
    if (!plan[0].key) {
      console.warn('No index used:', query);
    }
    
    return plan;
  }
  
  // 정기적인 성능 리포트
  async generatePerformanceReport() {
    const report = {
      timestamp: new Date(),
      averageQueryTime: this.calculateAverage(this.metrics.queryTimes),
      p95QueryTime: this.calculatePercentile(this.metrics.queryTimes, 95),
      p99QueryTime: this.calculatePercentile(this.metrics.queryTimes, 99),
      slowQueryCount: this.metrics.slowQueries.length,
      topSlowQueries: this.metrics.slowQueries
        .slice(-10)
        .map(q => ({
          query: q.query.substring(0, 100),
          time: q.executionTime
        }))
    };
    
    // 리포트 저장 및 알림
    await this.saveReport(report);
    await this.sendReport(report);
    
    return report;
  }
}
```

**최적화 결과:**
- 평균 쿼리 응답 시간: 2.5초 → 0.15초 (93% 개선)
- 데이터베이스 CPU 사용률: 85% → 45% (47% 감소)
- 동시 처리 가능 사용자: 1,000명 → 5,000명 (5배 증가)
- 캐시 히트율: 0% → 85%

---

## 참고 자료

### 공식 문서

**MySQL**
- [MySQL 8.0 Reference Manual](https://dev.mysql.com/doc/refman/8.0/en/) - MySQL 공식 문서
- [MySQL Performance Schema](https://dev.mysql.com/doc/refman/8.0/en/performance-schema.html) - 성능 모니터링
- [MySQL Optimization Guide](https://dev.mysql.com/doc/refman/8.0/en/optimization.html) - 최적화 가이드

**PostgreSQL**
- [PostgreSQL Documentation](https://www.postgresql.org/docs/) - PostgreSQL 공식 문서
- [Query Planning](https://www.postgresql.org/docs/current/planner-optimizer.html) - 쿼리 계획 및 옵티마이저
- [Performance Tips](https://www.postgresql.org/docs/current/performance-tips.html) - 성능 최적화 팁

**MariaDB**
- [MariaDB Knowledge Base](https://mariadb.com/kb/en/) - MariaDB 문서
- [Optimization and Tuning](https://mariadb.com/kb/en/optimization-and-tuning/) - 최적화 가이드

### 서적

**데이터베이스 성능 최적화**
- **"High Performance MySQL"** (Baron Schwartz 외)
  - MySQL 성능 최적화의 바이블
  - 실전 튜닝 사례 및 벤치마킹
  - [O'Reilly](https://www.oreilly.com/library/view/high-performance-mysql/9781492080503/)

- **"Database Internals"** (Alex Petrov)
  - 데이터베이스 내부 동작 원리
  - 스토리지 엔진, 인덱싱, 트랜잭션
  - [O'Reilly](https://www.databass.dev/)

- **"SQL Performance Explained"** (Markus Winand)
  - 실행 계획 분석 및 인덱스 최적화
  - 데이터베이스 독립적인 SQL 튜닝
  - [SQL Performance Explained](https://sql-performance-explained.com/)

**시스템 설계 및 아키텍처**
- **"Designing Data-Intensive Applications"** (Martin Kleppmann)
  - 대용량 데이터 처리 시스템 설계
  - 복제, 파티셔닝, 트랜잭션
  - [O'Reilly](https://dataintensive.net/)

- **"Database Reliability Engineering"** (Laine Campbell, Charity Majors)
  - 데이터베이스 운영 및 모니터링
  - SRE 관점의 데이터베이스 관리
  - [O'Reilly](https://www.oreilly.com/library/view/database-reliability-engineering/9781491925935/)

### 온라인 리소스

**학습 자료**
- [Use The Index, Luke!](https://use-the-index-luke.com/) - 인덱스 활용법 상세 가이드
- [PostgreSQL Performance Tuning](https://www.percona.com/blog/) - Percona 블로그
- [MySQL Performance Blog](https://www.percona.com/blog/category/mysql/) - MySQL 성능 최적화
- [DB Weekly](https://dbweekly.com/) - 데이터베이스 뉴스레터

**실행 계획 시각화**
- [PostgreSQL EXPLAIN Visualizer](https://explain.dalibo.com/) - 실행 계획 시각화 도구
- [MySQL Visual Explain](https://dev.mysql.com/doc/workbench/en/wb-performance-explain.html) - MySQL Workbench 시각화
- [pgAdmin](https://www.pgadmin.org/) - PostgreSQL 관리 도구

### 성능 모니터링 도구

**오픈소스 도구**
- [Prometheus](https://prometheus.io/) - 메트릭 수집 및 알림
- [Grafana](https://grafana.com/) - 데이터 시각화 대시보드
- [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) - PostgreSQL 쿼리 통계
- [MySQL Performance Schema](https://dev.mysql.com/doc/refman/8.0/en/performance-schema.html) - MySQL 성능 모니터링

**상용 도구**
- [Datadog](https://www.datadoghq.com/) - 통합 모니터링 플랫폼
- [New Relic](https://newrelic.com/) - APM 및 데이터베이스 모니터링
- [Percona Monitoring and Management](https://www.percona.com/software/database-tools/percona-monitoring-and-management) - 오픈소스 모니터링

### 벤치마킹 도구

- **sysbench** - 범용 성능 벤치마크
  - [GitHub](https://github.com/akopytov/sysbench)
  - OLTP 워크로드 시뮬레이션

- **pgbench** - PostgreSQL 벤치마크
  - [Documentation](https://www.postgresql.org/docs/current/pgbench.html)
  - TPC-B 유사 워크로드

- **mysqlslap** - MySQL 부하 테스트
  - [Documentation](https://dev.mysql.com/doc/refman/8.0/en/mysqlslap.html)
  - 동시성 시뮬레이션

### 커뮤니티 및 포럼

**Stack Overflow**
- [Database Administrators](https://dba.stackexchange.com/) - DB 전문가 Q&A
- [Stack Overflow - MySQL](https://stackoverflow.com/questions/tagged/mysql)
- [Stack Overflow - PostgreSQL](https://stackoverflow.com/questions/tagged/postgresql)

**Reddit**
- [r/PostgreSQL](https://www.reddit.com/r/PostgreSQL/)
- [r/MySQL](https://www.reddit.com/r/mysql/)
- [r/Database](https://www.reddit.com/r/Database/)

### 기술 블로그

**기업 엔지니어링 블로그**
- [Uber Engineering](https://eng.uber.com/) - 대규모 데이터베이스 운영 사례
- [Netflix Tech Blog](https://netflixtechblog.com/) - 분산 데이터베이스 아키텍처
- [Pinterest Engineering](https://medium.com/@Pinterest_Engineering) - 샤딩 및 스케일링
- [Airbnb Engineering](https://medium.com/airbnb-engineering) - 데이터베이스 마이그레이션

**데이터베이스 벤더 블로그**
- [Percona Blog](https://www.percona.com/blog/) - MySQL/PostgreSQL 전문 기술
- [MariaDB Blog](https://mariadb.com/resources/blog/) - MariaDB 최신 기능
- [AWS Database Blog](https://aws.amazon.com/blogs/database/) - 클라우드 데이터베이스

### 컨퍼런스 및 밋업

**주요 컨퍼런스**
- **Percona Live** - MySQL/PostgreSQL 전문 컨퍼런스
- **PostgreSQL Conference** - PostgreSQL 커뮤니티 이벤트
- **Oracle OpenWorld** - Oracle 및 MySQL 발표
- **AWS re:Invent** - AWS 데이터베이스 서비스

**온라인 강의**
- [Udemy - Database Design and Optimization](https://www.udemy.com/topic/database-design/) - 데이터베이스 설계
- [Coursera - Database Management](https://www.coursera.org/courses?query=database) - 데이터베이스 관리
- [LinkedIn Learning - Database Administration](https://www.linkedin.com/learning/topics/database-administration) - DB 관리

### 실무 체크리스트

**쿼리 최적화 체크리스트**
```
☐ SELECT * 사용하지 않기
☐ WHERE 절에 함수 적용 피하기
☐ 적절한 인덱스 생성 확인
☐ 복합 인덱스 컬럼 순서 검토
☐ 실행 계획 분석 수행
☐ N+1 쿼리 문제 확인
☐ 적절한 JOIN 방식 선택
☐ 서브쿼리 vs JOIN 성능 비교
```

**인덱스 설계 체크리스트**
```
☐ WHERE 절 컬럼 인덱싱
☐ JOIN 키 컬럼 인덱싱
☐ ORDER BY 컬럼 인덱싱
☐ 선택도 높은 컬럼 우선
☐ 복합 인덱스 순서 최적화
☐ 사용되지 않는 인덱스 제거
☐ 인덱스 조각화 모니터링
☐ 통계 정보 정기 업데이트
```

**성능 모니터링 체크리스트**
```
☐ 느린 쿼리 로그 활성화
☐ 쿼리 응답 시간 측정 (P95, P99)
☐ 데이터베이스 CPU 사용률
☐ 메모리 사용률 및 캐시 히트율
☐ 디스크 I/O 대기 시간
☐ 연결 수 및 연결 풀 상태
☐ 락 대기 시간 및 데드락
☐ 테이블 및 인덱스 크기
```

---

## 📝 결론

데이터베이스 성능 튜닝은 지속적인 모니터링과 최적화를 통해 시스템의 성능을 향상시키는 핵심 활동입니다.

### 핵심 포인트

- ✅ **쿼리 최적화**: 실행 계획 분석 및 인덱스 활용
- ✅ **인덱스 설계**: 선택도 높은 컬럼 우선, 복합 인덱스 전략
- ✅ **파티셔닝 및 샤딩**: 대용량 데이터 분산 처리
- ✅ **연결 풀 관리**: 적절한 풀 크기 및 모니터링
- ✅ **정기적 벤치마킹**: 성능 변화 추적 및 개선

### 모범 사례

1. **점진적 최적화**: 한 번에 모든 것을 최적화하지 말고 단계적으로 접근
2. **측정 기반 결정**: 추측이 아닌 실제 메트릭 기반으로 최적화
3. **정기적 모니터링**: 성능 지표를 지속적으로 추적
4. **인덱스 관리**: 사용하지 않는 인덱스 제거, 정기적 재구성
5. **용량 계획**: 데이터 증가에 따른 성능 저하 예방

### 관련 문서

- [데이터베이스 모니터링 및 성능 분석](./모니터링_및_성능_분석.md) - 성능 모니터링 전략
- [데이터베이스 백업 및 복구](./백업_및_복구_전략.md) - 백업 전략
- [데이터베이스 복제 전략](./복제_전략.md) - 고가용성 및 읽기 분산
- [데이터베이스 마이그레이션 전략](./마이그레이션_전략.md) - 스키마 변경 전략
- [RDBMS에서의 index](./RDBMS에서의 index.md) - 인덱스 상세 가이드
- [연결 풀 관리](../../Framework/Node/데이터베이스/연결_풀_관리.md) - Node.js 연결 풀 관리

---


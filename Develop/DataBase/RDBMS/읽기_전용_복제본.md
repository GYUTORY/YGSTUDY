---
title: 읽기 전용 복제본
tags: [database, rdbms, replication, read-replica, mysql, postgresql]
updated: 2025-12-09
---
...
**실무 팁:**
읽기 전용 복제본은 읽기 작업 부하를 분산시킨다. 마스터는 쓰기 작업만 처리하고, 복제본은 읽기 작업을 처리한다.
...
**실무 팁:**
복제본은 읽기 작업 부하를 분산시킨다. 마스터 장애 시 복제본을 마스터로 승격시킬 수 있다.
...
**실무 팁:**
자동 복구 설정은 복제 지연을 줄이고 안정성을 향상시킨다. 병렬 복제를 사용하면 성능이 향상된다.
...
**실무 팁:**
읽기/쓰기 분리는 라운드 로빈 방식으로 복제본을 선택한다. 복제 지연을 고려하여 마스터에서 읽을 수도 있다.
...
**실무 팁:**
세션 기반 분리는 같은 세션에서 같은 복제본을 사용한다. 일관성을 향상시킬 수 있다.
...
**실무 팁:**
지연 인식 라우팅은 복제 지연이 적은 복제본을 선택한다. 모든 복제본이 지연 중이면 마스터에서 읽는다.
...
**실무 팁:**
일관성 보장 읽기는 최근 쓰기가 있었을 때 마스터에서 읽는다. 최신 데이터가 필요한 경우에 사용한다.
...
**실무 팁:**
가중치 기반 로드 밸런싱은 서버 성능에 따라 가중치를 설정한다. 성능이 좋은 서버에 더 많은 요청을 분산시킨다.
...
**실무 팁:**
응답 시간 기반 로드 밸런싱은 응답 시간이 짧은 복제본을 선택한다. 실시간으로 응답 시간을 모니터링한다.
...
**실무 팁:**
Sequelize는 읽기/쓰기 분리를 지원한다. 마스터와 복제본 연결을 분리하여 사용한다.
...
**실무 팁:**
TypeORM은 여러 연결을 관리할 수 있다. 마스터와 복제본 연결을 분리하여 사용한다.
...
**실무 팁:**
Prisma는 여러 클라이언트를 생성하여 읽기/쓰기를 분리한다. 마스터와 복제본 클라이언트를 분리한다.
...
**실무 팁:**
복제 상태를 지속적으로 모니터링한다. 복제 지연, 에러율, 연결 상태를 확인한다.
...
**실무 팁:**
자동 장애 복구는 복제본 장애를 감지하고 자동으로 복구한다. 장애 복제본은 읽기에서 제외한다.
...
**실무 팁:**
읽기 전용 복제본은 읽기 작업 부하를 분산시킨다. 복제 지연을 모니터링하고, 지연이 심할 때는 마스터에서 읽는다.

...
읽기 전용 복제본(Read-Only Replica)은 마스터 데이터베이스의 데이터를 복제하여 읽기 작업의 부하를 분산시키는 기술이다.
...
**실무 팁:**
MySQL 복제는 바이너리 로그를 사용한다. GTID를 사용하면 복제 관리가 쉬워진다.
...
**실무 팁:**
PostgreSQL 복제는 WAL(Write-Ahead Logging)을 사용한다. 복제 슬롯을 사용하면 WAL 삭제를 방지할 수 있다.
...
### 그 전에 — 복제 지연은 버그가 아니라 전제다

읽기 분리를 붙이면 반드시 만나는 것이 **복제 지연(replication lag)** 이다. 마스터가 커밋한 내용이 복제본에 도착하기까지 시간이 걸리고, 비동기 복제인 이상 이 값은 0이 되지 않는다. 평소에는 눈에 안 띄다가 대량 UPDATE, 인덱스 생성, 배치 작업이 돌 때 급격히 벌어진다.

**가장 먼저 터지는 건 "저장 직후 조회"다.**

```
1. 사용자가 프로필을 수정한다        → 마스터에 쓰기
2. 화면이 프로필을 다시 불러온다      → 복제본에서 읽기
3. 복제본에는 아직 반영 전            → 옛날 값이 그대로 보인다
```

사용자 눈에는 "저장 버튼을 눌렀는데 안 바뀌었다"로 보인다. 새로고침하면 고쳐지니 재현도 잘 안 되고, 로그에도 에러가 안 남는다. 읽기 분리를 도입한 뒤 들어오는 버그 제보의 상당수가 이 모양이다.

그래서 **쿼리를 어디로 보낼지는 "이 데이터가 잠깐 낡아도 되는가"로 판단한다.**

| 마스터로 | 복제본으로 |
|---------|-----------|
| 방금 쓴 것을 다시 읽을 때 | 목록·검색·통계 |
| 트랜잭션 안의 SELECT | 대시보드, 리포트 |
| 잔액·재고처럼 정확해야 하는 값 | 공지, 카테고리 같은 잘 안 변하는 것 |
| 쓰기 판단의 근거가 되는 조회 | 캐시를 채우기 위한 조회 |

특히 **트랜잭션 안의 SELECT 는 예외 없이 마스터로 보낸다.** 트랜잭션은 한 커넥션 위에서 성립하는데, 중간에 복제본으로 새면 그 SELECT 는 트랜잭션 밖에서 실행되는 셈이라 격리 수준이 통째로 무너진다. 아래 "트랜잭션 인식" 절이 그 처리다.

**분리가 실제로 되고 있는지는 따로 확인해야 한다.** 설정에 reader 엔드포인트를 적어 뒀는데 값이 writer 를 가리키고 있으면, 코드는 멀쩡히 도는데 부하 분산 효과는 0이다. 에러가 안 나니 아무도 모른다. 배포 후 한 번은 복제본 쪽 커넥션 수나 QPS 가 실제로 올라가는지 봐야 한다.

**복제본이 죽었을 때 무조건 마스터로 넘기는 것도 위험하다.** 읽기 부하 전체가 마스터로 쏟아지면 마스터까지 같이 넘어간다. 폴백을 넣을 거면 상한을 두거나, 일부 기능은 차라리 실패시키는 편이 낫다.
...
**실무 팁:**
트랜잭션 내에서는 마스터에서 읽는다. 복제 지연으로 인한 데이터 불일치를 방지한다.
...
**실무 팁:**
복제 지연을 모니터링하여 지연이 심할 때는 마스터에서 읽는다. 지연 임계값을 설정한다.
...
읽기 전용 복제본을 활용하면 데이터베이스의 성능과 가용성을 향상시킬 수 있다. 올바른 구현과 모니터링을 통해 안정적이고 확장 가능한 시스템을 구축할 수 있다.
